...
/* ── Fantasy Music System ── */
const MUSIC = (() => {
  let ctx=null,mGain=null,schedNodes=[],beatTimer=null,curTrack=null,fadeTmr=null;
  const F={C2:65.4,D2:73.4,E2:82.4,F2:87.3,G2:98,A2:110,Bb2:116.5,C3:130.8,D3:146.8,Eb3:155.6,
    E3:164.8,F3:174.6,G3:196,Ab3:207.7,A3:220,Bb3:233.1,B3:246.9,C4:261.6,D4:293.7,Eb4:311.1,
    E4:329.6,F4:349.2,G4:392,Ab4:415.3,A4:440,Bb4:466.2,B4:493.9,C5:523.3,D5:587.3,E5:659.3,
    F5:698.5,G5:784,A5:880};
  const init=()=>{
    if(ctx)return true;
    try{ ctx=new(window.AudioContext||window.webkitAudioContext)();
      mGain=ctx.createGain(); mGain.gain.value=0; mGain.connect(ctx.destination);
      // Feedback reverb tail
      const dl=ctx.createDelay(0.7); dl.delayTime.value=0.38;
      const df=ctx.createGain(); df.gain.value=0.28;
      const dv=ctx.createGain(); dv.gain.value=0.18;
      dl.connect(df); df.connect(dl); dl.connect(dv); dv.connect(ctx.destination);
      mGain.connect(dl); return true;
    }catch(e){ return false; }
  };
  const n=(hz,t,dur,vol=.18,type='sawtooth',dt=0)=>{
    if(!ctx||!hz) return;
    const o=ctx.createOscillator(),g=ctx.createGain();
    o.type=type; o.frequency.value=hz; o.detune.value=dt;
    o.connect(g); g.connect(mGain);
    const at=t+0.005;
    g.gain.setValueAtTime(0,t); g.gain.linearRampToValueAtTime(vol,at+0.04);
    g.gain.setValueAtTime(vol*0.72,at+dur*0.5); g.gain.linearRampToValueAtTime(0,at+dur);
    o.start(t); o.stop(at+dur+0.08);
    schedNodes.push(o,g);
  };
  // Play a chord (pad layer)
  const chord=(freqs,t,dur,vol=.1)=>freqs.forEach((hz,i)=>n(hz,t+i*.005,dur,vol,'sawtooth',i*5-5));
  ...
  const kick=(t,vol=.22)=>{
    ...
    o.connect(g); g.connect(mGain); o.start(t); o.stop(t+.2);
  };

  // ── PATTERNS: [freq, beats, vol, type] ──
  // Each pattern plays at bpm, then loops
  const TRACKS = {
    // ★ MAIN MENU — "Kingdom of Verandah" (D minor, 88 BPM, Epic)
    // Catchy hook: D4-F4-A4-G4-F4-E4-D4, then bridge rises to D5
    menu: { bpm:88, bars:8, layers:[
      // Lead melody — triangle (flute-like)
      { type:'triangle', vol:.22, seq:[
        [F.D4,1.5],[F.F4,.5],[F.A4,1],[F.G4,1],          // bar 1 — rising call
        [F.F4,.5],[F.E4,.5],[F.D4,1],[null,2],            // bar 2 — falling, rest
        [F.G4,1],[F.A4,1],[F.C5,1],[F.Bb4,1],            // bar 3 — answer
        [F.A4,.5],[F.G4,.5],[F.F4,1],[F.E4,1],[F.D4,1],  // bar 4 — descent
        [F.D5,1],[F.C5,1],[F.Bb4,1],[F.A4,1],            // bar 5 — elevation!
        [F.G4,1],[F.A4,1],[F.C5,2],                       // bar 6 — hold
        [F.Bb4,.5],[F.A4,.5],[F.G4,.5],[F.F4,.5],[F.E4,1],[F.D4,1], // bar 7 — ornament
        [F.D4,1],[F.F4,1],[F.A4,1],[F.D5,1],              // bar 8 — triumphant close
      ]},
      // Harmony strings (sawtooth pad) — chord changes
      { type:'sawtooth', vol:.09, seq:[
        [F.D3,4],[F.A2,4],  // dm
        [F.F3,4],[F.C3,4],  // F major
        [F.G3,4],[F.A3,4],  // G, Am
        [F.D3,4],[F.D3,4],  // dm resolution
        [F.Bb2,4],[F.F2,4], // Bb - dramatic lift
        [F.G3,4],[F.A3,4],  // Gm Am
        [F.Eb3,4],[F.C3,4], // Eb C — tension
        [F.D3,4],[F.A2,4],  // back home
      ]},
      // Bass (sine)
      { type:'sine', vol:.28, seq:[
        [F.D2,2],[F.A2,2],[F.F2,2],[F.C2,2],
        [F.D2,2],[F.A2,2],[F.F2,2],[F.G2,2],
        [F.G2,2],[F.A2,2],[F.C3,2],[F.Bb2,2],
        [F.A2,2],[F.G2,2],[F.F2,2],[F.D2,2],
        [F.Bb2,2],[F.F2,2],[F.Bb2,2],[F.A2,2],
        [F.G2,2],[F.A2,2],[F.C3,2],[F.C3,2],
        [F.Eb3,2],[F.C3,2],[F.A2,2],[F.G2,2],
        [F.D2,2],[F.F2,2],[F.A2,2],[F.D3,2],
      ]},
    ]},

    // ★ OVERWORLD — "Red Oak Grove" (C major, 108 BPM, adventurous)
    overworld: { bpm:108, bars:4, layers:[
      { type:'triangle', vol:.18, seq:[
        [F.E4,1],[F.G4,1],[F.A4,.5],[F.G4,.5],[F.E4,1],  // bar 1
        [F.D4,1],[F.E4,1],[F.G4,2],                       // bar 2
        [F.A4,1],[F.G4,.5],[F.E4,.5],[F.D4,1],[F.C4,1],  // bar 3
        [F.G3,1],[F.A3,1],[F.E4,2],                       // bar 4
      ]},
      { type:'sawtooth', vol:.08, seq:[
        [F.C3,4],[F.G3,4],[F.F3,4],[F.G3,4],
      ]},
      { type:'sine', vol:.22, seq:[
        [F.C2,2],[F.G2,2],[F.F2,2],[F.G2,2],
        [F.A2,2],[F.E2,2],[F.F2,2],[F.C2,2],
      ]},
    ]},

    // ★ DUNGEON — "Ancient Root" (E minor, 72 BPM, dark/tense)
    dungeon: { bpm:72, bars:4, layers:[
      { type:'triangle', vol:.14, seq:[
        [F.E4,1.5],[F.D4,.5],[F.B3,1],[F.G3,1],
        [F.A3,1],[F.G3,1],[F.E3,2],
        [F.B3,1],[F.C4,.5],[F.B3,.5],[F.A3,1],[F.G3,1],
        [F.E3,2],[null,2],
      ]},
      { type:'sawtooth', vol:.07, seq:[
        [F.E3,4],[F.D3,4],[F.G3,4],[F.A3,4],
      ]},
      { type:'sine', vol:.26, seq:[
        [F.E2,3],[null,1],[F.A2,2],[F.D2,2],
        [F.G2,2],[F.E2,2],[F.A2,2],[F.B2,2],
      ]},
    ]},

    // ★ COMBAT / BOSS — "Face the Warden" (A minor, 140 BPM, intense)
    boss: { bpm:140, bars:4, layers:[
      { type:'square', vol:.16, seq:[
        [F.A4,.5],[F.A4,.5],[F.C5,1],[F.B4,.5],[F.A4,.5],[F.G4,1],
        [F.F4,.5],[F.G4,.5],[F.A4,1],[F.A4,2],
        [F.E4,.5],[F.E4,.5],[F.G4,1],[F.F4,.5],[F.E4,.5],[F.D4,1],
        [F.A3,.5],[F.C4,.5],[F.E4,1],[F.A4,2],
      ]},
      { type:'sawtooth', vol:.1, seq:[
        [F.A3,2],[F.G3,2],[F.F3,2],[F.E3,2],
        [F.D3,2],[F.E3,2],[F.A3,2],[F.A3,2],
      ]},
      { type:'sine', vol:.3, seq:[
        [F.A2,1],[null,1],[F.A2,1],[null,1],
        [F.G2,1],[null,1],[F.A2,2],
        [F.F2,1],[null,1],[F.E2,2],[null,1],
        [F.A1||55,1],[null,1],[F.A2,2],
      ]},
    ]},

    // ★ FROST ZONE — "Frostfang" (B minor, 80 BPM, crystalline)
    frost: { bpm:80, bars:4, layers:[
      { type:'triangle', vol:.16, seq:[
        [F.B4,1],[F.D5,1],[F.A4,1],[F.G4,1],
        [F.F5||F.F4*2,1],[F.E5||659,1],[F.D5,2],
        [F.C5||523,1],[F.B4,1],[F.A4,1],[F.G4,1],
        [F.E4,2],[F.B3,2],
      ]},
      { type:'sine', vol:.22, seq:[
        [F.B2,2],[F.F2,2],[F.A2,2],[F.G2,2],
        [F.B2,2],[F.D2,2],[F.E2,2],[F.B2,2],
      ]},
    ]},

    // ★ EMBER/STORM/HOLLOW — "The Void Stirs" (D minor, 70 BPM, dark/ominous)
    dark: { bpm:70, bars:4, layers:[
      { type:'triangle', vol:.13, seq:[
        [F.D4,2],[F.Eb4,2],[F.D4,1.5],[F.C4,.5],[F.Bb3,1],
        [F.A3,2],[null,2],
        [F.G3,1],[F.A3,1],[F.Bb3,1],[F.A3,1],
        [F.D3,4],
      ]},
      { type:'sawtooth', vol:.06, seq:[
        [F.D3,4],[F.Eb3,4],[F.C3,4],[F.D3,4],
      ]},
      { type:'sine', vol:.28, seq:[
        [F.D2,4],[F.Eb2||77.8,4],[F.C2,4],[F.D2,4],
      ]},
    ]},
  };

  let loopPos=0, loopBeat=0, loopTimerId=null;
  const schedLook=0.12; // look-ahead seconds
  
  const stopCurrent=()=>{
    if(loopTimerId){ clearTimeout(loopTimerId); loopTimerId=null; }
    schedNodes.forEach(nd=>{ try{ if(nd.stop) nd.stop(ctx.currentTime+.05); } catch(e){} });
    schedNodes=[];
  };

  const playTrack=(name,fadeSecs=1.5)=>{
    if(!init()||ctx.state==="suspended")return;
    if(curTrack===name) return;
    curTrack=name;
    stopCurrent();
    const trk=TRACKS[name]; if(!trk) return;
    const beat=60/trk.bpm;
    // Fade in
    mGain.gain.cancelScheduledValues(ctx.currentTime);
    mGain.gain.setValueAtTime(0,ctx.currentTime);
    mGain.gain.linearRampToValueAtTime(1,ctx.currentTime+fadeSecs);

    let schedTo=ctx.currentTime+schedLook;
    const doSchedule=()=>{
      if(!ctx) return;
      const now=ctx.currentTime;
      while(schedTo < now + 0.3){
        const barPos=loopBeat%(trk.bars*4); // position in bars (4 beats per bar)
        trk.layers.forEach(layer=>{
          let t=0; let idx=0;
          layer.seq.forEach(([hz,beats])=>{ if(t===barPos){ n(hz,schedTo,beats*beat*.9,layer.vol,layer.type); } t+=beats; });
          // Kick on beat 1 and 3 for boss/dark
          if((name==="boss"||name==="dark")&&(loopBeat%4===0||loopBeat%4===2)) kick(schedTo,.12);
        });
        schedTo+=beat; loopBeat++;
      }
      loopTimerId=setTimeout(doSchedule, 60);
    };
    loopBeat=0; doSchedule();
  };

  const stop=(fade=2)=>{
    if(!ctx)return;
    curTrack=null; stopCurrent();
    mGain.gain.cancelScheduledValues(ctx.currentTime);
    mGain.gain.linearRampToValueAtTime(0,ctx.currentTime+fade);
  };

  const zoneTrack=(zone)=>{
    if(zone==="dungeon") return "dungeon";
    if(zone==="frost"||zone==="ironpeak") return "frost";
    if(zone==="ember"||zone==="scorch"||zone==="storm"||zone==="stormspire"||zone==="hollow") return "dark";
    return "overworld";
  };

  return { play:playTrack, stop, zone:zoneTrack, resume:()=>{ if(ctx?.state==="suspended") ctx.resume(); } };
})();

/* ── Items ── */
const ITEMS = {
  hunter_bow:      { name:"Hunter's Bow",       icon:"bow", rar:"common",    type:"weapon",   slot:"weapon",   dmg:13, dex:3,            price:55,  desc:"A reliable recurve for forest hunters." },
    iron_sword:      { name:"Iron Shortsword",   icon:"dagger", rar:"common",    type:"weapon",   slot:"weapon",   dmg:14, str:2,            price:60,  desc:"A reliable blade for new Wardens." },
  steel_sword:     { name:"Steel Longsword",   icon:"sword", rar:"uncommon",  type:"weapon",   slot:"weapon",   dmg:24, str:4,            price:180, desc:"Forged in the furnaces of Stonewatch." },
  warden_blade:    { name:"Wardenlight Blade", icon:"relic",  rar:"epic",      type:"weapon",   slot:"weapon",   dmg:40, str:6, int:4,     price:900, desc:"Hums with ancient relic-light." },
  heartwood_edge:  { name:"Heartwood Edge",    icon:"wood", rar:"legendary",  type:"weapon",   slot:"weapon",   dmg:54, str:8, int:6, maxHp:20, price:1600, desc:"Hewn from Thornheart's core. The grove's last gift." },
  hunter_bow:      { name:"Oakwood Bow",       icon:"bow", rar:"common",    type:"weapon",   slot:"weapon",   dmg:18, dex:4,            price:90,  desc:"Crafted by Red Oak rangers." },
  ash_staff:       { name:"Ashwood Staff",     icon:"staff", rar:"uncommon",  type:"weapon",   slot:"weapon",   dmg:12, int:8, maxMana:20, price:150, desc:"Channels arcane energy." },
  bandit_blade:    { name:"Bandit's Cutlass",  icon:"knife", rar:"uncommon",  type:"weapon",   slot:"weapon",   dmg:20, dex:2,            price:140, desc:"A scoundrel's blade, still sharp." },
  leather_helm:    { name:"Leather Helm",      icon:"helm", rar:"common",    type:"armor",    slot:"helmet",   col:0x7a5a32, def:4,                    price:45,  desc:"Basic head protection." },
  iron_helm:       { name:"Iron Helm",         icon:"helm", rar:"uncommon",  type:"armor",    slot:"helmet",   col:0x9aa0aa, def:8,  str:2,            price:120, desc:"Heavy and sturdy." },
  ancient_circlet: { name:"Ancient Circlet",   icon:"crown", rar:"rare",      type:"armor",    slot:"helmet",   col:0xc9a23a, crest:0x66ccff, def:5,  int:6, maxMana:15, price:280, desc:"Carved with runes of the old Wardens." },
  leather_vest:    { name:"Leather Vest",      icon:"vest", rar:"common",    type:"armor",    slot:"chest",    col:0x7a5a32, def:8,                    price:80,  desc:"Light and flexible." },
  chainmail:       { name:"Chainmail Hauberk", icon:"plate", rar:"uncommon",  type:"armor",    slot:"chest",    col:0x8a909a, def:15, str:2,            price:220, desc:"Interlocked iron rings." },
  warden_robes:    { name:"Warden Robes",      icon:"robe", rar:"epic",      type:"armor",    slot:"chest",    col:0x3a6e9a, def:10, int:6, maxMana:30, price:450, desc:"The robes of a Warden of old." },
  leather_gloves:  { name:"Leather Gloves",    icon:"glove", rar:"common",    type:"armor",    slot:"gloves",   def:3,  dex:2,            price:40,  desc:"Supple traveling gloves." },
  iron_gauntlets:  { name:"Iron Gauntlets",    icon:"gaunt", rar:"uncommon",  type:"armor",    slot:"gloves",   def:6,  str:4,            price:110, desc:"Heavy iron fist-covers." },
  trail_boots:     { name:"Trail Boots",       icon:"boot", rar:"common",    type:"armor",    slot:"boots",    def:3,  dex:2,            price:45,  desc:"Well-worn travel boots." },
  iron_boots:      { name:"Iron Boots",        icon:"boot", rar:"uncommon",  type:"armor",    slot:"boots",    def:6,  str:2,            price:130, desc:"Iron-shod footwear." },
  silver_ring:     { name:"Silver Ring",       icon:"ring", rar:"uncommon",  type:"armor",    slot:"ring",     def:1,  int:4,            price:120, desc:"Inscribed with minor wards." },
  ruby_ring:       { name:"Ruby Ring",         icon:"ring", rar:"rare",      type:"armor",    slot:"ring",     str:4,  maxHp:10,         price:280, desc:"Set with Ironpeak rubies." },
  jade_ring:       { name:"Jade Ring",         icon:"ring", rar:"uncommon",  type:"armor",    slot:"ring",     dex:4,                    price:150, desc:"Cool jade, swifter steps." },
  jade_pendant:    { name:"Jade Pendant",      icon:"neck", rar:"uncommon",  type:"armor",    slot:"necklace", maxHp:20,                 price:150, desc:"A traveler's lucky charm." },
  scholars_chain:  { name:"Scholar's Chain",   icon:"neck", rar:"rare",      type:"armor",    slot:"necklace", int:6,  maxMana:25,       price:320, desc:"Heavy with arcane knowledge." },
  wardens_seal:    { name:"Warden's Seal",     icon:"seal", rar:"epic",      type:"armor",    slot:"necklace", str:4,  int:4, maxHp:30,  price:600, desc:"The seal of the ancient Wardens." },
  potion_minor:    { name:"Minor Draught",     icon:"potion", rar:"common",    type:"potion",   heal:40,                                  price:25,  desc:"Restores 40 HP." },
  potion_greater:  { name:"Greater Draught",   icon:"potion2", rar:"uncommon",  type:"potion",   heal:90,                                  price:70,  desc:"Restores 90 HP." },
  mana_potion:     { name:"Mana Phial",        icon:"mana", rar:"uncommon",  type:"potion",   mana:50,                                  price:35,  desc:"Restores 50 mana." },
  bread:           { name:"Hearth Bread",      icon:"bread", rar:"common",    type:"potion",   heal:18,                                  price:8,   desc:"Restores 18 HP." },
  corrupt_shard:   { name:"Corruption Shard",  icon:"shard", rar:"rare",      type:"material",                                          price:40,  desc:"A splinter of the Hollow King's spread." },
  herb:            { name:"Glowmoss Herb",     icon:"herb", rar:"common",    type:"material",                                          price:6,   desc:"Used by alchemists." },
  wolf_pelt:       { name:"Wolf Pelt",         icon:"pelt", rar:"common",    type:"material",                                          price:12,  desc:"Thick grey fur." },
  bandit_badge:    { name:"Bandit Badge",      icon:"badge", rar:"uncommon",  type:"material",                                          price:18,  desc:"Proof of a rogue dispatched." },
  bone_fragment:   { name:"Bone Fragment",     icon:"bone", rar:"common",    type:"material",                                          price:8,   desc:"Crumbled skeleton remains." },
  treant_heart:    { name:"Treant Heartwood",  icon:"heartwood", rar:"epic",      type:"material",                                          price:200, desc:"The core of a Thornheart Treant." },
  relic_frag:      { name:"Heartstone Fragment", icon:"gem", rar:"legendary", type:"quest",                                           price:0,   desc:"A shard of the cracking Heartstone." },
  earth_relic:     { name:"Earth Relic",       icon:"relic_earth", rar:"legendary", type:"quest",                                            price:0,   desc:"One of the Four Relics. The land itself thrums through it." },
  ember_blade:     { name:"Emberbrand Sword",   icon:"fire", rar:"epic",      type:"weapon",  slot:"weapon",  atk:38, str:5,            price:560, desc:"Forged in volcanic stone. Burns on contact." },
  obsidian_plate:  { name:"Obsidian Plate",      icon:"plate", rar:"epic",      type:"armor",   slot:"chest",   col:0x2a2230, def:28, maxHp:25,          price:620, desc:"Volcanic glass armour, near-impervious." },
  bronze_helm:     { name:"Bronze Helm",        icon:"helm", rar:"common",    type:"armor",    slot:"helmet",   col:0xb87a3a, def:5,  str:1,            price:60,  desc:"Burnished bronze, dented but proud." },
  verdant_plate:   { name:"Verdant Plate",      icon:"plate", rar:"rare",      type:"armor",    slot:"chest",    col:0x3a8a52, def:20, dex:3, maxHp:15,  price:430, desc:"Living bark hardened to steel." },
  crimson_helm:    { name:"Crimson Warhelm",    icon:"helm", rar:"epic",      type:"armor",    slot:"helmet",   col:0x9a2a22, crest:0xffaa33, def:14, str:4, maxHp:12, price:520, desc:"A red-plumed helm of the old guard." },
  gilded_plate:    { name:"Gilded Cuirass",     icon:"plate", rar:"legendary", type:"armor",    slot:"chest",    col:0xd4af37, def:34, str:5, maxHp:30,  price:1500, desc:"Gold-chased warden plate that turns blades." },
  lava_ring:       { name:"Lava Band",            icon:"ring", rar:"rare",      type:"armor",   slot:"ring1",   str:6,  def:4,             price:320, desc:"Still warm from the forge-pits of Emberveil." },
  flame_relic:     { name:"Flame Relic",          icon:"fire", rar:"legendary", type:"quest",                                             price:0,   desc:"One of the Four Relics. Burns with ancient purpose." },
  storm_relic:     { name:"Storm Relic",          icon:"storm", rar:"legendary", type:"quest",                                             price:0,   desc:"The last of the Four Relics. Crackling with caged thunder." },
  tempest_glaive:  { name:"Tempest Glaive",       icon:"storm", rar:"legendary", type:"weapon", slot:"weapon", dmg:60, dex:6, str:6, maxMana:20, price:1800, desc:"A storm-forged blade that hums with lightning." },
  stormcaller_staff:{name:"Stormcaller Staff",    icon:"bolt", rar:"legendary", type:"weapon", slot:"weapon", dmg:48, int:12, maxMana:50, price:1800, desc:"Calls the sky's wrath down upon foes." },
  storm_plate:     { name:"Tempest Plate",        icon:"plate", rar:"legendary", type:"armor",  slot:"chest",  col:0x4a6a9a, def:32, dex:4, maxHp:30, price:1500, desc:"Warden plate humming with static." },
  storm_helm:      { name:"Galecrown Helm",       icon:"helm", rar:"epic",      type:"armor",  slot:"helmet", col:0x5a7ab0, crest:0x66ddff, def:16, dex:3, maxMana:15, price:560, desc:"Crowned with a crackling blue plume." },
  storm_ring:      { name:"Thunder Signet",       icon:"gem", rar:"epic",      type:"armor",  slot:"ring",   dex:6, int:4, maxMana:20, price:680, desc:"A ring that tingles before the storm." },
    frost_relic:     { name:"Frost Relic",       icon:"frost", rar:"legendary", type:"quest",                                            price:0,   desc:"One of the Four Relics. Cold and patient as a glacier." },
  frost_blade:     { name:"Frostbite Edge",    icon:"dagger", rar:"rare",      type:"weapon",   slot:"weapon",   dmg:34, str:4, int:4, price:520, desc:"Its edge never warms; wounds it makes go numb." },
  rime_plate:      { name:"Rimeguard Plate",   icon:"plate", rar:"rare",      type:"armor",    slot:"chest",    col:0xaad0e6, def:22, str:3, maxHp:20, price:480, desc:"Hoarfrost-tempered steel from Ironpeak." },
  frost_helm:      { name:"Glacier Helm",      icon:"helm", rar:"rare",      type:"armor",    slot:"helmet",   col:0xaad0e6, crest:0x66e0ff, def:12, str:2, maxHp:10, price:300, desc:"Carved to shed snow and blows alike." },
  titan_ring:      { name:"Titan's Signet",    icon:"gem", rar:"epic",      type:"armor",    slot:"ring",     str:5, def:3, maxHp:25, price:640, desc:"Pried from a Frost Titan's frozen hand." },
  frost_shard:     { name:"Frost Shard",       icon:"frostshard", rar:"uncommon",  type:"material",                                          price:30,  desc:"A sliver of everlasting ice." },
  titan_core:      { name:"Titan Frost-Core",  icon:"ice", rar:"epic",      type:"material",                                          price:240, desc:"The frozen heart of a Frost Titan." },
  // ── M9: Hollow King drops ──
  hollow_crown:    { name:"Crown of the Void",     icon:"crown", rar:"legendary", type:"armor",  slot:"helmet", col:0x2a1830, crest:0xcc44ff, def:30, int:12, maxMana:70, price:2400, desc:"The Hollow King's crown — shadow-forged and void-cold." },
  shadow_cloak:    { name:"Shadowweave Cloak",     icon:"robe",  rar:"legendary", type:"armor",  slot:"chest",  col:0x18102a, def:32, int:8, dex:6, maxHp:50, price:2600, desc:"Spun from living shadow. Darkness that protects." },
  void_blade:      { name:"Voidreaver",            icon:"sword", rar:"legendary", type:"weapon", slot:"weapon", dmg:72, str:8, int:8, maxHp:30, price:2800, desc:"A blade that drinks light and returns death." },
};
const SLOTS = ["weapon","helmet","chest","gloves","boots","ring1","ring2","necklace"];
const SLOT_ICON = { weapon:"sword", helmet:"helm", chest:"plate", gloves:"glove", boots:"boot", ring1:"ring", ring2:"ring", necklace:"neck" };

// XP curve: max level 25, realistic pacing (~10-15h game)
// xpForLevel[n] = XP needed to go from level n to n+1
const XP_TABLE = [0,60,130,220,340,500,700,950,1260,1640,2100,2650,3300,4050,4920,5920,7060,8360,9840,11520,13420,15570,18000,20740,24000];
const MAX_LEVEL = 25;

// Icon visual config kept for reference only (replaced by pixel-art drawItemIcon below)
const ICON_DEF = {};
const _iconCache={};
function drawItemIconLegacy(key, size=40){ return drawItemIcon(key,"common",size); }
// Portrait icons for dialogue
const PORTRAIT_DEF = {
  spirit:  ["#0e2236","#70c8ff","✦"],   elder:   ["#2e1a0a","#e0a060","EL"],
  captain: ["#0e1e30","#7090d0","CPT"],  guard:   ["#0e1e10","#80b070","GRD"],
  sage:    ["#0e1e28","#80c8c0","SGE"],  hunter:  ["#1e2a0e","#b0c870","HNT"],
  npc:     ["#221a12","#c09070","NPC"],  tablet:  ["#1e1808","#c0a860","TAB"],
  citizen: ["#221a12","#c09070","CTZ"],
};
function drawPortraitIcon(key, size=44){
  const cv=document.createElement("canvas"); cv.width=size; cv.height=size;
  const c=cv.getContext("2d");
  const [bg,fg,sym]=PORTRAIT_DEF[key]||["#202030","#9090c0",key.slice(0,1)];
  c.fillStyle=bg; c.beginPath(); c.arc(size/2,size/2,size/2-1,0,Math.PI*2); c.fill();
  c.strokeStyle=fg+"88"; c.lineWidth=2; c.stroke();
  const isShort=sym.length<=2;
  c.fillStyle=fg; c.font=`bold ${isShort?Math.round(size*.5):Math.round(size*.28)}px Georgia,serif`;
  c.textAlign="center"; c.textBaseline="middle";
  c.fillText(sym,size/2,size/2+1);
  return cv.toDataURL();
}
...
function defaultState(cls, charName="Warden", hairCol=0, skinTone=0) {
  ...
  return {
    ...
    level:1, xp:0, xpNext:XP_TABLE[1]||60,
    ...
    pos: { x:0, z:8 },
    quest: { greeted:false, kills:0, need:5, completed:false, hasEarthRelic:false, hasFrostRelic:false, hasFlameRelic:false, hasStormRelic:false, hollowKingDefeated:false },
    story: { stage:0 },
    sq: { shepherd:{s:0,p:0}, lumber:{s:0,p:0}, wolves:{s:0,p:0}, herbs:{s:0,p:0}, child:{s:0,p:0} },
    dungeonMapUnlocked:false, stonewatch:false, skillCd:0,
    skillPoints:0, skillLevels:{primary:1,secondary:0},
  };
}
...
export default function KingdomOfVerandah() {
  ...
  const toast = useCallback(msg => {
    ...
    const pump = () => {
      if (!toastQueue.current.length) { toastActive.current=false; return; }
      toastActive.current = true;
      const msg2 = toastQueue.current.shift();
      const id = Date.now()+Math.random();
      setToasts(t => t.length>=3 ? [...t.slice(-2),{id,msg:msg2}] : [...t,{id,msg:msg2}]);
      setTimeout(()=>{ setToasts(t=>t.filter(x=>x.id!==id)); setTimeout(pump,80); }, 2200);
    };
    pump();
  }, []);
  ...
  useEffect(() => {
    ...
    g.hollowGroup    = new THREE.Group(); scene.add(g.hollowGroup);    g.hollowGroup.visible   = false;

    buildTerrain(g); buildVillage(g); scatterWorld(g); buildNPCs(g); buildDungeonEntrance(g); buildDungeon(g); buildOverworldExtras(g);
    buildRedOak(g); buildVerandah(g); buildIronpeak(g); buildFrostCavern(g); buildEmber(g); buildScorchdeep(g); buildStorm(g); buildStormspire(g); buildHollow(g);
    spawnEnemyWave(g); spawnZoneEnemies(g); makePlayer(g);
    ...
    MUSIC.play("overworld", 2.5); // start music on game entry
    ...
    g.interact = () => { SFX.resume(); MUSIC.resume(); try { doInteract(g, toast, setQuestTrack, setZone); } catch(err){ console.error("interact error",err); } };
    ...
    const loop = now => {
      ...
      // Campfire flicker
      if(g.campfireLight){ const f=2.2+Math.sin(Date.now()*.011)*0.5+Math.random()*.4; g.campfireLight.intensity=f; }
      // Dungeon torch pulse
      if(g.dunTorchLights&&g.zone==="dungeon"){ const t=Date.now()*.008; g.dunTorchLights.forEach((pl,i)=>{ pl.intensity=3.2+Math.sin(t+i*1.7)*.7+Math.random()*.3; }); }
      updateSky(g);
      ...
      if (!g.paused && g.zone==="hollow")      updateHollow(g,dt,state,toast,setQuestTrack);
      if (!g.paused && g.zone==="frost")       updateBossFrost(g,dt,state,toast);
      if (!g.paused && g.zone==="scorch")      updateBossScorch(g,dt,state,toast);
      if (!g.paused && g.zone==="stormspire")  updateBossStorm(g,dt,state,toast);
  ...
  useEffect(() => {
    ...
    if(k==="KeyQ") useClassSkill(g,g.saveState,toast);
  ...
  const beginGame = useCallback(fromSave => {
    ...
    if (st.quest && st.quest.hollowKingDefeated==null) st.quest.hollowKingDefeated=false;
    if (st.skillPoints==null) st.skillPoints=0;
    if (!st.skillLevels) st.skillLevels={primary:1,secondary:0};
  ...
  if (screen==="title") return (
    <div style={{...F,background:"radial-gradient(ellipse at 50% 18%,rgba(201,150,46,.22),transparent 60%),linear-gradient(180deg,#0d1422,#1a2438 40%,#243a2e)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"serif"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,width:280}}>
        <Btn gold onClick={()=>{MUSIC.play("menu",2);setScreen("create");}}>New Game</Btn>
  ...
  if (screen==="create") {
    ...
    const CLASS_INFO={
      Warrior:{icon:"sword",desc:"STR 14 · High HP · Melee tank",color:"#e07a3a"},
      Ranger: {icon:"bow",desc:"DEX 14 · High Stamina · Swift",color:"#5a9a3a"},
      Mage:   {icon:"staff",desc:"INT 16 · High Mana · Arcane power",color:"#8844cc"},
    };
          ...
          <div style={{marginBottom:14}}>
            ...
            {Object.entries(CLASS_INFO).map(([c,info])=>(
              <div key={c} onPointerDown={()=>setCls(c)}
                ...
                <img src={drawItemIcon(info.icon, info.rar||"common")} width={30} height={30} style={{imageRendering:"pixelated",borderRadius:5}} alt=""/>
                <div>
      ...
      <Panel style={{width:"min(94vw,440px)",padding:"22px 20px"}}>
        ...
        {Object.keys(BASE).map(c=>(
          <div key={c} onPointerDown={()=>setCls(c)} style={{cursor:"pointer",border:`2px solid ${cls===c?"#f0c659":"#5b4326"}`,borderRadius:8,padding:"10px 14px",marginBottom:8,background:cls===c?"rgba(201,150,46,.15)":"rgba(20,16,8,.4)"}}>
            <div style={{fontFamily:"Georgia",fontWeight:700,color:"#f0c659",fontSize:16,marginBottom:2}}>{c}</div>
  ...
  return (
    <div style={{...F,overflow:"hidden"}}>
      ...
      <div style={{...F,pointerEvents:"none",zIndex:20}}>
        ...
        {/* Skill label above skill button */}
        {G.current.saveState && (() => {
          const cls=G.current.saveState.cls; const cd=SKILL_CD[cls]||12;
          const elapsed=G.current.skillLastUsed?Math.min(cd,(performance.now()/1000-G.current.skillLastUsed)):cd;
          const ready=elapsed>=cd; const pct=elapsed/cd;
          const label=cls==="Warrior"?"Whirlwind":cls==="Ranger"?"Volley":"Surge";
        {/* Quick-use potion slot — bottom-left above joystick */}
        {G.current.saveState && (()=>{
          const st=G.current.saveState; const qk=st.quickSlot||null;
          const it=qk?ITEMS[qk]:null; const qty=qk?(st.inv[qk]||0):0;
          const use=()=>{
            if(!qk||qty<=0){toast("No consumable equipped — long-press potion in inventory");return;}
            const i=ITEMS[qk]; if(!i) return;
            const cs=calcStats(st);
            if(i.heal&&st.hp>=cs.maxHp){toast("Already full HP");return;}
            if(i.mana&&st.mana>=cs.maxMana){toast("Mana full");return;}
            if(i.heal){st.hp=Math.min(cs.maxHp,st.hp+i.heal);toast(`+${i.heal} HP`);}
            if(i.mana){st.mana=Math.min(cs.maxMana,st.mana+i.mana);toast(`+${i.mana} Mana`);}
            st.inv[qk]=(st.inv[qk]||1)-1; if(st.inv[qk]<=0)delete st.inv[qk];
            SFX.pickup(); updateHUD(G.current,setUi);
          };
          return <div onPointerDown={e=>{e.stopPropagation();e.preventDefault();use();}} style={{position:"absolute",left:8,bottom:210,width:54,height:54,borderRadius:10,background:"rgba(18,10,4,.88)",border:`2.5px solid ${qty>0?"#c9962e":"#5b4326"}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",pointerEvents:"auto",cursor:"pointer",zIndex:46,touchAction:"none"}}>
            {it&&qty>0?<><img src={drawItemIcon(it.icon,34)} style={{width:28,height:28,borderRadius:4}} alt=""/><div style={{fontSize:9,color:"#f0c659"}}>{qty}x</div></>:<div style={{fontSize:18,color:"#5b4326",lineHeight:1}}>⬡</div>}
            <div style={{fontSize:7,color:"#8a7040",letterSpacing:.5,marginTop:1}}>USE</div>
          </div>;
        })()}
          return <div style={{position:"absolute",right:108,bottom:238,textAlign:"center",pointerEvents:"none"}}>
            <div style={{fontSize:10,color:ready?"#cc88ff":"#886699",fontFamily:"Georgia",letterSpacing:1,whiteSpace:"nowrap"}}>{ready?`${label} ●`:`${label} ${Math.ceil(cd-elapsed)}s`}</div>
            <div style={{width:62,height:3,background:"rgba(100,40,120,.4)",borderRadius:2,marginTop:2}}>
              <div style={{width:`${pct*100}%`,height:"100%",background:ready?"#bb66ff":"#664499",borderRadius:2,transition:"width .2s"}}/>
            </div>
          </div>;
        })()}
        ...
        {zone==="hollow"  && <div style={{position:"absolute",top:12,left:"50%",transform:"translateX(-50%)",background:"rgba(6,2,10,.92)",border:"2px solid #9944ff",borderRadius:20,padding:"5px 16px",color:"#cc88ff",fontFamily:"Georgia",fontSize:12,letterSpacing:2}}>◈ HEARTSTONE CHAMBER</div>}
        {boss && (
        ...
        <div style={{position:"absolute",right:8,bottom:210,display:"flex",flexDirection:"column-reverse",gap:4,alignItems:"flex-end",maxWidth:"72vw",pointerEvents:"none",zIndex:50}}>
          {toasts.map(t=><div key={t.id} style={{background:"rgba(16,10,4,.92)",border:"1px solid #c9962e",borderLeft:"3px solid #f0c659",borderRadius:5,padding:"5px 12px",fontSize:12,color:"#efe2c2",fontFamily:"Georgia",animation:"tIn .25s",whiteSpace:"nowrap",textOverflow:"ellipsis",overflow:"hidden",maxWidth:"100%"}}>{t.msg}</div>)}
        </div>
      </div>
      ...
      {dlg && (
        <div style={{position:"absolute",left:"50%",bottom:16,transform:"translateX(-50%)",width:"min(94vw,700px)",zIndex:40,pointerEvents:"auto",touchAction:"none"}} onPointerDown={e=>{e.preventDefault();dlgAdvance();}}>
          <Panel style={{display:"flex",gap:14,padding:"14px 16px 12px"}}>
            <div style={{flexShrink:0,width:80,height:80,borderRadius:8,border:"2px solid #5b4326",boxShadow:"0 0 0 2px #c9962e inset",background:"#1a1208",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
              <img src={drawPortraitIcon(dlg.portrait||"✦",72)} style={{width:72,height:72,borderRadius:6}} alt=""/>
            </div>
      ...
      {overlay && (
        <div style={{...F,zIndex:60,background:"rgba(6,5,3,.65)",backdropFilter:"blur(2px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>{if(e.target===e.currentTarget)closeOverlay();}}>
          <Panel style={{width:"min(94vw,600px)",maxHeight:"88vh",overflowY:"auto",padding:"20px 20px 16px"}}>
            ...
            {overlay==="skills" && <SkillsOvl  state={G.current.saveState} G={G} toast={toast} closeOverlay={closeOverlay}/>}
            {overlay==="char"  && <CharOvl    state={G.current.saveState}/>}
            {overlay==="pause"  && <PauseOvl   G={G} toast={toast} setHasSave={setHasSave} close={closeOverlay} goTitle={()=>{saveGame(G.current.saveState,G.current);setScreen("title");}} openChar={()=>setOverlay("char")} setOverlay={setOverlay}/>}
            {overlay==="codex"  && <CodexOvl   state={G.current.saveState}/>}
...
/* ── Touch controls ── */
function TouchButton({label,onFire,r,b,sz=80,bg="rgba(162,59,44,.72)",onLongPress=null}){
  const ref=useRef(null);
  const fnRef=useRef(onFire); fnRef.current=onFire;
  const lpRef=useRef(onLongPress); lpRef.current=onLongPress;
  useEffect(()=>{
    const el=ref.current; if(!el) return;
    let lpTimer=null;
    const down=e=>{
      e.preventDefault(); e.stopPropagation(); SFX.resume();
      if(lpRef.current){
        // Has long-press: start timer, fire normal on release
        lpTimer=setTimeout(()=>{ lpTimer=null; lpRef.current(); },500);
      } else {
        fnRef.current(); // No long-press: fire immediately
      }
    };
    const up=e=>{ if(lpTimer){ clearTimeout(lpTimer); lpTimer=null; fnRef.current(); } };
...
function TouchCtrl({G, toast, setQuestTrack, setZone, dlgAdvance}) {
  ...
  const tb=(label,fn,r,b,sz=80,bg="rgba(162,59,44,.72)",longPress=null)=>(
    <TouchButton label={label} onFire={fn} r={r} b={b} sz={sz} bg={bg} onLongPress={longPress}/>
  );
  const g=G.current;
  return (
    <div style={{...F,pointerEvents:"none",zIndex:30}}>
      ...
      {tb(G.current.saveState?.cls==="Mage"?"★":G.current.saveState?.cls==="Ranger"?"↟":"⟳",
          ()=>useClassSkill(G.current,G.current.saveState,toast,false),
          118,176,58,"rgba(80,20,100,.82)",
          ()=>useClassSkill(G.current,G.current.saveState,toast,true))}
    </div>
  );
}

/* ── Inventory overlay ── */
// Canvas-drawn pixel-art item icons — no emoji, works on all platforms
function drawItemIcon(icon,rar){
  const key=icon+"_"+rar;
  if(_iconCache[key]) return _iconCache[key];
  const cv=document.createElement("canvas"); cv.width=48; cv.height=48;
  const c=cv.getContext("2d");
  const bg={legendary:"#2a1a00",epic:"#1a0a2a",rare:"#001a2a",uncommon:"#0a1a0a",common:"#1a1a1a"}[rar]||"#111";
  c.fillStyle=bg; c.fillRect(0,0,48,48);
  c.textAlign="center"; c.textBaseline="middle";
  // Each icon key maps to a drawn symbol
  const DRAW={
    // WEAPONS
    sword:()=>{ c.fillStyle="#c8d4e0"; c.fillRect(22,6,4,28); c.fillStyle="#9a8a52"; c.fillRect(14,22,20,4); c.fillStyle="#6b4a1a"; c.fillRect(20,34,8,6); },
    dagger:()=>{ c.fillStyle="#c8d4e0"; c.fillRect(22,8,4,22); c.fillStyle="#9a8a52"; c.fillRect(16,24,16,4); c.fillStyle="#6b4a1a"; c.fillRect(21,28,6,8); },
    relic:()=>{ c.fillStyle="#88ddcc"; c.fillRect(18,8,12,10); c.fillStyle="#44aaaa"; c.fillRect(14,16,20,20); c.fillStyle="#22ffee"; c.globalAlpha=.7; c.fillRect(20,20,8,8); c.globalAlpha=1; },
    wood:()=>{ c.fillStyle="#8a6030"; c.fillRect(12,6,6,36); c.fillStyle="#6a4820"; c.fillRect(16,4,4,40); c.fillStyle="#4a8a3a"; for(let i=0;i<4;i++){c.fillRect(18+i*4,10,3,4);} },
    bow:()=>{ c.strokeStyle="#8a6030"; c.lineWidth=4; c.beginPath(); c.arc(24,24,14,-.7,Math.PI+.7); c.stroke(); c.strokeStyle="#e8dcb0"; c.lineWidth=2; c.beginPath(); c.moveTo(24,10);c.lineTo(24,38); c.stroke(); c.fillStyle="#c8a878"; c.fillRect(22,20,4,8); },
    staff:()=>{ c.fillStyle="#6b4a1a"; c.fillRect(21,6,6,30); c.fillStyle="#8844ff"; c.beginPath(); c.arc(24,10,7,0,Math.PI*2); c.fill(); c.fillStyle="#cc88ff"; c.globalAlpha=.6; c.beginPath(); c.arc(24,10,4,0,Math.PI*2); c.fill(); c.globalAlpha=1; },
    knife:()=>{ c.fillStyle="#c0c8d4"; c.beginPath(); c.moveTo(24,6);c.lineTo(30,30);c.lineTo(18,30); c.fill(); c.fillStyle="#6b4a1a"; c.fillRect(20,30,8,8); },
    // ARMOR
    helm:()=>{ c.fillStyle="#9aa0aa"; c.beginPath(); c.arc(24,26,14,Math.PI,0); c.fill(); c.fillStyle="#707880"; c.fillRect(10,28,28,8); c.fillStyle="#c8ccd4"; c.fillRect(10,28,28,3); },
    crown:()=>{ c.fillStyle="#c9a23a"; c.fillRect(8,24,32,10); c.fillStyle="#c9a23a"; [[8,24],[17,16],[26,16],[35,24]].forEach(([x,y])=>{c.fillRect(x,y,8,12);}); c.fillStyle="#44aaff"; c.beginPath(); c.arc(24,20,4,0,Math.PI*2); c.fill(); },
    vest:()=>{ c.fillStyle="#7a5a32"; c.fillRect(12,8,24,32); c.fillStyle="#6a4a22"; c.fillRect(14,8,8,32); c.fillRect(26,8,8,32); c.fillStyle="#8a6a42"; c.fillRect(12,8,24,6); },
    plate:()=>{ c.fillStyle="#8a909a"; c.fillRect(10,8,28,32); c.fillStyle="#70787e"; c.fillRect(10,14,28,4); c.fillRect(10,22,28,4); c.fillStyle="#aab0b8"; c.fillRect(10,8,28,4); },
    robe:()=>{ c.fillStyle="#3a6e9a"; c.fillRect(10,6,28,36); c.fillStyle="#2a5080"; c.fillRect(10,6,28,6); c.fillStyle="#aaddff"; c.globalAlpha=.5; c.fillRect(20,8,8,34); c.globalAlpha=1; },
    gaunt:()=>{ c.fillStyle="#808890"; c.fillRect(12,8,24,14); c.fillRect(10,22,8,18); c.fillRect(30,22,8,18); for(let i=0;i<5;i++) c.fillRect(10+i*5+2,20,4,6); },
    glove:()=>{ c.fillStyle="#8a6a3a"; c.fillRect(14,8,20,18); c.fillRect(12,26,6,12); c.fillRect(30,26,6,12); c.fillRect(18,24,5,10); c.fillRect(25,24,5,10); },
    boot:()=>{ c.fillStyle="#7a6a5a"; c.fillRect(14,6,16,28); c.fillRect(10,34,24,8); c.fillStyle="#5a4a3a"; c.fillRect(10,36,24,3); },
    ring:()=>{ c.strokeStyle="#c0a840"; c.lineWidth=6; c.beginPath(); c.arc(24,24,12,0,Math.PI*2); c.stroke(); c.fillStyle="#ff4444"; c.beginPath(); c.arc(24,12,5,0,Math.PI*2); c.fill(); },
    neck:()=>{ c.strokeStyle="#c0a840"; c.lineWidth=4; c.beginPath(); c.arc(24,24,14,-2.2,-.9); c.stroke(); c.beginPath(); c.arc(24,24,14,-.9+Math.PI,.9+Math.PI); c.stroke(); c.fillStyle="#44ddaa"; c.beginPath(); c.arc(24,32,6,0,Math.PI*2); c.fill(); },
    seal:()=>{ c.fillStyle="#c9a23a"; c.beginPath(); c.arc(24,24,16,0,Math.PI*2); c.fill(); c.fillStyle="#8a6820"; c.font="bold 20px serif"; c.fillText("✦",24,26); },
    // CONSUMABLES
    potion:()=>{ c.fillStyle="#88221a"; c.fillRect(18,22,12,18); c.fillStyle="#aa3328"; c.fillRect(16,24,16,14); c.fillStyle="#cc4a3a"; c.globalAlpha=.6; c.fillRect(20,26,8,8); c.globalAlpha=1; c.fillStyle="#8a7060"; c.fillRect(18,14,12,10); c.fillRect(20,10,8,6); },
    potion2:()=>{ c.fillStyle="#334488"; c.fillRect(18,22,12,18); c.fillStyle="#4455aa"; c.fillRect(16,24,16,14); c.fillStyle="#6677cc"; c.globalAlpha=.6; c.fillRect(20,26,8,8); c.globalAlpha=1; c.fillStyle="#8a7060"; c.fillRect(18,14,12,10); c.fillRect(20,10,8,6); },
    mana:()=>{ c.fillStyle="#113388"; c.fillRect(18,22,12,18); c.fillStyle="#2244bb"; c.fillRect(16,24,16,14); c.fillStyle="#44aaff"; c.globalAlpha=.7; c.fillRect(20,26,8,8); c.globalAlpha=1; c.fillStyle="#8a7060"; c.fillRect(18,14,12,10); c.fillRect(20,10,8,6); },
    bread:()=>{ c.fillStyle="#c8903a"; c.beginPath(); c.arc(24,28,14,Math.PI,0); c.fill(); c.fillStyle="#a87030"; c.fillRect(10,28,28,8); c.fillStyle="#e0a850"; c.beginPath(); c.arc(24,24,8,Math.PI,0); c.fill(); },
    herb:()=>{ c.fillStyle="#3a8a2a"; c.fillRect(22,8,4,32); c.beginPath(); c.ellipse(16,16,8,6,-.5,0,Math.PI*2); c.fill(); c.beginPath(); c.ellipse(32,20,8,6,.5,0,Math.PI*2); c.fill(); c.beginPath(); c.ellipse(14,26,7,5,-.3,0,Math.PI*2); c.fill(); },
    // MATERIALS
    shard:()=>{ c.fillStyle="#8844cc"; c.beginPath(); c.moveTo(24,6);c.lineTo(32,22);c.lineTo(28,38);c.lineTo(20,38);c.lineTo(16,22);c.closePath(); c.fill(); c.fillStyle="#cc88ff"; c.globalAlpha=.5; c.beginPath(); c.moveTo(24,10);c.lineTo(28,22);c.lineTo(24,32); c.fill(); c.globalAlpha=1; },
    pelt:()=>{ c.fillStyle="#8a7060"; c.beginPath(); c.arc(24,20,14,0,Math.PI*2); c.fill(); c.fillStyle="#6a5040"; c.fillRect(14,28,20,14); c.fillStyle="#aaa098"; for(let i=0;i<6;i++){ c.beginPath(); c.arc(14+i*4+2,16+Math.random()*6,2,0,Math.PI*2); c.fill(); } },
    badge:()=>{ c.fillStyle="#cc8833"; c.beginPath(); [0,1,2,3,4].forEach(i=>{const a=i/5*Math.PI*2-Math.PI/2;c.lineTo(24+Math.cos(a)*15,24+Math.sin(a)*15);}); c.closePath(); c.fill(); c.fillStyle="#aa6622"; c.font="bold 14px sans-serif"; c.fillText("!",24,26); },
    bone:()=>{ c.fillStyle="#d8ceb4"; c.fillRect(14,10,8,28); c.beginPath(); c.arc(18,10,6,0,Math.PI*2); c.fill(); c.beginPath(); c.arc(18,38,6,0,Math.PI*2); c.fill(); c.fillStyle="#c8be9a"; c.fillRect(14,10,8,28); },
    heartwood:()=>{ c.fillStyle="#5a3a18"; c.fillRect(18,4,12,38); c.fillStyle="#8a6a30"; c.beginPath(); c.arc(24,18,10,0,Math.PI*2); c.fill(); c.fillStyle="#4a8a3a"; c.globalAlpha=.7; c.beginPath(); c.arc(24,18,6,0,Math.PI*2); c.fill(); c.globalAlpha=1; },
    gem:()=>{ c.fillStyle="#88ddff"; c.beginPath(); c.moveTo(24,6);c.lineTo(40,20);c.lineTo(32,42);c.lineTo(16,42);c.lineTo(8,20);c.closePath(); c.fill(); c.fillStyle="#ffffff"; c.globalAlpha=.5; c.beginPath(); c.moveTo(24,8);c.lineTo(34,20);c.lineTo(24,16); c.fill(); c.globalAlpha=1; },
    // QUEST ITEMS
    relic_earth:()=>{ c.fillStyle="#4a8a3a"; c.beginPath(); c.arc(24,24,16,0,Math.PI*2); c.fill(); c.fillStyle="#2a6a1a"; c.beginPath(); c.arc(24,24,10,0,Math.PI*2); c.fill(); c.fillStyle="#88ff44"; c.globalAlpha=.7; c.beginPath(); c.arc(24,24,6,0,Math.PI*2); c.fill(); c.globalAlpha=1; },
    relic_frost:()=>{ c.fillStyle="#44aaee"; c.beginPath(); c.arc(24,24,16,0,Math.PI*2); c.fill(); c.fillStyle="#22aaee"; [0,1,2,3,4,5].forEach(i=>{const a=i/6*Math.PI*2; c.fillRect(24+Math.cos(a)*6-2,24+Math.sin(a)*6-2,4,4);}); c.fillStyle="#aaeeff"; c.globalAlpha=.7; c.beginPath(); c.arc(24,24,4,0,Math.PI*2); c.fill(); c.globalAlpha=1; },
    relic_flame:()=>{ c.fillStyle="#cc4422"; c.beginPath(); c.arc(24,24,16,0,Math.PI*2); c.fill(); c.fillStyle="#ff8833"; c.beginPath(); c.moveTo(24,8);c.lineTo(32,22);c.lineTo(36,18);c.lineTo(28,36);c.lineTo(20,36);c.lineTo(16,18);c.lineTo(20,22);c.closePath(); c.fill(); c.fillStyle="#ffee44"; c.globalAlpha=.7; c.beginPath(); c.arc(24,24,5,0,Math.PI*2); c.fill(); c.globalAlpha=1; },
    relic_storm:()=>{ c.fillStyle="#3a4a88"; c.beginPath(); c.arc(24,24,16,0,Math.PI*2); c.fill(); c.fillStyle="#aaccff"; c.beginPath(); c.moveTo(28,6);c.lineTo(20,24);c.lineTo(26,24);c.lineTo(18,42);c.lineTo(34,20);c.lineTo(28,20);c.closePath(); c.fill(); },
    frostshard:()=>{ c.fillStyle="#88ccff"; c.beginPath(); [0,1,2,3,4,5].forEach(i=>{const a=i/6*Math.PI*2;c.moveTo(24,24);c.lineTo(24+Math.cos(a)*16,24+Math.sin(a)*16);}); c.strokeStyle="#88ccff"; c.lineWidth=3; c.stroke(); c.beginPath(); c.arc(24,24,5,0,Math.PI*2); c.fill(); },
    ice:()=>{ c.fillStyle="#88eeff"; c.fillRect(10,10,28,28); c.fillStyle="#aaffff"; c.globalAlpha=.6; c.fillRect(10,10,28,14); c.globalAlpha=1; c.fillStyle="#338899"; c.fillRect(10,10,28,3); c.fillRect(10,36,28,3); c.fillRect(10,10,3,28); c.fillRect(35,10,3,28); },
  };
  const fn=DRAW[icon];
  if(fn){ fn(); }
  else {
    // Generic fallback — draw first 2 chars of icon name
    c.fillStyle="#888"; c.font="bold 18px Georgia,serif"; c.fillText((icon||"?").slice(0,2).toUpperCase(),24,27);
  }
  // Rarity border
  const rarBorder={legendary:"#f0c659",epic:"#aa44ff",rare:"#44aaff",uncommon:"#44cc44",common:"#888"}[rar]||"#555";
  c.strokeStyle=rarBorder; c.lineWidth=3; c.strokeRect(1.5,1.5,45,45);
...
function InvOverlay({state, G, toast, setUi}) {
  ...
  const useItem = k => {
    ...
    // Tap to assign to quick slot; double-tap info shows use option
    if(!state.quickSlot||state.quickSlot!==k){ state.quickSlot=k; toast(`Quick slot: ${it.name} — tap USE button or tap again to use`); SFX.pickup(); redraw(); return; }
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,marginBottom:12}}>
      {SLOTS.map(slot=>{
        ...
        return <div key={slot} onClick={()=>k&&unequip(slot)} style={{display:"flex",gap:6,alignItems:"center",border:`1px solid ${k?"#c9962e":"#5b4326"}`,borderRadius:6,padding:"5px 8px",background:k?"rgba(201,150,46,.1)":"rgba(0,0,0,.08)",cursor:k?"pointer":"default",fontSize:12}}>
          <img src={drawItemIcon(it?.icon||SLOT_ICON[slot], it?.rar||"common")} width={28} height={28} style={{imageRendering:"pixelated",borderRadius:4,display:"block"}} alt=""/>
    ...
    <div style={{borderTop:"1px solid rgba(91,67,38,.3)",paddingTop:10,marginBottom:8}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(56px,1fr))",gap:6}}>
        {Object.keys(state.inv).map(k=>{
          ...
          return <div key={k} onClick={()=>setSel(sel===k?null:k)} style={{border:`2px solid ${sel===k?"#f0c659":RAR_COL[it.rar]}`,borderRadius:8,padding:"4px 3px",textAlign:"center",cursor:"pointer",background:sel===k?"rgba(201,150,46,.18)":"rgba(0,0,0,.08)"}}>
            <img src={drawItemIcon(it.icon,40)} width={40} height={40} style={{borderRadius:4}}/>
    ...
    {si && (
      <div style={{border:`2px solid ${RAR_COL[si.rar]}`,borderRadius:8,padding:"10px 12px",background:"rgba(0,0,0,.06)",marginTop:4}}>
        <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:4}}>
          <img src={drawItemIcon(si.icon,si.rar)} width={48} height={48} style={{imageRendering:"pixelated",borderRadius:6}}/>
...
function QuestOvl({state}) {
  ...
  return <>
    ...
    {q.hollowKingDefeated && (
      <div style={{padding:"10px 12px",borderRadius:8,background:"linear-gradient(135deg,rgba(80,20,100,.4),rgba(40,10,60,.4))",border:"2px solid #9944ff",marginBottom:12,textAlign:"center"}}>
        <div style={{fontSize:14,color:"#dd88ff",fontFamily:"Georgia",fontWeight:700,marginBottom:2}}>🏆 KINGDOM OF VERANDAH — COMPLETE</div>
        <div style={{fontSize:11,color:"#aa66cc"}}>The Hollow King is defeated. The Heartstone shines. {state.charName||"Warden"} has saved the realm.</div>
      </div>
    )}
    ...
    {q.hasEarthRelic && (
      <div style={{padding:"8px 0",borderBottom:"1px dashed rgba(91,67,38,.3)",marginBottom:10}}>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:14,color:"#241a10",marginBottom:3}}><b>The Storm Relic</b><span style={{color:q.hasStormRelic?"#3f7d54":q.hasFlameRelic?"#a23b2c":"#7a5e2e"}}>{q.hasStormRelic?"DONE":q.hasFlameRelic?"ACTIVE":"LOCKED"}</span></div>
        <div style={{fontSize:12,color:"#7a5e2e"}}>{q.hasStormRelic?"Storm Relic recovered ✓":q.hasFlameRelic?"Travel to Tempest Reach; ascend the Stormspire and defeat the Stormcaller.":"Recover the Flame Relic first."}</div>
      </div>
    )}
    {q.hasStormRelic && (
      <div style={{padding:"8px 0",borderBottom:"1px dashed rgba(91,67,38,.3)",marginBottom:10}}>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:14,color:"#241a10",marginBottom:3}}><b>◈ The Hollow King</b><span style={{color:q.hollowKingDefeated?"#3f7d54":"#a23b2c"}}>{q.hollowKingDefeated?"DONE":"FINAL"}</span></div>
        <div style={{fontSize:12,color:"#7a5e2e"}}>{q.hollowKingDefeated?"The Hollow King is defeated. Verandah is saved! ✓":"Speak with Captain Aldric in Verandah. Enter the Heartstone Chamber portal in the castle courtyard."}</div>
      </div>
    )}
...
const CODEX_ENTRIES=[
  {id:"corrupted",name:"Corrupted Shade",zone:"Red Oak Grove",icon:"shade",rarity:"common",
    desc:"Once ordinary forest spirits, twisted by the Hollow King's corrupting shadow. Their void-tinged cores pulse with hatred for the living.",
    lore:"The corruption began thirty years ago, when the Heartstone first cracked. Spirits too weak to resist were hollowed out, their warmth replaced by the King's cold malice.",
    drops:"Minor Draught, Gold"},
  {id:"wolf",name:"Grove Wolf",zone:"Red Oak Grove",icon:"wolf",rarity:"common",
    desc:"Large grey wolves emboldened by the corruption driving away their natural prey. Aggressive and fast, but fragile.",
    lore:"Wolves of the Red Oak were once respected guardians of the forest's borders. The Hollow corruption made prey scarce, and hunger made them dangerous.",
    drops:"Wolf Pelt, Bone Fragment"},
  {id:"bandit",name:"Grove Bandit",zone:"Overworld",icon:"sword",rarity:"uncommon",
    desc:"Desperate outlaws who fled the capital when the Hollow King's influence weakened law and order. Armed, angry, and dangerous in numbers.",
    lore:"Most were merchants or farmers before the Shadow Year. Some joined willingly; others were conscripted by bandit captains who filled the power vacuum.",
    drops:"Iron Shortsword, Bandit's Cutlass, Gold"},
  {id:"skeleton",name:"Ancient Skeleton",zone:"Overworld",icon:"bone",rarity:"uncommon",
    desc:"Long-dead Warden soldiers reanimated by void-energy seeping up from the earth. They retain their martial muscle memory, making them disciplined fighters.",
    lore:"These were the First Wardens — soldiers who fell defending the Heartstone in the Hollow King's original campaign three centuries ago.",
    drops:"Bone Fragment, Iron Helm, Gold"},
  {id:"treant",name:"Thornheart Treant",zone:"Ancient Root Temple",icon:"wood",rarity:"boss",
    desc:"The corrupted ancient spirit of the Red Oak grove's oldest tree. Massive and relentless, it guards the Earth Relic deep in the temple's roots.",
    lore:"Thornheart lived for eight centuries as the grove's guardian before the Hollow King's roots reached it. Its corruption was slow — the tree fought for years before falling. A tragedy the Guardian Spirits do not speak of lightly.",
    drops:"Earth Relic, Heartwood Edge (Legendary), XP"},
  {id:"rootling",name:"Rootling",zone:"Ancient Root Temple",icon:"wood",rarity:"uncommon",
    desc:"Living root tendrils summoned by Thornheart during battle. Individually weak but they create constant pressure.",
    lore:"Fragments of Thornheart's own body, torn off and animated. They feel nothing — they are the tree's immune system.",
    drops:"Minor XP"},
  {id:"frostwolf",name:"Frostwolf",zone:"Ironpeak Mountains",icon:"wolf",rarity:"uncommon",
    desc:"Wolves adapted to the Ironpeak's lethal cold. Their breath frosts the air and their bites carry a numbing chill.",
    lore:"Frostwolves were considered lucky omens in Ironpeak villages — their fur was offered to travellers. That was before the hunger drove them to hunt people.",
    drops:"Frost Pelt, Wolf Fang"},
  {id:"titan",name:"Frost Titan",zone:"Frostfang Cavern",icon:"frost",rarity:"boss",
    desc:"An ancient giant of ice and stone, the Frost Titan has slept in the deepest cavern for centuries. The Hollow King's power awakened it as a guardian of the Frost Relic.",
    lore:"The Titan predates the Kingdom of Verandah itself. Ancient records call it 'the Mountain's Heartbeat' — a being of pure elemental force. It does not think; it endures.",
    drops:"Frost Relic, Rime Plate, XP"},
  {id:"emberwolf",name:"Emberwolf",zone:"Emberveil Wastes",icon:"fire",rarity:"uncommon",
    desc:"Wolves that evolved in the volcanic wastelands, their fur singed away, their hide hardened by ash. They run across lava flows without harm.",
    lore:"Emberwolves are the Emberveil's apex predators. Merchants who cross the wastes pay handsomely for their ash-resistant hides — if anyone survives to bring them back.",
    drops:"Ember Pelt, Ash Fragment"},
  {id:"colossus",name:"Flame Colossus",zone:"Scorchdeep",icon:"fire",rarity:"boss",
    desc:"A living embodiment of the volcano's fury — stone and magma fused into a titanic warrior. Scorchdeep has been its prison for three hundred years.",
    lore:"The Flame Colossus was created by a mage-king who wanted to harness volcanic power. It destroyed its creator and has dwelled in Scorchdeep ever since, content in its forge-heat.",
    drops:"Flame Relic, Obsidian Plate, XP"},
  {id:"stormwisp",name:"Stormwisp",zone:"Tempest Reach",icon:"bolt",rarity:"uncommon",
    desc:"Crackling balls of living lightning. They drift on storm winds and discharge into anything that comes close. They are not malicious — just electrically unstable.",
    lore:"Stormwisps form in the eye of hurricanes. The Stormcaller learned to command them as an extension of its own storm-body.",
    drops:"Storm Shard, Gold"},
  {id:"stormcaller",name:"Stormcaller",zone:"Stormspire",icon:"storm",rarity:"boss",
    desc:"An ancient elemental lord of storm and sky. It has ruled the Tempest Reach for eons, indifferent to mortals — until the Hollow King's shadow threatened even the skies.",
    lore:"The Stormcaller is neither evil nor good. It guards the Storm Relic because it was there when the Relics were first placed — a witness to the original binding. It fights you to test your worth.",
    drops:"Storm Relic, Tempest Glaive / Stormcaller Staff, XP"},
  {id:"hollowking",name:"The Hollow King",zone:"Heartstone Chamber",icon:"crown",rarity:"boss",
    desc:"The ancient enemy. Once a mortal Warden-King who sought to cheat death by binding his soul to the Heartstone itself, corrupting it. His physical form is void given shape.",
    lore:"His true name is lost. He was the Seventh King of Verandah — brilliant, beloved, and terrified of dying. His ritual worked. He did not die. But what remained was no longer human.",
    drops:"Crown of the Void, Shadowweave Cloak, Voidreaver, XP"},
];

function CodexOvl({state}){
  const [sel,setSel]=useState(null);
  const killed=state.codex||{};
  const entry=sel?CODEX_ENTRIES.find(e=>e.id===sel):null;
  const rarCol={common:"#9a9a9a",uncommon:"#5ab05a",boss:"#f0c659"};
  if(entry) return <>
    <div onPointerDown={()=>setSel(null)} style={{cursor:"pointer",marginBottom:10,color:"#8a7040",fontSize:12}}>← Back to Codex</div>
    <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}>
      <img src={drawItemIcon(entry.icon,44)} style={{width:40,height:40,borderRadius:6}} alt=""/>
      <div><div style={{fontFamily:"Georgia",fontWeight:700,fontSize:16,color:"#f0c659"}}>{entry.name}</div>
        <div style={{fontSize:11,color:rarCol[entry.rarity]||"#9a9a9a"}}>{entry.rarity.toUpperCase()} · {entry.zone}</div></div>
    </div>
    <div style={{fontSize:12,color:"#cdb98a",marginBottom:8,lineHeight:1.6}}>{entry.desc}</div>
    <div style={{fontSize:11,color:"#8a7050",fontStyle:"italic",marginBottom:8,lineHeight:1.5,borderLeft:"2px solid #5b4326",paddingLeft:8}}>{entry.lore}</div>
    <div style={{fontSize:11,color:"#a0d080"}}>Drops: {entry.drops}</div>
  </>;
  return <>
    <h2 style={TS}>Codex — Bestiary</h2>
    <p style={{fontSize:11,color:"#7a6030",marginBottom:10}}>Tap an entry to read. Enemies are revealed when you encounter them.</p>
    {CODEX_ENTRIES.map(e=>{
      const seen=killed[e.id]||0;
      return <div key={e.id} onPointerDown={()=>{ if(seen>0||e.rarity==="boss") setSel(e.id); }} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 10px",marginBottom:5,borderRadius:7,background:seen>0?"rgba(40,28,12,.6)":"rgba(14,10,4,.5)",border:`1px solid ${seen>0?"#5b4326":"#2a1e10"}`,cursor:seen>0?"pointer":"default",opacity:seen>0?1:.5}}>
        <img src={drawItemIcon(e.icon,32)} style={{width:28,height:28,borderRadius:4,filter:seen>0?"none":"grayscale(1) brightness(.4)"}} alt=""/>
        <div style={{flex:1}}>
          <div style={{color:seen>0?"#f0c659":"#6a5030",fontSize:13,fontFamily:"Georgia"}}>{seen>0?e.name:"???"}</div>
          <div style={{fontSize:10,color:rarCol[e.rarity]||"#9a9a9a"}}>{e.rarity.toUpperCase()}</div>
        </div>
        {seen>0&&<div style={{fontSize:10,color:"#8a7040"}}>{seen} killed</div>}
      </div>;
    })}
  </>;
}
...
const SKILL_INFO={
  Warrior:{
    primary:{name:"Whirlwind",icon:"⚔",cdLabel:"12s CD",desc:"360° spin — hits every nearby enemy.",levels:["Reach 3.4, 1.8× dmg","Reach 4.0, 2.3× dmg","Reach 4.6, 2.7× dmg"],cost:[0,1,2]},
    secondary:{name:"Iron Resolve",icon:"🛡",cdLabel:"18s CD",desc:"Heal HP + brief defense boost.",levels:["Locked (Lv 10)","Heal 12%, 4.5s","Heal 18%, 6s"],unlockLv:10,cost:[0,1,2]},
  },
  Ranger:{
    primary:{name:"Volley",icon:"🏹",cdLabel:"10s CD · 20 Mana",desc:"Fan of arrows in a wide spread.",levels:["5 arrows, 0.82× each","7 arrows, 1.1× each","9 arrows, 1.3× piercing"],cost:[0,1,2]},
    secondary:{name:"Eagle Eye",icon:"🎯",cdLabel:"18s CD · 30 Mana",desc:"Next N ranged shots auto-crit.",levels:["Locked (Lv 10)","Next 3 = crit","Next 4 = crit"],unlockLv:10,cost:[0,1,2]},
  },
  Mage:{
    primary:{name:"Arcane Surge",icon:"✦",cdLabel:"14s CD · 40 Mana",desc:"AoE magic burst around you.",levels:["Radius 5.5, 2.2× dmg","Radius 7.0, 2.7× dmg","Radius 8.5, 3.2× dmg"],cost:[0,1,2]},
    secondary:{name:"Mana Shield",icon:"🔮",cdLabel:"18s CD · 30 Mana",desc:"Mana absorbs % of incoming damage.",levels:["Locked (Lv 10)","27% for 8s","39% for 10s"],unlockLv:10,cost:[0,1,2]},
  },
};
function SkillsOvl({state,G,toast}){
  const [tick,setTick]=useState(0);
  const sl=state.skillLevels||{primary:1,secondary:0};
  const sp=state.skillPoints||0;
  const cls=state.cls||"Warrior";
  const info=SKILL_INFO[cls]||SKILL_INFO.Warrior;
  const upgrade=(which)=>{
    const cur=sl[which]||0; const inf=info[which];
    if(cur>=3){toast("Max level reached");return;}
    if(which==="secondary"&&state.level<(inf.unlockLv||10)){toast(`Unlocks at Level ${inf.unlockLv||10}`);return;}
    const cost=inf.cost[Math.min(cur,2)]||1;
    if(sp<cost){toast(`Need ${cost} Skill Point${cost>1?"s":""}. Earn at levels 5,10,15,20,25`);return;}
    state.skillPoints=(state.skillPoints||0)-cost;
    state.skillLevels[which]=cur+1;
    toast(`${inf.name} upgraded to Lv ${cur+1}!`); SFX.levelup(); setTick(n=>n+1);
  };
  const Stars=({lv})=>(<div style={{display:"flex",gap:2}}>{[0,1,2].map(i=><span key={i} style={{color:i<lv?"#f0c659":"#5b4326",fontSize:15}}>★</span>)}</div>);
  const Card=({which})=>{
    const inf=info[which]; const lv=sl[which]||0;
    const locked=which==="secondary"&&state.level<(inf.unlockLv||10);
    const nextCost=lv<3?inf.cost[Math.min(lv,2)]||1:0;
    return <div style={{border:`2px solid ${lv>0?"#c9962e":"#5b4326"}`,borderRadius:8,padding:"10px 12px",marginBottom:10,background:lv>0?"rgba(201,150,46,.08)":"rgba(0,0,0,.18)"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
        <span style={{fontSize:20}}>{inf.icon}</span>
        <div style={{flex:1}}><div style={{color:"#f0c659",fontFamily:"Georgia",fontWeight:700,fontSize:14}}>{inf.name}</div><div style={{fontSize:10,color:"#8a7040"}}>{inf.cdLabel}</div></div>
        {lv>0&&<Stars lv={lv}/>}
      </div>
      <div style={{fontSize:12,color:"#cdb98a",marginBottom:5}}>{inf.desc}</div>
      <div style={{fontSize:11,color:lv>0?"#a0d080":"#c0d080",background:"rgba(0,0,0,.28)",borderRadius:4,padding:"3px 7px",marginBottom:6}}>
        {locked?inf.levels[0]:lv>0?inf.levels[lv-1]:"Not yet unlocked"}
        {lv>0&&lv<3&&<span style={{color:"#88aaff"}}> → {inf.levels[lv]}</span>}
      </div>
      {!locked&&lv<3&&<div onPointerDown={()=>upgrade(which)} style={{cursor:"pointer",background:sp>=nextCost?"rgba(201,150,46,.28)":"rgba(50,30,10,.4)",border:"1px solid #c9962e",borderRadius:5,padding:"4px",fontSize:12,color:sp>=nextCost?"#f0c659":"#8a7040",textAlign:"center"}}>{lv===0?`Unlock — ${nextCost} SP`:`Upgrade to Lv ${lv+1} — ${nextCost} SP`}</div>}
      {locked&&<div style={{fontSize:11,color:"#7a6030",textAlign:"center"}}>🔒 Reach Level {inf.unlockLv||10}</div>}
      {lv>=3&&<div style={{fontSize:11,color:"#f0c659",textAlign:"center"}}>✓ MASTERED</div>}
    </div>;
  };
  return <>
    <h2 style={TS}>Skills — {cls}</h2>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:12,padding:"6px 10px",background:"rgba(201,150,46,.08)",borderRadius:6,border:"1px solid #5b4326"}}>
      <span style={{color:"#cdb98a",fontSize:13}}>Skill Points available</span>
      <span style={{color:"#f0c659",fontFamily:"Georgia",fontWeight:700,fontSize:18}}>{sp}</span>
    </div>
    <div style={{fontSize:10,color:"#7a6030",marginBottom:6,letterSpacing:2,textTransform:"uppercase"}}>Primary — Skill button (⟳/↟/★)</div>
    <Card which="primary"/>
    <div style={{fontSize:10,color:"#7a6030",marginBottom:6,letterSpacing:2,textTransform:"uppercase",marginTop:4}}>Secondary — long-press Skill button</div>
    <Card which="secondary"/>
    <p style={{fontSize:11,color:"#5a4020",marginTop:6,lineHeight:1.5}}>Skill points earned at levels 5·10·15·20·25. Tap the purple Skill button to use Primary; long-press for Secondary.</p>
  </>;
}
...
function PauseOvl({G, toast, setHasSave, close, goTitle, openChar, setOverlay}) {
  const state=G.current.saveState;
  return <>
    <h2 style={TS}>Paused</h2>
    <p style={{textAlign:"center",color:"#7a5e2e",fontSize:11,letterSpacing:3,textTransform:"uppercase",marginBottom:12}}>Kingdom of Verandah</p>
    <Btn gold onClick={close}>Resume</Btn>
    <Btn onClick={openChar}>Character Sheet</Btn>
    <Btn onClick={()=>setOverlay("skills")}>Skills & Abilities</Btn>
    <Btn onClick={()=>setOverlay("codex")}>Codex — Bestiary</Btn>
    ...
    <div style={{marginTop:10,fontSize:11,color:"#6b5630",lineHeight:1.8}}>
      WASD / joystick: Move &nbsp;•&nbsp; Drag: Look &nbsp;•&nbsp; ⚔: Attack &nbsp;•&nbsp; 💥: Heavy<br/>
      ✦: Interact &nbsp;•&nbsp; ⟳: Skill (long-press = Secondary) &nbsp;•&nbsp; ⬡: Quick Potion<br/>
      I: Inventory &nbsp;•&nbsp; J: Quests &nbsp;•&nbsp; Q: Class Skill &nbsp;•&nbsp; Esc: Menu
    </div>
  </>;
}
...
function updatePromptUI(g, setPrompt) {
  ...
  if(!best){
    ...
    else if(g.zone==="verandah" && g.hollowEntrance && p.distanceTo(g.hollowEntrance)<4){
      const q=g.saveState?.quest;
      best=q?.hasEarthRelic&&q?.hasFrostRelic&&q?.hasFlameRelic&&q?.hasStormRelic
        ? {label:q.hollowKingDefeated?"Enter the Heartstone Chamber":"◈ Enter the Heartstone Chamber"}
        : {label:"◈ Sealed — bring all Four Relics"};
    }
    else if(g.zone==="hollow" && g.hollowExit && p.distanceTo(g.hollowExit)<4) best={label:"Leave the Chamber"};
...
function buildVillage(g) {
  ...
  const mk=(geo,col)=>{ const m=new THREE.Mesh(geo,LM(col)); m.castShadow=true; return m; };
  const grp=g.overworldGroup;

  // ── Houses (2 ruined, 3 intact) ──
  const addH=(x,z,ruined)=>{
    const body=mk(new THREE.BoxGeometry(5,3,4),ruined?0x6e5a44:0xcdb592); body.position.set(x,tH(x,z)+(ruined?.9:1.5),z); body.receiveShadow=true; grp.add(body);
    if(!ruined){
      const r=mk(new THREE.ConeGeometry(4,2.2,4),0x8c3f2e); r.position.set(x,tH(x,z)+4.1,z); r.rotation.y=Math.PI/4; grp.add(r);
      // window shutters
      [-1,1].forEach(s=>{ const w=mk(new THREE.BoxGeometry(.7,.7,.08),0x7a5020); w.position.set(x+s*1.4,tH(x,z)+1.8,z+2.05); grp.add(w); });
      // door
      const d=mk(new THREE.BoxGeometry(.9,1.6,.08),0x5c3a18); d.position.set(x,tH(x,z)+.8,z+2.05); grp.add(d);
    }
  ...
  // ── Village well ──
  const well=mk(new THREE.CylinderGeometry(1.1,1.1,1.2,10),0x7d7d80); well.position.set(2,tH(2,0)+.6,0); well.receiveShadow=true; grp.add(well);
  const wellCap=mk(new THREE.ConeGeometry(1.4,.8,6),0x6a3a18); wellCap.position.set(2,tH(2,0)+2.0,0); grp.add(wellCap);
  const crossH=mk(new THREE.BoxGeometry(.18,.18,2.8),0x5a3a18); crossH.position.set(2,tH(2,0)+1.7,0); grp.add(crossH);
  g.props.push({pos:new THREE.Vector3(2,0,0),radius:1.3});

  // ── Campfire in the center clearing ──
  const cfx=4,cfz=4;
  for(let i=0;i<5;i++){ const a=i/5*Math.PI*2; const log=mk(new THREE.CylinderGeometry(.09,.13,.9,6),0x5c3a18); log.rotation.z=Math.PI/2.1; log.rotation.y=a; log.position.set(cfx+Math.cos(a)*.28,tH(cfx,cfz)+.06,cfz+Math.sin(a)*.28); grp.add(log); }
  const fire=new THREE.Mesh(new THREE.ConeGeometry(.38,.6,7),new THREE.MeshBasicMaterial({color:0xff6010,transparent:true,opacity:.85})); fire.position.set(cfx,tH(cfx,cfz)+.5,cfz); grp.add(fire);
  const firePL=new THREE.PointLight(0xff6010,2.8,12); firePL.position.set(cfx,tH(cfx,cfz)+1.2,cfz); grp.add(firePL); g.campfireLight=firePL;
  const ember=new THREE.Mesh(new THREE.CylinderGeometry(.3,.4,.18,8),new THREE.MeshBasicMaterial({color:0xff4400,transparent:true,opacity:.9})); ember.position.set(cfx,tH(cfx,cfz)+.1,cfz); grp.add(ember);

  // ── Dirt path from spawn toward dungeon entrance ──
  const pathMat=new THREE.MeshLambertMaterial({color:0x8a6a44,flatShading:true,transparent:true,opacity:.55});
  for(let i=0;i<18;i++){ const t=i/17; const px=t*32+Math.sin(i*1.4)*.8,pz=(t-.5)*4; const patch=new THREE.Mesh(new THREE.CircleGeometry(1.4+Math.random()*.5,8),pathMat); patch.rotation.x=-Math.PI/2; patch.position.set(px,tH(px,pz)+.04,pz); grp.add(patch); }

  // ── Fence posts around the north edge of the village ──
  for(let i=0;i<14;i++){ const fx=-8+i*2.4, fz=-18; const post=mk(new THREE.CylinderGeometry(.12,.15,.9,6),0x6a4a28); post.position.set(fx,tH(fx,fz)+.45,fz); grp.add(post); if(i<13){ const rail=mk(new THREE.BoxGeometry(2.4,.1,.08),0x7a5a32); rail.position.set(fx+1.2,tH(fx,fz)+.7,fz); grp.add(rail); } }

  // ── Notice board (near spawn) ──
  const nbx=-4,nbz=4;
  const nbPost=mk(new THREE.BoxGeometry(.2,.2,1.6),0x6a4a28); nbPost.rotation.x=Math.PI/2; nbPost.position.set(nbx,tH(nbx,nbz)+.8,nbz); grp.add(nbPost);
  const board=mk(new THREE.BoxGeometry(1.4,.06,1.0),0xc8a060); board.position.set(nbx,tH(nbx,nbz)+1.5,nbz-.1); board.rotation.x=.2; grp.add(board);
  const tack=mk(new THREE.BoxGeometry(.7,.04,.5),0xf0d8a0); tack.position.set(nbx,tH(nbx,nbz)+1.56,nbz-.1); tack.rotation.x=.2; grp.add(tack);

  // ── Market stall (near the well) ──
  const sx=-6,sz=2;
  [[sx-.9,sz],[sx+.9,sz],[sx-.9,sz+2.5],[sx+.9,sz+2.5]].forEach(([x,z])=>{ const p2=mk(new THREE.CylinderGeometry(.1,.12,2.2,6),0x6a4a28); p2.position.set(x,tH(x,z)+1.1,z); grp.add(p2); });
  const awning=mk(new THREE.BoxGeometry(2.4,.1,2.8),0xb04428); awning.position.set(sx,tH(sx,sz)+2.3,sz+1.25); awning.rotation.x=-.12; grp.add(awning);
  const counter=mk(new THREE.BoxGeometry(2.2,.18,1),0x9a7a44); counter.position.set(sx,tH(sx,sz)+1.0,sz+.5); grp.add(counter);

  // ── Scattered stones + wildflower patches ──
  [[-6,-6],[8,-2],[5,-10],[-2,-8],[10,-14],[-4,12]].forEach(([x,z])=>{ const st=mk(new THREE.DodecahedronGeometry(.4+Math.random()*.3,0),0x787878); st.position.set(x,tH(x,z)+.2,z); st.rotation.set(Math.random(),Math.random(),Math.random()); grp.add(st); });
  [[-8,4],[6,-8],[3,10],[-3,-4],[10,2],[7,12],[-5,14]].forEach(([x,z])=>{ const fl=mk(new THREE.SphereGeometry(.22,5,4),Math.random()>.5?0xee8888:0xeeee44); fl.position.set(x+Math.random()*.6,tH(x,z)+.3,z+Math.random()*.6); grp.add(fl); });

  // ── Stone archway at the village entrance (south path) ──
  const archX=0, archZ=-18;
  [[-.3,archZ,2.4,.6,5],[.3,archZ,2.4,.6,5]].forEach(([x,,z,w,h])=>{}); // skip, use below
  const pilL=mk(new THREE.BoxGeometry(.8,4.5,.8),0x888890); pilL.position.set(-3,tH(-3,archZ)+2.25,archZ); grp.add(pilL);
  const pilR=mk(new THREE.BoxGeometry(.8,4.5,.8),0x888890); pilR.position.set(3,tH(3,archZ)+2.25,archZ); grp.add(pilR);
  const archSpan=mk(new THREE.BoxGeometry(7.2,.8,1.0),0x888890); archSpan.position.set(0,tH(0,archZ)+4.8,archZ); grp.add(archSpan);
  const archTop=mk(new THREE.DodecahedronGeometry(.6,0),0x6a8060); archTop.position.set(0,tH(0,archZ)+5.7,archZ); grp.add(archTop);
  g.props.push({pos:new THREE.Vector3(-3,0,archZ),radius:.6});
  g.props.push({pos:new THREE.Vector3(3,0,archZ),radius:.6});

  // ── Small garden beds near houses ──
  const garden=(gx,gz)=>{
    const bed=mk(new THREE.BoxGeometry(2.8,.3,1.4),0x3a2a14); bed.position.set(gx,tH(gx,gz)+.15,gz); grp.add(bed);
    for(let i=0;i<6;i++){ const px=gx-1.2+i*.5, pz=gz+(.3-(i%2)*.5);
      const stk=mk(new THREE.CylinderGeometry(.04,.04,.4,4),0x4a7a3a); stk.position.set(px,tH(px,pz)+.4,pz); grp.add(stk);
      const flr=mk(new THREE.SphereGeometry(.14,4,3),[0xcc3333,0xdd9933,0x44aa44,0xdd3388,0x44aacc][i%5]); flr.position.set(px,tH(px,pz)+.68,pz); grp.add(flr); }
  };
  garden(-11,5); garden(11,3);

  // ── Abandoned cart on the path ──
  const cx2=8, cz2=-4;
  const carBody=mk(new THREE.BoxGeometry(2.8,.5,1.5),0x8a5a28); carBody.position.set(cx2,tH(cx2,cz2)+.7,cz2); grp.add(carBody);
  [[cx2-1,cz2],[cx2+1,cz2]].forEach(([wx,wz])=>{ const wh=new THREE.Mesh(new THREE.TorusGeometry(.6,.12,6,12),new THREE.MeshLambertMaterial({color:0x5a3a18,flatShading:true})); wh.rotation.y=Math.PI/2; wh.position.set(wx,tH(wx,wz)+.6,wz); grp.add(wh); });
  const shaft=mk(new THREE.CylinderGeometry(.08,.08,2.4,6),0x7a5028); shaft.rotation.z=Math.PI/2; shaft.position.set(cx2-2.1,tH(cx2,cz2)+.45,cz2); grp.add(shaft);

  // ── Scarecrow in the field ──
  const scx=-14, scz=10;
  const scPost=mk(new THREE.CylinderGeometry(.1,.12,2.8,6),0x7a5a2a); scPost.position.set(scx,tH(scx,scz)+1.4,scz); grp.add(scPost);
  const scArms=mk(new THREE.BoxGeometry(2.2,.14,.14),0x6a4a18); scArms.position.set(scx,tH(scx,scz)+2.1,scz); grp.add(scArms);
  const scBody=mk(new THREE.BoxGeometry(.6,1.0,.4),0xcc9944); scBody.position.set(scx,tH(scx,scz)+1.7,scz); grp.add(scBody);
  const scHead=mk(new THREE.BoxGeometry(.44,.44,.44),0xe8c870); scHead.position.set(scx,tH(scx,scz)+2.6,scz); grp.add(scHead);
  const scHat=mk(new THREE.CylinderGeometry(.06,.36,.2,8),0x5a3a18); scHat.position.set(scx,tH(scx,scz)+2.96,scz); grp.add(scHat);

  // ── Stone firepit ring around campfire ──
  for(let i=0;i<8;i++){ const a=i/8*Math.PI*2,r=.72; const s=mk(new THREE.BoxGeometry(.32,.22,.22),0x6a6860); s.position.set(cfx+Math.cos(a)*r,tH(cfx,cfz)+.12,cfz+Math.sin(a)*r); s.rotation.y=a; grp.add(s); }

  // ── Wooden benches around the fire ──
  [[cfx+2.2,cfz],[cfx-2.2,cfz],[cfx,cfz+2.2],[cfx,cfz-2.2]].forEach(([bx,bz],i)=>{
    const bench=mk(new THREE.BoxGeometry(i%2===0?1.6:.3,i%2===0?.3:1.6,.3),0x8a6030); bench.position.set(bx,tH(bx,bz)+.55,bz); grp.add(bench);
  });
}
...
function addNPC(g, zoneKey, opts) {
  ...
  const gr=new THREE.Group();
  const lm=c=>new THREE.MeshLambertMaterial({color:c,flatShading:true});
  const mk=(geo,c)=>{ const m=new THREE.Mesh(geo,lm(c)); m.castShadow=true; return m; };
  // Declared at function scope so all paths can reference them safely
  let legL=null, legR=null, armL=null, armR=null;

  if(spirit){
    // Spirit — glowing orb form
    const body=new THREE.Mesh(new THREE.SphereGeometry(.38,12,12),new THREE.MeshLambertMaterial({color:0xbfe3ff,transparent:true,opacity:.82,emissive:0x335588,emissiveIntensity:.7})); body.position.y=.9; gr.add(body);
    const glow=new THREE.Mesh(new THREE.SphereGeometry(.52,10,10),new THREE.MeshBasicMaterial({color:0x66bbff,transparent:true,opacity:.22})); glow.position.y=.9; gr.add(glow);
    const head=new THREE.Mesh(new THREE.SphereGeometry(.28,10,10),new THREE.MeshLambertMaterial({color:0xd8f0ff,transparent:true,opacity:.85,emissive:0x335588,emissiveIntensity:.8})); head.position.y=1.7; gr.add(head);
    // flowing wisps
    for(let i=0;i<4;i++){ const w=new THREE.Mesh(new THREE.SphereGeometry(.1,6,6),new THREE.MeshBasicMaterial({color:0x88ccff,transparent:true,opacity:.5})); w.position.set(Math.cos(i/4*Math.PI*2)*.4,.4+i*.08,Math.sin(i/4*Math.PI*2)*.4); gr.add(w); }
  } else {
    // Humanoid — skin tone from col, clothes from col
    const skinCol = kind==="guard" ? 0xe0b080 : [0xf4d4b0,0xe8b896,0xc8945a,0x8b5e3c][Math.floor(Math.random()*4)];
    const hairCol = [0x2c1e14,0x6b3c18,0xc8a23a,0x7a2e12][Math.floor(Math.random()*4)];

    // Legs (pivot at hip, animated)
    legL=new THREE.Group(); legL.position.set(-.14,.88,0);
    const llMesh=mk(new THREE.BoxGeometry(.26,.74,.26),col); llMesh.position.y=-.37; legL.add(llMesh);
    const llfMesh=mk(new THREE.BoxGeometry(.24,.18,.3),0x3a2a18); llfMesh.position.set(0,-.74,.04); legL.add(llfMesh);
    gr.add(legL);

    legR=new THREE.Group(); legR.position.set(.14,.88,0);
    const lrMesh=mk(new THREE.BoxGeometry(.26,.74,.26),col); lrMesh.position.y=-.37; legR.add(lrMesh);
    const lrfMesh=mk(new THREE.BoxGeometry(.24,.18,.3),0x3a2a18); lrfMesh.position.set(0,-.74,.04); legR.add(lrfMesh);
    gr.add(legR);

    // Torso
    const torso=mk(new THREE.BoxGeometry(.68,.88,.42),col); torso.position.set(0,1.28,0); gr.add(torso);
    // belt detail
    const belt=mk(new THREE.BoxGeometry(.7,.1,.44),0x2a1e0e); belt.position.set(0,.86,0); gr.add(belt);

    // Arms (pivot at shoulder, animated)
    armL=new THREE.Group(); armL.position.set(-.42,1.62,0);
    const alMesh=mk(new THREE.BoxGeometry(.24,.72,.26),col); alMesh.position.y=-.36; armL.add(alMesh);
    const alhand=mk(new THREE.BoxGeometry(.2,.2,.22),skinCol); alhand.position.y=-.74; armL.add(alhand);
    gr.add(armL);

    armR=new THREE.Group(); armR.position.set(.42,1.62,0);
    const arMesh=mk(new THREE.BoxGeometry(.24,.72,.26),col); arMesh.position.y=-.36; armR.add(arMesh);
    const arhand=mk(new THREE.BoxGeometry(.2,.2,.22),skinCol); arhand.position.y=-.74; armR.add(arhand);
    gr.add(armR);

    // Head + neck
    const neck=mk(new THREE.BoxGeometry(.22,.18,.2),skinCol); neck.position.set(0,1.76,0); gr.add(neck);
    const head=mk(new THREE.BoxGeometry(.44,.44,.44),skinCol); head.position.set(0,1.96,0); gr.add(head);

    // Hair
    if(kind!=="guard") {
      const hair=mk(new THREE.BoxGeometry(.48,.18,.46),hairCol); hair.position.set(0,2.16,-.02); gr.add(hair);
    }

    // Face
    const fm=new THREE.MeshBasicMaterial({color:0x1a1410});
    const wm=new THREE.MeshBasicMaterial({color:0xf4f0e8});
    [-1,1].forEach(s=>{ const ey=new THREE.Mesh(new THREE.BoxGeometry(.1,.1,.04),wm); ey.position.set(s*.1,1.98,.235); gr.add(ey); const pu=new THREE.Mesh(new THREE.BoxGeometry(.055,.07,.04),fm); pu.position.set(s*.1,1.975,.246); gr.add(pu); });
    const mo=new THREE.Mesh(new THREE.BoxGeometry(.16,.03,.03),fm); mo.position.set(0,1.84,.235); gr.add(mo);

    // Kind-specific extras
    if(kind==="guard"){
      const helm=mk(new THREE.SphereGeometry(.3,8,8,0,Math.PI*2,0,Math.PI*.58),0x9aa0aa); helm.position.set(0,1.98,0); gr.add(helm);
      const plume=mk(new THREE.ConeGeometry(.07,.36,5),0xcc3322); plume.position.set(0,2.32,0); gr.add(plume);
      const spear=mk(new THREE.CylinderGeometry(.035,.035,2.4,6),0x6b4a2b); spear.position.set(.48,1.1,0); gr.add(spear);
      const speartip=mk(new THREE.ConeGeometry(.085,.28,6),0xccd2da); speartip.position.set(.48,2.44,0); gr.add(speartip);
    } else if(kind==="merchant"){
      const hat=mk(new THREE.CylinderGeometry(.18,.28,.28,8),0x3a2a18); hat.position.set(0,2.22,0); gr.add(hat);
      const brim=mk(new THREE.CylinderGeometry(.32,.32,.06,10),0x3a2a18); brim.position.set(0,2.09,0); gr.add(brim);
    } else if(kind==="questgiver"||kind==="questtarget"){
      // cloak drape
      const cloak=mk(new THREE.BoxGeometry(.72,.9,.46),Math.random()>.5?0x4a3060:0x3a4a28); cloak.position.set(0,1.08,-.04); gr.add(cloak);
    }
  }
  ...
  let mark=null;
  {
    ...
    if(kind==="spirit"||kind==="dspirit"){ sym="✦"; scol="#7fd8ff"; }
    else if(kind==="merchant"){ sym="$"; scol="#79e06a"; }
    else if(kind==="questgiver"){ sym=sq?"?":"!"; scol="#ffd23f"; }
    else if(kind==="questtarget"){ sym="!"; scol="#ff8fd0"; }
    else if(kind==="signpost"){ sym="⇄"; scol="#e6eef6"; }
    if(sym){ mark=makeMarker(sym,scol); mark.position.y=2.7; gr.add(mark); }
  }
...
function buildDungeonEntrance(g) {
  ...
  const mossStone=new THREE.MeshLambertMaterial({color:0x5a6050,flatShading:true});
  const grp=g.overworldGroup;

  // ── Rock cliff face — bottom starts AT ground level ──
  const cliff=new THREE.Mesh(new THREE.BoxGeometry(14,8,8),rockMat); cliff.position.set(ex+8,ey+4,ez); cliff.castShadow=true; grp.add(cliff);
  // Extra soil/rock to fill any gap between cliff base and terrain
  const cliffBase=new THREE.Mesh(new THREE.BoxGeometry(14,2,10),rockMat); cliffBase.position.set(ex+8,ey-.8,ez); grp.add(cliffBase);
  // Boulders clustered around/atop the cliff, not blocking entrance
  [[ex+9,ey+7,ez,3.8],[ex+13,ey+3,ez+4,2.6],[ex+13,ey+3,ez-4,2.8],[ex+6,ey+6,ez+3,2.0],[ex+6,ey+6,ez-3,1.9]].forEach(([x,y,z,r])=>{
    const b=new THREE.Mesh(new THREE.DodecahedronGeometry(r,0),rockMat); b.position.set(x,y,z); b.rotation.set(Math.random(),Math.random(),Math.random()); b.castShadow=true; grp.add(b);
  });

  // ── Stone arch doorway — clearly frames the opening ──
  const archW=4.8, archH=5.2;
  // Side pillars
  [-1,1].forEach(s=>{
    const pillar=new THREE.Mesh(new THREE.BoxGeometry(1,archH,.9),mossStone); pillar.position.set(ex+1,ey+archH/2,ez+s*archW*.38); pillar.castShadow=true; grp.add(pillar);
    // Stone cap on each pillar
    const cap=new THREE.Mesh(new THREE.BoxGeometry(1.3,.5,1.3),rockMat); cap.position.set(ex+1,ey+archH+.2,ez+s*archW*.38); grp.add(cap);
  });
  // Lintel across the top
  const lintel=new THREE.Mesh(new THREE.BoxGeometry(.9,.8,archW),mossStone); lintel.position.set(ex+1,ey+archH-.2,ez); lintel.castShadow=true; grp.add(lintel);
  // Keystone
  const key=new THREE.Mesh(new THREE.BoxGeometry(1.2,.7,1.2),rockMat); key.position.set(ex+1,ey+archH+.25,ez); grp.add(key);
  // Runic carvings (flat dark slabs on pillars)
  [-1,1].forEach(s=>{
    const slab=new THREE.Mesh(new THREE.BoxGeometry(.1,.8,.5),darkMat); slab.position.set(ex+.55,ey+2.5,ez+s*archW*.35); grp.add(slab);
  });

  // ── Cave darkness behind the arch ──
  const mouth=new THREE.Mesh(new THREE.PlaneGeometry(archW*.7,archH*.75),new THREE.MeshBasicMaterial({color:0x040506,side:THREE.DoubleSide}));
  mouth.position.set(ex+1.4,ey+archH*.38,ez); mouth.rotation.y=Math.PI/2; grp.add(mouth);
  // Green moss glow emanating from inside
  const glow=new THREE.Mesh(new THREE.PlaneGeometry(3.4,4),new THREE.MeshBasicMaterial({color:0x2f7a3a,transparent:true,opacity:.35,side:THREE.DoubleSide}));
  glow.position.set(ex+1.2,ey+archH*.38,ez); glow.rotation.y=Math.PI/2; grp.add(glow);
  const glowLight=new THREE.PointLight(0x44aa44,1.5,9); glowLight.position.set(ex+1,ey+2.5,ez); grp.add(glowLight);

  // ── Entrance flanking torches ──
  const torchMat=new THREE.MeshBasicMaterial({color:0xff9030});
  [-1,1].forEach(s=>{
    const tx=ex-1.5, tz=ez+s*3.4;
    for(let i=0;i<3;i++){ const a=i/3*Math.PI*2; const leg=new THREE.Mesh(new THREE.CylinderGeometry(.04,.05,.8,5),rockMat); leg.position.set(tx+Math.cos(a)*.18,ey+.4,tz+Math.sin(a)*.18); leg.rotation.z=Math.cos(a)*.28; leg.rotation.x=-Math.sin(a)*.28; grp.add(leg); }
    const bowl=new THREE.Mesh(new THREE.CylinderGeometry(.18,.12,.18,8),rockMat); bowl.position.set(tx,ey+.9,tz); grp.add(bowl);
    const flame=new THREE.Mesh(new THREE.ConeGeometry(.14,.32,6),torchMat); flame.position.set(tx,ey+1.12,tz); grp.add(flame);
    const tpl=new THREE.PointLight(0xff7020,2.2,8); tpl.position.set(tx,ey+1.6,tz); grp.add(tpl);
  });
  for(let i=0;i<4;i++){
    const flag=new THREE.Mesh(new THREE.BoxGeometry(2.4,.14,2.2),mossStone); flag.position.set(ex-.4-i*2.4,ey+.07,ez); flag.receiveShadow=true; grp.add(flag);
  }

  // ── Dirt path from grove ──
  const pathMat=new THREE.MeshLambertMaterial({color:0x7a5f3e,flatShading:true,transparent:true,opacity:.7});
  for(let i=0;i<8;i++){ const t=i/7; const px=ex-10-t*20+Math.sin(i*1.7)*1.5, pz=ez+Math.cos(i*1.3)*2.4; const patch=new THREE.Mesh(new THREE.CircleGeometry(1.6+Math.random()*.5,8),pathMat); patch.rotation.x=-Math.PI/2; patch.position.set(px,tH(px,pz)+.04,pz); grp.add(patch); }

  // ── Guardian pines flanking ──
  [[-5,-6],[-5,6]].forEach(([dx,dz])=>{
    const tr=new THREE.Group();
    const tk=new THREE.Mesh(new THREE.CylinderGeometry(.3,.45,2.6,6),new THREE.MeshLambertMaterial({color:0x5a3f24,flatShading:true})); tk.position.y=1.3; tr.add(tk);
    const cn=new THREE.Mesh(new THREE.ConeGeometry(2,3.4,7),new THREE.MeshLambertMaterial({color:0x2f5f2c,flatShading:true})); cn.position.y=3.8; cn.castShadow=true; tr.add(cn);
    tr.position.set(ex+dx,tH(ex+dx,ez+dz),ez+dz); grp.add(tr);
  });

  // ── Trigger and collision ──
  // Trigger sits in OPEN ground at the arch threshold
  g.dungeonEntrance=new THREE.Vector3(ex-0.5,ey,ez);
  // Collision only on the solid cliff face (east), NOT blocking the entrance
  g.props.push({pos:new THREE.Vector3(ex+8,0,ez),radius:4.5});
}
...
const ESPEC={
  ...
  treant:   {hp:[1200,1200],dmg:[42,42],speed:[1.5,1.5],xp:[420,420],aggro:18},
  ...
  titan:    {hp:[1400,1400],dmg:[54,54],speed:[1.6,1.6],xp:[520,520],aggro:20},
  emberwolf:{hp:[48,48],dmg:[16,16],speed:[6,6],xp:[40,40],aggro:12},
  magmawisp:{hp:[70,100],dmg:[20,26],speed:[3.2,3.6],xp:[55,80],aggro:14},
  colossus: {hp:[1600,1600],dmg:[62,62],speed:[1.5,1.5],xp:[620,620],aggro:20},
  galehound:{hp:[60,60],dmg:[18,18],speed:[6.4,6.4],xp:[48,48],aggro:13},
  stormwisp:{hp:[80,110],dmg:[24,30],speed:[3.6,4],xp:[64,90],aggro:15},
  stormcaller:{hp:[1900,1900],dmg:[68,68],speed:[1.8,1.8],xp:[780,780],aggro:24},
  hollowking: {hp:[3400,3400],dmg:[82,82],speed:[2.2,2.2],xp:[2200,2200],aggro:32},
};
...
function spawnEnemy(g,x,z,type="corrupted",tier=1,zoneKey="overworld"){
  ...
  const scale=type==="rootling"?.7:type==="treant"?2.2:type==="titan"?2.4:type==="colossus"?2.6:type==="stormcaller"?2.5:type==="hollowking"?2.3:(type==="wolf"||type==="frostwolf"||type==="emberwolf"||type==="galehound")?.7:type==="corrupted"?(tier>=2?1.2:1):1;
  ...
  if(type==="wolf"||type==="frostwolf"||type==="galehound"){
    ...
    // legs as pivot groups for walk animation
    const wolfLegs=[];
    [[-.26,.3,.62],[.26,.3,.62],[-.26,.3,-.62],[.26,.3,-.62]].forEach(([lx,ly,lz],i)=>{
      const lg=new THREE.Group(); lg.position.set(lx,ly,lz);
      const legM=bx(new THREE.BoxGeometry(.2,.62,.24),c2); legM.position.y=-.31; lg.add(legM);
      const paw=bx(new THREE.BoxGeometry(.24,.16,.34),c3); paw.position.set(0,-.56,.05); lg.add(paw);
      gr.add(lg); wolfLegs.push(lg);
    });
    gr.userData.flegFL=wolfLegs[0]; gr.userData.flegFR=wolfLegs[1]; gr.userData.flegBL=wolfLegs[2]; gr.userData.flegBR=wolfLegs[3];
    // bushy tail, raised
    ...
    // Legs as pivot groups for walk anim
    const legL=new THREE.Group(); legL.position.set(-.18,.88,0); const llM=bx(new THREE.BoxGeometry(.28,.82,.3),0x3a2e20); llM.position.y=-.41; legL.add(llM); gr.add(legL);
    const legR=new THREE.Group(); legR.position.set(.18,.88,0);  const lrM=bx(new THREE.BoxGeometry(.28,.82,.3),0x3a2e20); lrM.position.y=-.41; legR.add(lrM); gr.add(legR);
    ...
    // Arms as pivot groups
    const armL=new THREE.Group(); armL.position.set(-.52,1.82,0); const alM=bx(new THREE.BoxGeometry(.24,.76,.26),0x5a4530); alM.position.y=-.38; armL.add(alM); gr.add(armL);
    const armR=new THREE.Group(); armR.position.set(.52,1.82,0);  const arM=bx(new THREE.BoxGeometry(.24,.76,.26),0x5a4530); arM.position.y=-.38; armR.add(arM);
    const bsw=new THREE.Group(); bsw.position.set(0,-.62,0);
    const bbl=new THREE.Mesh(new THREE.BoxGeometry(.08,1.0,.04),lm(0xb8c4cc)); bbl.position.set(0,.06,0); bbl.castShadow=true; bsw.add(bbl);
    const bgrd=new THREE.Mesh(new THREE.BoxGeometry(.38,.08,.08),lm(0x887755)); bgrd.position.set(0,-.44,0); bsw.add(bgrd);
    armR.add(bsw); gr.add(armR);
    // Store refs on a temp object so they get captured below
    gr.userData.legL=legL; gr.userData.legR=legR; gr.userData.armL=armL; gr.userData.armR=armR;
  } else if(type==="skeleton"){
    const boneM={emissive:0x223300,emissiveIntensity:.18};
    const boneL=0xe2d8bc, boneD=0xc9bd9a;
    // Legs as pivot groups for walk animation
    const skLL=new THREE.Group(); skLL.position.set(-.16,.88,0);
    const ll1=bx(new THREE.BoxGeometry(.14,.56,.16),boneL,boneM); ll1.position.y=-.28; skLL.add(ll1);
    const ll2=bx(new THREE.BoxGeometry(.12,.52,.14),boneD,boneM); ll2.position.y=-.72; skLL.add(ll2);
    const lf=bx(new THREE.BoxGeometry(.18,.1,.3),boneD,boneM); lf.position.set(0,-.96,.1); skLL.add(lf);
    gr.add(skLL);
    const skLR=new THREE.Group(); skLR.position.set(.16,.88,0);
    const rl1=bx(new THREE.BoxGeometry(.14,.56,.16),boneL,boneM); rl1.position.y=-.28; skLR.add(rl1);
    const rl2=bx(new THREE.BoxGeometry(.12,.52,.14),boneD,boneM); rl2.position.y=-.72; skLR.add(rl2);
    const rf=bx(new THREE.BoxGeometry(.18,.1,.3),boneD,boneM); rf.position.set(0,-.96,.1); skLR.add(rf);
    gr.add(skLR);
    // Pelvis + spine
    ...
    // Arms as pivot groups for swing animation
    const skAL=new THREE.Group(); skAL.position.set(-.32,1.88,0);
    const al1=bx(new THREE.CylinderGeometry(.06,.055,.5,6),boneL,boneM); al1.position.y=-.25; skAL.add(al1);
    const al2=bx(new THREE.CylinderGeometry(.05,.05,.46,6),boneD,boneM); al2.position.y=-.64; skAL.add(al2);
    skAL.rotation.z=.18; gr.add(skAL);
    const skAR=new THREE.Group(); skAR.position.set(.32,1.88,0);
    const ar1=bx(new THREE.CylinderGeometry(.06,.055,.5,6),boneL,boneM); ar1.position.y=-.25; skAR.add(ar1);
    const ar2=bx(new THREE.CylinderGeometry(.05,.05,.46,6),boneD,boneM); ar2.position.y=-.46; ar2.rotation.x=-.5; skAR.add(ar2);
    // Rusty sword in right hand
    const sgrp=new THREE.Group(); sgrp.position.set(0,-.86,.14);
    const blade=bx(new THREE.BoxGeometry(.07,.85,.03),0x8a8478); blade.position.set(0,.42,0); sgrp.add(blade);
    const sguard=bx(new THREE.BoxGeometry(.3,.07,.07),0x6a6050); sgrp.add(sguard);
    skAR.add(sgrp); gr.add(skAR);
    gr.userData.legL=skLL; gr.userData.legR=skLR; gr.userData.armL=skAL; gr.userData.armR=skAR;
  ...
  } else if(type==="hollowking"){
    const sc=scale;
    const voidM=lm(0x0e0814,{emissive:0x330044,emissiveIntensity:.55});
    const crackM=new THREE.MeshBasicMaterial({color:0xbb44ff});
    const voidDark=lm(0x090610,{emissive:0x220033,emissiveIntensity:.4});
    // Wide cloak base
    const cloak=bx(new THREE.CylinderGeometry(.55*sc,1.6*sc,3.0*sc,10),0x0a0610,{emissive:0x220033,emissiveIntensity:.3}); cloak.position.y=1.5*sc; gr.add(cloak);
    // Legs (pivot for walk anim)
    const hkLL=new THREE.Group(); hkLL.position.set(-.5*sc,3.0*sc,0);
    const hkLLM=bx(new THREE.BoxGeometry(.46*sc,1.1*sc,.46*sc),0x0e0814,{emissive:0x220033,emissiveIntensity:.4}); hkLLM.position.y=-.55*sc; hkLL.add(hkLLM);
    const hkLLf=bx(new THREE.BoxGeometry(.44*sc,.2*sc,.56*sc),0x0a0610); hkLLf.position.set(0,-1.12*sc,.1*sc); hkLL.add(hkLLf); gr.add(hkLL);
    const hkLR=new THREE.Group(); hkLR.position.set(.5*sc,3.0*sc,0);
    const hkLRM=bx(new THREE.BoxGeometry(.46*sc,1.1*sc,.46*sc),0x0e0814,{emissive:0x220033,emissiveIntensity:.4}); hkLRM.position.y=-.55*sc; hkLR.add(hkLRM);
    const hkLRf=bx(new THREE.BoxGeometry(.44*sc,.2*sc,.56*sc),0x0a0610); hkLRf.position.set(0,-1.12*sc,.1*sc); hkLR.add(hkLRf); gr.add(hkLR);
    // Torso
    body=bx(new THREE.BoxGeometry(1.7*sc,1.5*sc,.95*sc),0x0e0814,{emissive:0x330044,emissiveIntensity:.5}); body.position.y=4.0*sc; gr.add(body);
    // Void crack seams on chest
    [[0,.15,.5],[-.35,-.1,.5],[.35,.2,.5]].forEach(([x,y,z])=>{ const cr=new THREE.Mesh(new THREE.BoxGeometry(.08*sc,.9*sc,.04),crackM); cr.position.set(x*sc,(4.0+y)*sc,z*sc); cr.rotation.z=(Math.random()-.5)*.7; gr.add(cr); });
    // Pauldrons
    [-1,1].forEach(sx=>{ const pl=bx(new THREE.BoxGeometry(.8*sc,.6*sc,.8*sc),0x0e0814,{emissive:0x330044,emissiveIntensity:.45}); pl.position.set(sx*1.2*sc,4.7*sc,0); gr.add(pl); const sp=bx(new THREE.ConeGeometry(.18*sc,.7*sc,5),0x0a0610,{emissive:0x440066,emissiveIntensity:.6}); sp.position.set(sx*1.2*sc,5.2*sc,0); gr.add(sp); });
    // Arms (pivot for walk anim)
    const hkAL=new THREE.Group(); hkAL.position.set(-1.3*sc,4.5*sc,0);
    const hkALM=bx(new THREE.BoxGeometry(.44*sc,1.4*sc,.44*sc),0x0e0814,{emissive:0x220033,emissiveIntensity:.4}); hkALM.position.y=-.7*sc; hkAL.add(hkALM);
    for(let c=0;c<3;c++){ const cl=bx(new THREE.ConeGeometry(.1*sc,.52*sc,4),0x0a0610,{emissive:0x440066,emissiveIntensity:.7}); cl.position.set((c-1)*.18*sc,-1.46*sc,.12*sc); cl.rotation.x=-.5; hkAL.add(cl); } gr.add(hkAL);
    const hkAR=new THREE.Group(); hkAR.position.set(1.3*sc,4.5*sc,0);
    const hkARM=bx(new THREE.BoxGeometry(.44*sc,1.4*sc,.44*sc),0x0e0814,{emissive:0x220033,emissiveIntensity:.4}); hkARM.position.y=-.7*sc; hkAR.add(hkARM);
    // Right hand holds a void shard
    const shard=bx(new THREE.CylinderGeometry(.08*sc,.14*sc,1.8*sc,5),0x2a0a44,{emissive:0x9944ff,emissiveIntensity:.9}); shard.position.set(0,-1.7*sc,.2*sc); shard.rotation.x=-.3; hkAR.add(shard);
    for(let c=0;c<3;c++){ const cl=bx(new THREE.ConeGeometry(.1*sc,.52*sc,4),0x0a0610,{emissive:0x440066,emissiveIntensity:.7}); cl.position.set((c-1)*.18*sc,-1.46*sc,.12*sc); cl.rotation.x=-.5; hkAR.add(cl); } gr.add(hkAR);
    // Neck + Head
    const hn=bx(new THREE.BoxGeometry(.52*sc,.42*sc,.52*sc),0x0e0814,{emissive:0x220033,emissiveIntensity:.5}); hn.position.y=5.3*sc; gr.add(hn);
    const hd=bx(new THREE.BoxGeometry(.96*sc,.96*sc,.86*sc),0x0e0814,{emissive:0x330044,emissiveIntensity:.5}); hd.position.y=5.95*sc; gr.add(hd);
    // Hollow eye sockets — glowing purple
    [-1,1].forEach(sx=>{ const sock=new THREE.Mesh(new THREE.BoxGeometry(.3*sc,.26*sc,.08),lm(0x000000)); sock.position.set(sx*.24*sc,5.98*sc,.46*sc); gr.add(sock); const glow=new THREE.Mesh(new THREE.SphereGeometry(.12*sc,8,8),new THREE.MeshBasicMaterial({color:0xdd44ff})); glow.position.set(sx*.24*sc,5.98*sc,.48*sc); gr.add(glow); });
    // Jaw crack
    const jawCr=new THREE.Mesh(new THREE.BoxGeometry(.2*sc,.06,.04),crackM); jawCr.position.set(0,5.66*sc,.48*sc); gr.add(jawCr);
    // Crown of void spikes
    for(let i=0;i<8;i++){ const a=i/8*Math.PI*2; const sp2=bx(new THREE.ConeGeometry(.14*sc,.72*sc,4),0x0e0814,{emissive:0x440066,emissiveIntensity:.75}); sp2.position.set(Math.cos(a)*.42*sc,6.6*sc,Math.sin(a)*.42*sc); sp2.rotation.z=Math.cos(a)*.32; sp2.rotation.x=Math.sin(a)*.32; gr.add(sp2); }
    // Central crown jewel (glowing)
    const jewel=new THREE.Mesh(new THREE.OctahedronGeometry(.22*sc,0),new THREE.MeshBasicMaterial({color:0xff66ff})); jewel.position.y=6.9*sc; gr.add(jewel);
    // Void aura point light
    const vl=new THREE.PointLight(0x9944ff,3.5,10*sc); vl.position.y=4*sc; gr.add(vl);
    gr.userData.legL=hkLL; gr.userData.legR=hkLR; gr.userData.armL=hkAL; gr.userData.armR=hkAR;
  } else {
    // treant boss
  ...
  const hbW=type==="treant"?2.4*scale:type==="titan"||type==="colossus"||type==="stormcaller"||type==="hollowking"?6:type==="bandit"||type==="skeleton"?1.6:type==="wraith"||type==="magmawisp"||type==="stormwisp"?1.5:1.4*scale;
  const hbY=type==="rootling"?1.3:type==="treant"?4.6*scale:type==="titan"||type==="colossus"?5.6*scale:type==="stormcaller"?5.8*scale:type==="hollowking"?8.5*scale:(type==="wolf"||type==="frostwolf"||type==="emberwolf"||type==="galehound")?1.8:type==="bandit"?2.9:type==="skeleton"?2.75:type==="wraith"||type==="magmawisp"||type==="stormwisp"?2.3:2.1*scale;
  ...
  const en={mesh:gr,body,hbCanvas:hbc,hbTex:hbt,hp,maxhp:hp,dmg,speed,xpVal:xp,type,tier,scale,zone:zoneKey,
    state:"wander",wanderT:0,wanderDir:Math.random()*Math.PI*2,attackCD:0,hitFlash:0,home:new THREE.Vector3(x,0,z),bob:Math.random()*6,dead:false,revived:false,aggro:sp.aggro,walk:0,
    legL:gr.userData.legL||null, legR:gr.userData.legR||null, armL:gr.userData.armL||null, armR:gr.userData.armR||null,
    flegFL:gr.userData.flegFL||null,flegFR:gr.userData.flegFR||null,flegBL:gr.userData.flegBL||null,flegBR:gr.userData.flegBR||null};
...
function makePlayer(g){
  ...
  // Helmet overlay — crown sits on top of head, cheek guards come down the sides, face stays open
  const armHelmet=new THREE.Group(); armHelmet.visible=false; gr.add(armHelmet);
  const helmCrown=mk(new THREE.BoxGeometry(.52,.28,.52),0x9aa0aa); helmCrown.position.set(0,2.12,0); armHelmet.add(helmCrown);
  const helmBrim=mk(new THREE.BoxGeometry(.58,.1,.58),0x9aa0aa); helmBrim.position.set(0,1.99,0); armHelmet.add(helmBrim);
  [-1,1].forEach(sx=>{ const cheek=mk(new THREE.BoxGeometry(.1,.3,.44),0x9aa0aa); cheek.position.set(sx*.27,1.82,0); armHelmet.add(cheek); });
  const helmBack=mk(new THREE.BoxGeometry(.52,.28,.14),0x9aa0aa); helmBack.position.set(0,1.84,-.24); armHelmet.add(helmBack);
...
function updatePlayerArmor(g,state){
  ...
  if(pd.armHelmet){ const k=eq.helmet; pd.armHelmet.visible=!!k; if(k){ const hcol=colOf(k); pd.armHelmet.children.forEach(c=>{ if(c.material) c.material.color.setHex(hcol); }); pd.armCrest.visible=!!ITEMS[k]?.crest; if(ITEMS[k]?.crest) pd.armCrest.material.color.setHex(ITEMS[k].crest); } else pd.armCrest.visible=false; }
...
// ── Class Skills ──
// Warrior: Whirlwind — spin strike hitting ALL nearby enemies
// Ranger:  Volley    — rapid 5 arrows in a wide fan
// Mage:    Arcane Surge — big AoE magic burst around the player
const SKILL_CD={Warrior:12, Ranger:10, Mage:14};
const SKILL_MANA={Warrior:0, Ranger:20, Mage:40};
const SKILL_NAME={Warrior:"Whirlwind ⚔", Ranger:"Volley 🏹", Mage:"Arcane Surge ✦"};

function useClassSkill(g,state,toast,secondary=false){
  if(g.paused||g.dlgState) return;
  const sl=state.skillLevels||{primary:1,secondary:0};
  const lvl=secondary?sl.secondary:sl.primary;
  if(!lvl){ toast?.("Skill not unlocked — check Skills tab"); return; }
  const now=performance.now()/1000;
  const cdKey=secondary?"skillLastUsed2":"skillLastUsed";
  const cd=secondary?18:(SKILL_CD[state.cls]||12);
  const elapsed=now-(g[cdKey]||0);
  if(elapsed<cd){ toast?.(`${secondary?"Secondary":"Skill"}: ${Math.ceil(cd-elapsed)}s`); return; }
  const manaCost=secondary?30:(SKILL_MANA[state.cls]||0);
  if(state.mana<manaCost){ toast?.("Not enough mana"); return; }
  state.mana=Math.max(0,state.mana-manaCost);
  g[cdKey]=now;
  const cs=calcStats(state);
  const p=g.player.position;
  const dmgMult=1+lvl*.45;
  if(!secondary){
    if(state.cls==="Warrior"){
      const reach=2.8+lvl*.6; let hits=0;
      g.enemies.forEach(en=>{ if(en.dead||en.zone!==g.zone)return; const d=Math.hypot(en.mesh.position.x-p.x,en.mesh.position.z-p.z); if(d<reach){ const dmg=Math.round(cs.atkTotal*1.8*dmgMult*(.9+Math.random()*.3)); en.hp-=dmg; en.hitFlash=.18; en.state="chase"; drawEHB(en); addDmgNum(g,en.mesh.position.x,en.mesh.position.y+1.4,en.mesh.position.z,dmg,true); spawnSparks(g,en.mesh.position.x,en.mesh.position.y+.9,en.mesh.position.z,true); hits++; } });
      SFX.heavy(); if(hits)SFX.hit(); toast(hits?`Whirlwind! Hit ${hits}!`:"Whirlwind!"); g.spinAnim={t:.5,dir:1};
    } else if(state.cls==="Ranger"){
      const arrows=3+lvl*2; const spread=.13*lvl;
      for(let i=0;i<arrows;i++) fireProjectile(g,state,"arrow",.82*dmgMult,(i-(arrows-1)/2)*spread);
      SFX.bow(); toast(`Volley — ${arrows} arrows!`);
    } else {
      const radius=4+lvl*1.5; let hits=0;
      g.enemies.forEach(en=>{ if(en.dead||en.zone!==g.zone)return; const d=Math.hypot(en.mesh.position.x-p.x,en.mesh.position.z-p.z); if(d<radius){ const dmg=Math.round(cs.atkTotal*2.2*dmgMult*(.85+Math.random()*.3)); en.hp-=dmg; en.hitFlash=.2; en.state="chase"; drawEHB(en); addDmgNum(g,en.mesh.position.x,en.mesh.position.y+1.4,en.mesh.position.z,dmg,true); spawnSparks(g,en.mesh.position.x,en.mesh.position.y+.9,en.mesh.position.z,true); hits++; } });
      spawnSparks(g,p.x,p.y+1,p.z,true); for(let i=0;i<8;i++){const a=i/8*Math.PI*2;spawnSparks(g,p.x+Math.cos(a)*3.5,p.y+.5,p.z+Math.sin(a)*3.5,true);}
      SFX.magic(); toast(hits?`Arcane Surge! Hit ${hits}!`:"Arcane Surge!");
    }
  } else {
    if(state.cls==="Warrior"){
      const heal=Math.round(cs.maxHp*(0.12+lvl*.06)); state.hp=Math.min(cs.maxHp,state.hp+heal);
      g.defBuffT=3+lvl*1.5; toast(`Iron Resolve! +${heal}HP, defense boost ${(3+lvl*1.5).toFixed(0)}s`); SFX.levelup();
    } else if(state.cls==="Ranger"){
      g.eagleEye=(2+lvl); toast(`Eagle Eye! Next ${2+lvl} shots auto-crit!`); SFX.bow();
    } else {
      g.manaShieldPct=.15+lvl*.12; g.manaShieldT=6+lvl*2; toast(`Mana Shield! Absorbs ${Math.round((.15+lvl*.12)*100)}% damage for ${6+lvl*2}s`); SFX.magic();
    }
  }
}
function fireProjectile(g,state,kind,mult,spread){
  const p=g.player.position;
  // Fire in the direction the character is FACING, not where the camera looks
  const yaw=(g.player.rotation.y)+(spread||0);
...
function updatePlayer(g,dt,state){
  ...
  if(g.rooted>0){ fwd=0; strafe=0; } // rooted — can't move
  const mag=Math.hypot(strafe,fwd), cs=calcStats(state);
  const spd=state.speed*cs.speedMult*(g.blocking?.5:1)*(g.sprint&&state.st>0?1.5:1)*(g.rollTimer>0?2.2:1)*(1-(g.frostSlow||0));
  let moving=false;
  if(mag>.05){
  ...
  // Warrior whirlwind spin visual
  if(g.spinAnim){ g.spinAnim.t-=dt; g.player.rotation.y+=dt*18*g.spinAnim.dir; if(g.spinAnim.t<=0) g.spinAnim=null; }
  ...
  if(ow){ const dist=Math.hypot(p.position.x,p.position.z); if(dist>112){p.position.x*=112/dist;p.position.z*=112/dist;} }
    ...
    // Hard outer bounds matching the dungeon layout (entrance z≈10 → boss floor z≈-96, rooms x≈±36)
    p.position.x=clamp(p.position.x,-35,35); p.position.z=clamp(p.position.z,-96,8.4);
  ...
  else if(g.zone==="hollow"){ p.position.x=clamp(p.position.x,-24,24); p.position.z=clamp(p.position.z,-26,22); }
...
function updateEnemies(g,dt,state,toast,setQuestTrack){
  ...
  g.enemies.forEach(en=>{
    ...
    if(en.state==="chase"){
      if(dist>1.5*en.scale){
        m.position.x+=dx/dist*en.speed*dt; m.position.z+=dz/dist*en.speed*dt;
        m.rotation.y=Math.atan2(dx/dist,dz/dist);
        en.walk=(en.walk||0)+dt*en.speed*2.6;
        // Humanoid walk swing
        if(en.legL){ en.legL.rotation.x=Math.sin(en.walk)*.6; en.legR.rotation.x=-Math.sin(en.walk)*.6; }
        if(en.armL){ en.armL.rotation.x=-Math.sin(en.walk)*.45; en.armR.rotation.x=Math.sin(en.walk)*.45; }
        if(en.type==="wolf"||en.type==="frostwolf"||en.type==="emberwolf"||en.type==="galehound"){
          if(en.flegFL) en.flegFL.rotation.x=Math.sin(en.walk)*.8; if(en.flegFR) en.flegFR.rotation.x=-Math.sin(en.walk)*.8;
          if(en.flegBL) en.flegBL.rotation.x=-Math.sin(en.walk)*.8; if(en.flegBR) en.flegBR.rotation.x=Math.sin(en.walk)*.8;
        }
      }
      ...
      en.walk=(en.walk||0)+dt*en.speed*.9;
      if(en.legL){ en.legL.rotation.x=Math.sin(en.walk)*.3; en.legR.rotation.x=-Math.sin(en.walk)*.3; }
      if(en.armL){ en.armL.rotation.x=Math.sin(en.walk)*.2+.06; en.armR.rotation.x=-Math.sin(en.walk)*.2+.06; }
      if(en.flegFL){ en.flegFL.rotation.x=Math.sin(en.walk)*.4; en.flegFR.rotation.x=-Math.sin(en.walk)*.4; en.flegBL.rotation.x=-Math.sin(en.walk)*.4; en.flegBR.rotation.x=Math.sin(en.walk)*.4; }
    ...
    if(en.hp<=0&&!en.dead){
      ...
      state.codex[en.type]=(state.codex[en.type]||0)+1;
      ...
      if(en.type==="bandit"&&state.sq?.harbour?.s===1&&en.mesh.position.z<-45){ const h=state.sq.harbour; h.p++; if(h.p>=5){h.s=2;toast("Dockside Threat complete — return to Harbourmaster Thorne");SFX.quest();} else toast(`Bandits: ${h.p}/5`); }
      if(en.type==="treant"){
      ...
      if(en.type==="stormcaller"){
        ...
        if(q.hasEarthRelic&&q.hasFrostRelic&&q.hasFlameRelic&&q.hasStormRelic&&!q.heartstoneRestored){
          q.heartstoneRestored=true; if(state.story) state.story.stage=20;
          setTimeout(()=>toast("✦ All Four Relics are gathered…"),900);
          setTimeout(()=>toast("✦ The Heartstone stirs beneath Verandah."),2200);
          setTimeout(()=>toast("✦ A portal opens in the castle courtyard."),3600);
          setTimeout(()=>toast("◈ Speak with Captain Aldric in Verandah."),5000);
        }
      }
      if(en.type==="hollowking"){
        state.quest.hollowKingDefeated=true;
        state.inv.hollow_crown=(state.inv.hollow_crown||0)+1;
        state.inv.shadow_cloak=(state.inv.shadow_cloak||0)+1;
        state.inv.void_blade=(state.inv.void_blade||0)+1;
        gainXP(state,1500,toast);
        if(state.story) state.story.stage=99;
        g.setBoss?.(null);
        // Clear remaining hollow enemies
        g.enemies.forEach(e=>{ if(e.zone==="hollow"&&e.type!=="hollowking"&&!e.dead){ e.dead=true; e.mesh.parent?.remove(e.mesh); } });
        // Heartstone blazes to life
        if(g.heartstoneLight){ g.heartstoneLight.intensity=18; g.heartstoneLight.color.setHex(0xffd080); }
        if(g.heartstoneCrystal){ g.heartstoneCrystal.material.color.setHex(0xffd060); g.heartstoneCrystal.material.emissive.setHex(0xffaa00); g.heartstoneCrystal.material.emissiveIntensity=1.5; }
        SFX.levelup(); updateQuestTrack(state,setQuestTrack); updateHUD(g,g.setUi);
        setTimeout(()=>toast("✦ The Hollow King crumbles into void dust…"),600);
        setTimeout(()=>toast("✦ The Heartstone blazes with full golden light!"),2000);
        setTimeout(()=>toast("✦ The corruption recedes from every corner of Verandiah…"),3600);
        setTimeout(()=>toast("✦ The Four Relics are one with the Heartstone once more."),5200);
        setTimeout(()=>toast("🏆 WARDEN OF THE REALM — "+(state.charName||"Hero")+" — the land is saved!"),7000);
        // Spawn completion guardian
        setTimeout(()=>{ if(!g.hollowSpiritSpawned){ g.hollowSpiritSpawned=true; addNPC(g,"hollow",{name:"Guardian Spirit",kind:"dspirit",spirit:true,x:0,z:4,col:0xffd080}); }},1000);
      }
    }
  });
...
function updatePickups(g,dt,state,toast,setUi){
  const p=g.player.position;
  g.pickupBatchT=(g.pickupBatchT||0)-dt;
  let batchGold=0, batchItems={}, batchDirty=false;
  for(let i=g.pickups.length-1;i>=0;i--){
    ...
    if(Math.hypot(p.x-pk.mesh.position.x,p.z-pk.mesh.position.z)<1.6){
      if(pk.gold>0){ state.gold+=pk.gold; batchGold+=pk.gold; }
      else {
        state.inv[pk.item]=(state.inv[pk.item]||0)+pk.amount;
        batchItems[pk.item]=(batchItems[pk.item]||0)+pk.amount;
        if(pk.item==="herb"&&state.sq?.herbs?.s===1){ const h=state.sq.herbs; h.p++; if(h.p>=5){h.s=2;toast("Herbs gathered — return to Herbalist Wren");SFX.quest();} }
      }
      SFX.pickup(); pk.mesh.parent?.remove(pk.mesh); g.pickups.splice(i,1); batchDirty=true;
    }
  }
  if(batchDirty){
    updateHUD(g,setUi);
    // Only show toast if cooldown elapsed — group everything into one line
    if(g.pickupBatchT<=0){
      g.pickupBatchT=1.2;
      const parts=[];
      if(batchGold>0) parts.push(`+${batchGold}◈`);
      Object.entries(batchItems).forEach(([k,n])=>{ const it=ITEMS[k]; parts.push(it?`${it.name}${n>1?` ×${n}`:""}`:k); });
    ...
    if((n.kind==="citizen"||n.kind==="guard") && n.zone===g.zone){
      ...
      if(dest){
        ...
        if(d>0.45){
          ...
          n.mesh.rotation.y=Math.atan2(dx,dz); n.mesh.position.y=n.baseY;
          n.walk=(n.walk||0)+dt*n.speed*2.4;
          // Leg swing
          if(n.legL) n.legL.rotation.x=Math.sin(n.walk)*.55;
          if(n.legR) n.legR.rotation.x=-Math.sin(n.walk)*.55;
          if(n.armL) n.armL.rotation.x=-Math.sin(n.walk)*.4;
          if(n.armR) n.armR.rotation.x=Math.sin(n.walk)*.4;
        } else {
          if(night && n.kind==="citizen"){ n.mesh.visible=false; }
          else {
            n.tgt=null; n.wait=1.5+Math.random()*3.5; n.mesh.position.y=n.baseY;
            if(n.legL) n.legL.rotation.x=0;
            if(n.legR) n.legR.rotation.x=0;
            if(n.armL) n.armL.rotation.x=.08;
            if(n.armR) n.armR.rotation.x=.08;
          }
        }
      } else {
        // idle — gentle sway
        if(n.legL) n.legL.rotation.x=0;
        if(n.legR) n.legR.rotation.x=0;
        if(n.armL) n.armL.rotation.x=Math.sin(n.bob*.6)*.06+.06;
        if(n.armR) n.armR.rotation.x=-Math.sin(n.bob*.6)*.06+.06;
      }
...
function updateSky(g){
  if(g.zone==="hollow"){ g.scene.background.setHex(0x060408); g.sun.intensity=0.08; g.hemi.intensity=0.22; g.hemi.color.setHex(0x440066); g.hemi.groundColor.setHex(0x110018); if(g.scene.fog)g.scene.fog.color.setHex(0x060408); return; }
  if(g.zone==="dungeon"){ g.scene.background.setHex(0x150e08); g.sun.intensity=1.1; g.hemi.intensity=.9; g.hemi.color.setHex(0xcc8844); g.hemi.groundColor.setHex(0x331a08); if(g.scene.fog)g.scene.fog.color.setHex(0x150e08); return; }
...
function gainXP(state,n,toast){
  ...
  state.xp+=n;
  while(state.level<MAX_LEVEL && state.xp>=(state.xpNext||XP_TABLE[state.level]||9999)){
    state.xp-=state.xpNext; state.level++;
    state.xpNext=XP_TABLE[state.level]||Math.floor((XP_TABLE[MAX_LEVEL-1]||24000)*1.3);
    // Stat gains per level — class-tuned
    state.maxHp  += state.cls==="Warrior"?18:state.cls==="Mage"?10:14;
    state.maxSt  += state.cls==="Ranger"?14:8;
    state.maxMana+= state.cls==="Mage"?16:state.cls==="Ranger"?8:5;
    state.atk    += state.cls==="Warrior"?3:state.cls==="Mage"?4:2;
    ...
    // Skill unlock messages at milestone levels
    const msg=state.level===5?` — Skill enhanced!`:state.level===10?` — Power surges within you!`:state.level===15?` — A new mastery unlocked!`:state.level===20?` — Elite tier reached!`:state.level===MAX_LEVEL?` — MAX LEVEL REACHED!`:"";
    // Award skill points at milestones
    if([5,10,15,20,25].includes(state.level)){ state.skillPoints=(state.skillPoints||0)+1; toast?.(`⬆ Level ${state.level}!${msg} (+1 Skill Point)`); }
    else toast?.(`⬆ Level ${state.level}!${msg}`);
  }
}
...
function buildDungeon(g){
  ...
  const floorMat=new THREE.MeshLambertMaterial({color:0x2a2420,flatShading:true,polygonOffset:true,polygonOffsetFactor:1,polygonOffsetUnits:1});
  const wallMat=new THREE.MeshLambertMaterial({color:0x4a443e,flatShading:true,polygonOffset:true,polygonOffsetFactor:2,polygonOffsetUnits:2});
  const mossMat=new THREE.MeshLambertMaterial({color:0x3a5230,flatShading:true,polygonOffset:true,polygonOffsetFactor:1,polygonOffsetUnits:1});
  const torchMat=new THREE.MeshBasicMaterial({color:0xff9a40});
  ...
  const torch=(cx,cz)=>{
    // Iron brazier on a tripod stand
    for(let i=0;i<3;i++){
      const a=i/3*Math.PI*2;
      const leg=new THREE.Mesh(new THREE.CylinderGeometry(.045,.055,.9,5),wallMat); leg.position.set(cx+Math.cos(a)*.22,.45,cz+Math.sin(a)*.22); leg.rotation.z=Math.cos(a)*.28; leg.rotation.x=-Math.sin(a)*.28; dg.add(leg);
    }
    const bowl=new THREE.Mesh(new THREE.CylinderGeometry(.22,.15,.2,8),wallMat); bowl.position.set(cx,.96,cz); dg.add(bowl);
    const rim=new THREE.Mesh(new THREE.TorusGeometry(.22,.04,6,10),wallMat); rim.position.set(cx,1.06,cz); rim.rotation.x=Math.PI/2; dg.add(rim);
    const flame=new THREE.Mesh(new THREE.ConeGeometry(.16,.38,6),torchMat); flame.position.set(cx,1.28,cz); dg.add(flame);
    const pl=new THREE.PointLight(0xff8833,3.2,18); pl.position.set(cx,1.8,cz); dg.add(pl); g.dunTorchLights.push(pl);
  };
  ...
  F(0,1.5,20,16);
  // Side walls full length, north wall seals the exit end
  W(-10,1.5,1,16); W(10,1.5,1,16); W(0,9.5,20,2); // thicker north (exit) wall
  // Corner seal walls — close the escape gaps at NW/NE corners of entry
  W(-9.5,9.4,1,2); W(9.5,9.4,1,2);
  W(-6.5,-6.5,7,1); W(6.5,-6.5,7,1); // south entrance wall w/ corridor gap
  torch(-9,7); torch(9,7); torch(-9,-4); torch(9,-4);
  // Corridor A torches
  torch(-3,-8); torch(3,-8); torch(-3,-13); torch(3,-13);
  ...
  // ── CORRIDOR A ──
  F(0,-10,8,10); W(-3.5,-10,1,11); W(3.5,-10,1,11);

  // ── JUNCTION ──
  room(0,-23,20,18,"Junction");
  F(0,-23,20,18);
  W(-6.5,-14,7,1); W(6.5,-14,7,1);     // north wall gap (from corridor A)
  W(-10,-19.5,1,9); W(-10,-28.5,1,5);  // west wall, leaves gap ~z:-24 to -26 (door)
  W(10,-19.5,1,9); W(10,-28.5,1,5);    // east wall, leaves gap ~z:-24 to -26 (door)
  W(-6.5,-32,7,1); W(6.5,-32,7,1);     // south wall gap to corridor B
  // ── Seal corner escape gaps at junction edges ──
  // NW and NE corners where side-wall meets north wall
  W(-9.5,-14.5,1,1.5); W(9.5,-14.5,1,1.5);
  // Door channel walls — narrow the passage to prevent diagonal squeeze-through
  W(-11.5,-23,1,4); W(11.5,-23,1,4);   // outer walls just past the junction
  ...
  torch(-3,-35); torch(3,-35); torch(-3,-41); torch(3,-41);
  ...
  torch(-3,-61); torch(3,-61); torch(-3,-67); torch(3,-67);

  // ── BOSS ARENA ──
  room(0,-80,32,28,"Thornheart's Den");
  F(0,-80,32,28,mossMat);
  W(-16,-80,1,28); W(16,-80,1,28); W(0,-94.5,32,2); // thick south arena wall
  ...
  g.dunMinZ=-96; g.dunMaxZ=12;
  g.dunXMin=-38; g.dunXMax=38;
  // Invisible perimeter walls — hard boundary the player can never exit
  g.dunWalls.push({x1:-38,x2:38,z1:10.5,z2:14,mesh:null,open:false});
  g.dunWalls.push({x1:-38,x2:38,z1:-98,z2:-96,mesh:null,open:false});
  g.dunWalls.push({x1:-38,x2:-36,z1:-98,z2:14,mesh:null,open:false});
  g.dunWalls.push({x1:36,x2:38,z1:-98,z2:14,mesh:null,open:false});
...
function enterDungeon(g,state,toast,setZone){
  ...
  updateSky(g); if(g.scene.fog) g.scene.fog.density=.018;
...
function updateDungeon(g,dt,state,toast){
  ...
  if(boss){
    ...
    if(g.bossActive){
      ...
      if(boss.slamCD<=0 && !boss.slamming){ boss.slamming=1.0; boss.slamCD=hpFrac<.5?3.5:5.5; g.bossRing.position.set(p.x,.3,p.z); g.bossRing.userData={tx:p.x,tz:p.z}; }
      if(boss.slamming>0){
        boss.slamming-=dt; const tele=1.0-boss.slamming;
        g.bossRing.material.opacity=Math.min(.8,tele); g.bossRing.scale.setScalar(1+tele*3.2);
        if(boss.slamming<=0){
          const rr=4.8; const dd=Math.hypot(p.x-g.bossRing.userData.tx,p.z-g.bossRing.userData.tz);
          if(dd<rr && g.invuln<=0){ const mult=hpFrac<.5?1.7:1.3; damagePlayer(g,state,boss.dmg*mult,toast); toast(hpFrac<.5?"THORNHEART ENRAGES! Root slam!":"Root slam!"); }
      ...
      // Vine Grab — player standing still near boss gets rooted
      boss.standT = g.player.position.distanceTo(boss.lastP||g.player.position.clone())<0.4
        ? (boss.standT||0)+dt : 0;
      boss.lastP=g.player.position.clone();
      if(d<5&&boss.standT>1.8&&!g.rooted&&!boss.slamming){
        g.rooted=2.2; boss.standT=0; toast("Vines erupt — you're rooted!"); SFX.hurt?.();
        damagePlayer(g,state,boss.dmg*.6,toast);
      }
      if(g.rooted>0){ g.rooted-=dt; if(g.rooted<=0) toast("You break free!"); }
      if(hpFrac<.5 && !boss.enraged){ boss.enraged=true; boss.speed=2.2; toast("THORNHEART ENRAGES — roots erupt everywhere!"); for(let i=0;i<3;i++){ const a=i/3*Math.PI*2; spawnEnemy(g,boss.mesh.position.x+Math.cos(a)*5,boss.mesh.position.z+Math.sin(a)*5,"rootling",1,"dungeon"); } }
      // Summon rootlings — from full hp, more frequent below 65%
      const summonThresh=hpFrac<.65?8:14;
      if(boss.summonCD<=0){ boss.summonCD=summonThresh; const rc=g.enemies.filter(e=>e.type==="rootling"&&!e.dead).length;
        const count=hpFrac<.5?3:hpFrac<.65?2:1;
        if(rc<5){ toast(rc===0?"Thornheart calls the grove!":"More roots rise!"); for(let i=0;i<count;i++){ const a=Math.random()*Math.PI*2; spawnEnemy(g,boss.mesh.position.x+Math.cos(a)*5,boss.mesh.position.z+Math.sin(a)*5,"rootling",1,"dungeon"); } }
      }
    }
...
function doInteract(g,toast,setQuestTrack,setZone){
  ...
  g.npcs.forEach(n=>{
    ...
    if(Math.hypot(p.x-n.mesh.position.x,p.z-n.mesh.position.z)<4.5){
      ...
      if(n.kind==="spirit")          g.dlgOpen(spiritTree(g.saveState,g,toast,setQuestTrack),"start");
      ...
      else if(n.kind==="citizen")    g.dlgOpen({start:{speaker:n.name,portrait:"citizen",text:(n.dialogue&&n.dialogue.length)?n.dialogue[Math.floor(Math.random()*n.dialogue.length)]:"Good day to you, Warden."}},"start");
      else if(n.kind==="guard")      g.dlgOpen({start:{speaker:n.name,portrait:"guard",text:(n.dialogue&&n.dialogue[0])||"Keep your blade sheathed in town, Warden. Red Oak's under our watch."}},"start");
        ...
        if(st&&st.s===1){ st.s=2; toast(`Found! Return to ${SIDEQUESTS[n.sq].giver}`); SFX.quest(); g.dlgOpen({start:{speaker:n.name,portrait:"npc",text:n.found||"Oh, thank you for finding me!"}},"start"); }
        else g.dlgOpen({start:{speaker:n.name,portrait:"npc",text:n.idle||"...Hello there."}},"start");
      }
    }
  });
  if(done) return;
  ...
  if(g.zone==="verandah"&&g.hollowEntrance&&Math.hypot(p.x-g.hollowEntrance.x,p.z-g.hollowEntrance.z)<4){ enterHollow(g,g.saveState,toast,setZone); return; }
  if(g.zone==="hollow"&&g.hollowExit&&Math.hypot(p.x-g.hollowExit.x,p.z-g.hollowExit.z)<4){ exitHollow(g,g.saveState,toast,setZone); return; }
  ...
  if(g.zone==="dungeon"){
    ...
    for(const t of (g.tablets||[])){ if(Math.hypot(p.x-t.pos.x,p.z-t.pos.z)<2.6){ g.dlgOpen({start:{speaker:"Lore Tablet — "+t.title,portrait:"tablet",text:t.text}},"start"); return; } }
  }
...
function drawMinimap(g){
  ...
  const p=g.player.position,yaw=g.player.rotation.y;
  const s=Math.sin(yaw),co=Math.cos(yaw);
  // Character-up: forward = player facing direction = TOP of minimap
...
function dungeonSpiritTree(state,g,toast,setQuestTrack){
  return {
    start:{speaker:"Guardian Spirit",portrait:"spirit",text:"You laid Thornheart to rest. It was the last of the old keepers — grown wild with grief, but a Warden still, deep down. You did it mercy.",next:"a"},
    a:{speaker:"Guardian Spirit",portrait:"spirit",text:"The Earth Relic is yours. One of Four. With it, the sealed road to the Ironpeak Mountains opens — the Frost Relic waits in the Frostfang Cavern beyond.",next:"b"},
    b:{speaker:"Guardian Spirit",portrait:"spirit",text:"And look — to the north of the grove, Stonewatch tower stands revealed. The old Warden waystation. Rest there, and carry word of this victory to Captain Aldric in Verandah.",choices:[{label:"I will, Spirit.",action:()=>{ toast("Ironpeak Mountains unlocked · Stonewatch revealed"); }}]},
  };
}
function spiritTree(state,g,toast,setQuestTrack){
  ...
  if(!q.greeted) return {
    start:{speaker:"Guardian Spirit",portrait:"spirit",text:"You live, child of ash. The flames took your home… but not you. That is no accident.",next:"p2"},
    p2:{speaker:"Guardian Spirit",portrait:"spirit",text:"You are the last of the Wardens of Verandah. The Heartstone cracks, and the Hollow King stirs beneath the capital.",next:"p3"},
    p3:{speaker:"Guardian Spirit",portrait:"spirit",text:"Corruption has crept into this grove. Cleanse it — strike down five Hollow-spawn — and I shall reveal your true path.",choices:[{label:"I will do it.",action:greet},{label:"What are the Four Relics?",next:"lore"}]},
    lore:{speaker:"Guardian Spirit",portrait:"spirit",text:"Earth, Frost, Flame, Storm — the Four Relics that power the Heartstone. Scattered when the Hollow King first rose. You must reclaim them all.",choices:[{label:"I accept this burden.",action:greet}]},
  };
  if(q.hasFrostRelic) return {start:{speaker:"Guardian Spirit",portrait:"spirit",text:"Earth and Frost — two Relics, reclaimed. The Hollow King writhes at their light. The Flame and Storm Relics remain. The Emberveil Wastes await those bold enough to carry the heat."}};
  if(q.hasEarthRelic) return {start:{speaker:"Guardian Spirit",portrait:"spirit",text:"The Earth Relic calls to the land beneath my feet. Now seek the Frost Relic — it sleeps in the Frostfang Cavern within the Ironpeak Mountains. Use the travel signpost to reach them."}};
  if(q.completed) return {start:{speaker:"Guardian Spirit",portrait:"spirit",text:()=>"Go east, through the stone archway, into the Ancient Root Temple. The Earth Relic lies guarded by the Thornheart Treant. It will not yield willingly."}};
  if(q.kills<q.need) return {start:{speaker:"Guardian Spirit",portrait:"spirit",text:()=>`${q.need-q.kills} corrupted still defile the grove. End them.`}};
  return {
    start:{speaker:"Guardian Spirit",portrait:"spirit",text:"It is done. You carry yourself like a Warden of old.",onEnter:()=>completeCleanse(state,g,toast,setQuestTrack),next:"after"},
    after:{speaker:"Guardian Spirit",portrait:"spirit",text:"The Ancient Root Temple lies east — beyond the stone archway. Within rests the Earth Relic. Defeat the Thornheart Treant and claim it."},
  };
}
...
function buildVerandah(g){
  ...
  const lm=c=>new THREE.MeshLambertMaterial({color:c,flatShading:true});
  const mk=(geo,c)=>{ const m=new THREE.Mesh(geo,lm(c)); m.castShadow=true; m.receiveShadow=true; grp.add(m); return m; };
  const prop=(x,z,r)=>g.props.push({pos:new THREE.Vector3(x,0,z),radius:r,zone:"verandah"});

  // ── ISLAND TERRAIN ──
  // Surrounding water (dark blue, extends far)
  const sea=new THREE.Mesh(new THREE.CircleGeometry(140,48),new THREE.MeshLambertMaterial({color:0x1a3a5c}));
  sea.rotation.x=-Math.PI/2; sea.position.y=-.4; grp.add(sea);
  // Shallow ring (lighter)
  const shallows=new THREE.Mesh(new THREE.CircleGeometry(72,36),lm(0x2a5a7a));
  shallows.rotation.x=-Math.PI/2; shallows.position.y=-.3; grp.add(shallows);
  // Island ground
  const island=new THREE.Mesh(new THREE.CircleGeometry(64,44),lm(0x8a8870));
  island.rotation.x=-Math.PI/2; island.receiveShadow=true; island.position.y=.01; grp.add(island);
  // Sandy beach ring
  const beach=new THREE.Mesh(new THREE.RingGeometry(55,64,36),lm(0xc8b878));
  beach.rotation.x=-Math.PI/2; beach.position.y=.02; grp.add(beach);
  // Stone plaza
  const plaza=new THREE.Mesh(new THREE.CircleGeometry(20,32),lm(0xb0a890));
  plaza.rotation.x=-Math.PI/2; plaza.position.y=.04; grp.add(plaza);
  // Road network (stone paths)
  [[0,0,40,4,0],[0,0,4,40,0],[0,0,28,4,Math.PI/4]].forEach(([x,z,w,d,ry])=>{
    const rd=new THREE.Mesh(new THREE.BoxGeometry(w,.06,d),lm(0xa09880)); rd.position.set(x,.03,z); rd.rotation.y=ry; grp.add(rd);
  });

  // ── CASTLE DISTRICT (north) ──
  const stone=lm(0x9a958a), stoneDark=lm(0x7a7570), purple=lm(0x5a3a8a);
  // Main keep
  const keep=new THREE.Mesh(new THREE.BoxGeometry(22,13,8),stone); keep.position.set(0,6.5,-38); grp.add(keep);
  // Keep roof
  const keepRoof=new THREE.Mesh(new THREE.BoxGeometry(24,2,10),stoneDark); keepRoof.position.set(0,13,-38); grp.add(keepRoof);
  // Battlements
  for(let i=-10;i<=10;i+=3){ const batt=new THREE.Mesh(new THREE.BoxGeometry(1.5,2,1.5),stone); batt.position.set(i,15,-33.5); grp.add(batt); }
  // Towers
  [-12,12].forEach(tx=>{
    const tw=new THREE.Mesh(new THREE.CylinderGeometry(3.2,3.4,18,10),stone); tw.position.set(tx,9,-38); grp.add(tw);
    const cap=new THREE.Mesh(new THREE.ConeGeometry(4,5,10),purple); cap.position.set(tx,20,-38); grp.add(cap);
    // Tower windows
    for(let y=5;y<17;y+=4){ const win=new THREE.Mesh(new THREE.BoxGeometry(.6,1.2,.5),lm(0x3a3030)); win.position.set(tx,y,-34.7); grp.add(win); }
  });
  // Castle gate + portcullis
  const gate=new THREE.Mesh(new THREE.BoxGeometry(5,7,1.4),stoneDark); gate.position.set(0,3.5,-33.3); grp.add(gate);
  const arch=new THREE.Mesh(new THREE.BoxGeometry(5,.8,1.5),stone); arch.position.set(0,8,-33.2); grp.add(arch);
  const portcullis=new THREE.Mesh(new THREE.BoxGeometry(4.2,.1,5),lm(0x4a4a3a)); portcullis.rotation.x=Math.PI/2; portcullis.position.set(0,5,-33.2); grp.add(portcullis);
  // Flag poles
  [-8,8].forEach(fx=>{ const fp=new THREE.Mesh(new THREE.CylinderGeometry(.1,.1,6,5),lm(0x5a4a30)); fp.position.set(fx,22,-38); grp.add(fp); const flag=new THREE.Mesh(new THREE.BoxGeometry(.1,2.2,3.8),lm(0x6a3a9a)); flag.position.set(fx,22,-36.1); flag.rotation.y=-.1; grp.add(flag); });
  prop(0,0,-38,12);

  // Aldric stands in front of the castle gate
  addNPC(g,"verandah",{name:"Captain Aldric",kind:"questgiver",x:0,z:-25,col:0x4a5a7a});
  // Royal guard patrols
  addNPC(g,"verandah",{name:"Royal Guard",kind:"guard",x:-8,z:-26,col:0x6a7a9a,dialogue:["The capital is secure. Report to Aldric if you have news of the Relics."],wanderC:[-8,-26],wanderR:5});
  addNPC(g,"verandah",{name:"Royal Guard",kind:"guard",x:8,z:-26,col:0x6a7a9a,dialogue:["Verandah stands watch, Warden."],wanderC:[8,-26],wanderR:5});

  // ── MARKET DISTRICT (east) ──
  // Large market hall
  const mHall=new THREE.Mesh(new THREE.BoxGeometry(18,5,12),lm(0xba9a6a)); mHall.position.set(28,2.5,8); grp.add(mHall);
  const mRoof=new THREE.Mesh(new THREE.BoxGeometry(20,1,14),stoneDark); mRoof.position.set(28,5.5,8); grp.add(mRoof);
  for(let x=-8;x<=8;x+=4){ const mc=new THREE.Mesh(new THREE.CylinderGeometry(.5,.6,5,7),stone); mc.position.set(28+x,2.5,8); grp.add(mc); }
  prop(28,8,9);
  // Market stalls
  const stall=(x,z,awCol,label)=>{
    const base=new THREE.Mesh(new THREE.BoxGeometry(3.5,.3,2.5),lm(0x7a5a30)); base.position.set(x,.15,z); grp.add(base);
    const awn=new THREE.Mesh(new THREE.BoxGeometry(4,.2,3),lm(awCol)); awn.position.set(x,2.4,z); awn.rotation.x=-.1; grp.add(awn);
    [[x-1.6,z+.2,1.2],[x+1.6,z+.2,1.2],[x-1.6,z-1.2,1.2],[x+1.6,z-1.2,1.2]].forEach(([px,pz,h])=>{ const p2=new THREE.Mesh(new THREE.CylinderGeometry(.08,.1,h*2,5),lm(0x6a4a20)); p2.position.set(px,h*.9,pz); grp.add(p2); });
    prop(x,z,2);
  };
  stall(18,2,0xaa3333,"Armour"); stall(18,14,0x33aa55,"Herbs"); stall(18,22,0x2244aa,"Books");
  stall(36,2,0xcc7722,"Weapons"); stall(36,14,0x885522,"Furs"); stall(36,22,0x448866,"Potions");
  // Merchants
  addNPC(g,"verandah",{name:"Master Armorer Bren",kind:"merchant",x:18,z:2,col:0x606060,shopName:"Royal Armory",stock:["chainmail","iron_helm","iron_gauntlets","iron_boots","ruby_ring","steel_sword","gilded_plate","rime_plate"]});
  addNPC(g,"verandah",{name:"Court Mage Sela",kind:"merchant",x:36,z:2,col:0x5a3a8a,shopName:"The Arcanum",stock:["ash_staff","mana_potion","scholars_chain","silver_ring","ancient_circlet","warden_robes","stormcaller_staff"]});
  addNPC(g,"verandah",{name:"Herbalist Minna",kind:"merchant",x:18,z:14,col:0x4a7a3a,shopName:"Minna's Remedies",stock:["potion_minor","potion_greater","mana_potion","herb","bread"]});
  addNPC(g,"verandah",{name:"Trader Pell",kind:"merchant",x:36,z:22,col:0x8a6b3a,shopName:"Pell's General Wares",stock:["potion_minor","potion_greater","mana_potion","bread","leather_vest","warden_blade","trail_boots","leather_gloves"]});
  addNPC(g,"verandah",{name:"Fletcher Dorn",kind:"merchant",x:36,z:14,col:0x5a4a28,shopName:"Dorn's Quivers",stock:["hunter_bow","oakwood_bow","leather_helm","iron_boots","jade_ring","bandit_blade"]});

  // ── HARBOR DISTRICT (south) ──
  // Dock planks
  for(let i=0;i<5;i++){
    const dock=new THREE.Mesh(new THREE.BoxGeometry(5,.3,18),lm(0x6a4a28)); dock.position.set(-20+i*8,.1,46); grp.add(dock);
    // Mooring posts
    [-8,8].forEach(dz=>{ const post=new THREE.Mesh(new THREE.CylinderGeometry(.2,.2,2,6),lm(0x5a3a18)); post.position.set(-20+i*8,.8,46+dz); grp.add(post); });
  }
  // Boat (simple low-poly)
  const hull=new THREE.Mesh(new THREE.BoxGeometry(9,2,18),lm(0x5a3a18)); hull.position.set(-10,-.5,50); grp.add(hull);
  const mast=new THREE.Mesh(new THREE.CylinderGeometry(.2,.2,9,6),lm(0x6a4a28)); mast.position.set(-10,5,50); grp.add(mast);
  const sail=new THREE.Mesh(new THREE.BoxGeometry(.2,6,7),lm(0xe8dfc0)); sail.position.set(-10,5.5,50); grp.add(sail);
  const boat2hull=new THREE.Mesh(new THREE.BoxGeometry(7,1.6,14),lm(0x4a2e18)); boat2hull.position.set(14,-.3,52); grp.add(boat2hull);
  // Lighthouse
  const lhBase=new THREE.Mesh(new THREE.CylinderGeometry(2.5,3,12,10),stone); lhBase.position.set(38,6,42); grp.add(lhBase);
  const lhTop=new THREE.Mesh(new THREE.CylinderGeometry(2,2.4,3,10),lm(0xf0d880)); lhTop.position.set(38,13.5,42); grp.add(lhTop);
  const lhCap=new THREE.Mesh(new THREE.ConeGeometry(2.5,3,10),lm(0xcc3322)); lhCap.position.set(38,16.5,42); grp.add(lhCap);
  const lhLight=new THREE.PointLight(0xffee88,3.5,30); lhLight.position.set(38,14.5,42); grp.add(lhLight);
  prop(38,42,3);
  // Harbor NPCs
  addNPC(g,"verandah",{name:"Harbourmaster Thorne",kind:"questgiver",sq:"harbour",x:-10,z:36,col:0x4a5a7a});
  addNPC(g,"verandah",{name:"Sailor Finn",kind:"citizen",x:14,z:44,col:0x5a6a7a,dialogue:["Fair winds, Warden! The sea-lanes are treacherous of late.","Been sailing these waters fifteen years — never seen it this rough."]});
  addNPC(g,"verandah",{name:"Fisher Ola",kind:"citizen",x:-20,z:48,col:0x6a5a40,dialogue:["Catch was thin today. Something's disturbing the waters east of the lighthouse."]});

  // ── TEMPLE DISTRICT (west) ──
  // Temple of the Wardens
  const tempBase=new THREE.Mesh(new THREE.BoxGeometry(22,1,18),stone); tempBase.position.set(-32,.5,-10); grp.add(tempBase);
  const tempBody=new THREE.Mesh(new THREE.BoxGeometry(16,8,14),stone); tempBody.position.set(-32,5,-10); grp.add(tempBody);
  const tempRoof=new THREE.Mesh(new THREE.BoxGeometry(18,3,16),stoneDark); tempRoof.position.set(-32,9.5,-10); grp.add(tempRoof);
  const pediment=new THREE.Mesh(new THREE.CylinderGeometry(.01,9,4,3),stoneDark); pediment.position.set(-32,12.5,-10); pediment.rotation.y=Math.PI/6; grp.add(pediment);
  // Columns
  for(let i=0;i<6;i++){
    const col2=new THREE.Mesh(new THREE.CylinderGeometry(.5,.55,8,8),stone); col2.position.set(-39+i*2.8,4.5,-17); grp.add(col2);
  }
  // Temple interior glow
  const tGlow=new THREE.PointLight(0xffd080,2.5,18); tGlow.position.set(-32,5,-10); grp.add(tGlow);
  prop(-32,-10,11);
  // Temple NPCs
  addNPC(g,"verandah",{name:"High Priestess Vara",kind:"questgiver",sq:"temple_relics",x:-32,z:-18,col:0xd4b896});
  addNPC(g,"verandah",{name:"Temple Acolyte",kind:"citizen",x:-28,z:-14,col:0xc8b090,dialogue:["We pray for the Heartstone daily. Thank the Four Relics for your coming.","Light never truly fades. The Wardens prove it."]});

  // ── RESIDENTIAL DISTRICT (centre-east) ──
  const house=(x,z,w,h,roofC,wallC)=>{
    const b=new THREE.Mesh(new THREE.BoxGeometry(w,h,w*.8),lm(wallC||0xcdb592)); b.position.set(x,h/2,z); grp.add(b);
    const r=new THREE.Mesh(new THREE.ConeGeometry(w*.72,h*.55,4),lm(roofC||0x8c3f2e)); r.position.set(x,h+h*.27,z); r.rotation.y=Math.PI/4; grp.add(r);
    // door + window
    const d=new THREE.Mesh(new THREE.BoxGeometry(.8,1.4,.1),lm(0x5c3a18)); d.position.set(x,h*.35,z+w*.4); grp.add(d);
    const w2=new THREE.Mesh(new THREE.BoxGeometry(.7,.7,.1),lm(0x7a5020)); w2.position.set(x+.9,h*.6,z+w*.4); grp.add(w2);
    prop(x,z,w*.5);
  };
  house(-14,8,6,4,0x8c3f2e,0xcdb592); house(-14,22,5,3.5,0x6a3a2e,0xba9060);
  house(-4,28,5,4,0x7a4f3a,0xc0a870); house(8,28,6,4,0x8c3f2e,0xb09070);
  house(-22,14,5,3,0x7a3a2e,0xba9860); house(4,14,5,3.5,0x6a4a2e,0xd0b080);
  // Citizens
  addNPC(g,"verandah",{name:"Scholar Wen",kind:"questgiver",sq:"scholar",x:-4,z:22,col:0x8a7050});
  addNPC(g,"verandah",{name:"Merchant Osric",kind:"citizen",x:8,z:22,col:0x6a5838,dialogue:["Trade has dried up since the Hollow King's shadow fell over the roads.","If you clear the eastern road of bandits, business picks up for everyone."]});
  addNPC(g,"verandah",{name:"Child Kael",kind:"citizen",x:-8,z:18,col:0xf0c8a0,dialogue:["Can you see the lighthouse from here? Dad says it never goes out.","Are you really a Warden? Mum says they're all gone."]});
  addNPC(g,"verandah",{name:"Elder Nora",kind:"citizen",x:4,z:26,col:0xd0b090,dialogue:["Seen three kings come and go in this city. The Hollow King is the worst shadow yet.","My granddad was a Warden. Carried a blade like yours."]});
  addNPC(g,"verandah",{name:"City Guard",kind:"guard",x:-16,z:0,col:0x6a7a9a,dialogue:["Keep to the main roads, Warden."],wanderC:[-16,0],wanderR:10});
  addNPC(g,"verandah",{name:"City Guard",kind:"guard",x:20,z:-10,col:0x6a7a9a,dialogue:["The capital is secure."],wanderC:[20,-10],wanderR:10});

  // ── CENTRAL FOUNTAIN + PLAZA ──
  const fount=new THREE.Mesh(new THREE.CylinderGeometry(3.2,3.8,1.2,16),stone); fount.position.set(0,.6,0); grp.add(fount);
  const foutInner=new THREE.Mesh(new THREE.CylinderGeometry(2.2,2.2,1,14),lm(0x1a3a5c)); foutInner.position.set(0,.8,0); grp.add(foutInner);
  const foutStat=new THREE.Mesh(new THREE.CylinderGeometry(.3,.4,3,8),stone); foutStat.position.set(0,2.4,0); grp.add(foutStat);
  const foutOrb=new THREE.Mesh(new THREE.SphereGeometry(.6,10,10),lm(0xd4c088)); foutOrb.position.set(0,4,0); grp.add(foutOrb);
  const foutLight=new THREE.PointLight(0x88aaff,1.8,12); foutLight.position.set(0,3,0); grp.add(foutLight);
  prop(0,0,3.5);
  // Statues around the plaza
  for(let i=0;i<4;i++){
    const a=i/4*Math.PI*2+Math.PI/4, sd=16;
    const sBase=new THREE.Mesh(new THREE.BoxGeometry(1.4,1,1.4),stone); sBase.position.set(Math.cos(a)*sd,.5,Math.sin(a)*sd); grp.add(sBase);
    const sFig=new THREE.Mesh(new THREE.CylinderGeometry(.3,.4,2.2,8),stoneDark); sFig.position.set(Math.cos(a)*sd,2.1,Math.sin(a)*sd); grp.add(sFig);
    const sHead=new THREE.Mesh(new THREE.SphereGeometry(.3,7,7),stoneDark); sHead.position.set(Math.cos(a)*sd,3.4,Math.sin(a)*sd); grp.add(sHead);
    prop(Math.cos(a)*sd,Math.sin(a)*sd,1.0);
  }
  // Bench rings
  for(let i=0;i<6;i++){
    const a=i/6*Math.PI*2, bd=9;
    const bench=new THREE.Mesh(new THREE.BoxGeometry(2.4,.3,1),lm(0x8a6a3a)); bench.position.set(Math.cos(a)*bd,.3,Math.sin(a)*bd); bench.rotation.y=a; grp.add(bench);
    const bl=new THREE.Mesh(new THREE.BoxGeometry(.2,.5,.9),lm(0x7a5a2a)); bl.position.set(Math.cos(a)*bd-.9,.55,Math.sin(a)*bd); bench.rotation.y=a; grp.add(bl);
    const br=new THREE.Mesh(new THREE.BoxGeometry(.2,.5,.9),lm(0x7a5a2a)); br.position.set(Math.cos(a)*bd+.9,.55,Math.sin(a)*bd); grp.add(br);
  }
  // Travel signpost
  addSign(g,"verandah",0,18);
  // ── HEARTSTONE PORTAL in castle courtyard ──
  const hkPortalRing=new THREE.Mesh(new THREE.TorusGeometry(2,.28,10,28),new THREE.MeshBasicMaterial({color:0x9944ff,transparent:true,opacity:.7})); hkPortalRing.position.set(0,2,-22); grp.add(hkPortalRing);
  const hkPortalFill=new THREE.Mesh(new THREE.CircleGeometry(1.8,24),new THREE.MeshBasicMaterial({color:0x440088,transparent:true,opacity:.45,side:THREE.DoubleSide})); hkPortalFill.position.set(0,2,-22); grp.add(hkPortalFill);
  const hkPortalLight=new THREE.PointLight(0x9944ff,1.2,8); hkPortalLight.position.set(0,2.5,-22); grp.add(hkPortalLight);
  const hkPortalBase=new THREE.Mesh(new THREE.CylinderGeometry(.5,.7,.4,8),new THREE.MeshLambertMaterial({color:0x2a1840,flatShading:true})); hkPortalBase.position.set(0,.2,-22); grp.add(hkPortalBase);
  g.hollowEntrance=new THREE.Vector3(0,0,-22);
  // Wandering citizens
  [[-5,6],[5,6],[-10,2],[10,-2],[0,10]].forEach(([x,z],i)=>{
    const names=["Citizen Orla","Citizen Bram","Citizen Ysa","Citizen Holt","Apprentice Tiv"];
    const convos=[["A fine day in Verandah.","The Heartstone's light is fading — I've felt it."],["Strange omens lately.","Glad the Wardens are back."],["Keep the Hollow away from my shop!","Safer here than in the villages."]];
    addNPC(g,"verandah",{name:names[i],kind:"citizen",x,z,col:[0xb09060,0x7a6040,0xa08050,0x906848,0xc0a878][i],dialogue:convos[i%convos.length],wanderC:[x,z],wanderR:8});
  });

  // ── BRIDGE FROM MAINLAND ──
  for(let i=0;i<6;i++){
    const bs=new THREE.Mesh(new THREE.BoxGeometry(6,.6,4),stone); bs.position.set(0,.3,24+i*5); grp.add(bs);
    [-2.8,2.8].forEach(bx2=>{ const rail=new THREE.Mesh(new THREE.BoxGeometry(.3,1,4),stoneDark); rail.position.set(bx2,.9,24+i*5); grp.add(rail); });
  }
  // Bridge approach torches
  [-3,3].forEach(tx=>{ const tp=new THREE.Mesh(new THREE.CylinderGeometry(.1,.12,2,6),lm(0x6a4a20)); tp.position.set(tx,1.8,22); grp.add(tp); const tf=new THREE.Mesh(new THREE.SphereGeometry(.16,6,6),new THREE.MeshBasicMaterial({color:0xff7020})); tf.position.set(tx,2.9,22); grp.add(tf); const tpl=new THREE.PointLight(0xff7020,1.6,8); tpl.position.set(tx,3,22); grp.add(tpl); });
  // Street lamps
  for(let i=0;i<8;i++){
    const a=i/8*Math.PI*2, ld=38;
    const lp=new THREE.Mesh(new THREE.CylinderGeometry(.08,.1,3.5,6),lm(0x5a4a28)); lp.position.set(Math.cos(a)*ld,1.75,Math.sin(a)*ld); grp.add(lp);
    const ll=new THREE.PointLight(0xffcc66,1.2,10); ll.position.set(Math.cos(a)*ld,3.8,Math.sin(a)*ld); grp.add(ll);
    const lb=new THREE.Mesh(new THREE.BoxGeometry(.4,.5,.4),lm(0xc8a020)); lb.position.set(Math.cos(a)*ld,3.5,Math.sin(a)*ld); grp.add(lb);
  }
}

function switchZone(g, zone, pos, toast, setZone){
  ["overworld","dungeon","redoak","verandah","ironpeak","frost","ember","scorch","storm","stormspire","hollow"].forEach(z=>{ const grp=g[z+"Group"]; if(grp) grp.visible=(z===zone); });
  ...
  updateSky(g); if(g.scene.fog) g.scene.fog.density = (zone==="dungeon")?.018:(zone==="ironpeak")?.02:(zone==="frost")?.05:.012;
  g.paused=false; if(g.dlgState) g.dlgClose();
  toast(zone==="redoak"?"Red Oak Town":zone==="verandah"?"Verandah, the Capital":zone==="ironpeak"?"The Ironpeak Mountains":zone==="ember"?"The Emberveil Wastes":zone==="overworld"?"Red Oak Grove":zone==="hollow"?"The Heartstone Chamber":"");
  MUSIC.play(MUSIC.zone(zone), 2);
}
...
const SIDEQUESTS={
  ...
  wolves:{ name:"Wolf Hunt", giver:"Hunter Bryn", reward:{gold:115,xp:110}, count:4, portrait:"hunter",
  ...
  child:{ name:"The Missing Child", giver:"Goodwife Mirna", reward:{gold:140,xp:130}, portrait:"citizen",
  ...
  harbour:{ name:"Dockside Threat", giver:"Harbourmaster Thorne", reward:{gold:280,xp:220}, portrait:"captain",
    offer:"Corsairs have been raiding the eastern shipping lanes. Clear five bandits from the southern bandit camp — a show of force may deter them.",
    active:(p)=>`Bandits cleared: ${p}/5. Find them south of the river, past the bridge.`,
    ready:"That's the kind of dispatch that keeps the lanes open. Well done, Warden.",
    done:"The docks have been quiet since. My thanks — and the coin." },
  temple_relics:{ name:"Light the Flame", giver:"High Priestess Vara", reward:{gold:320,xp:260}, portrait:"elder",
    offer:"For each Relic you recover, return here and we will light a votive candle in the old Warden's tradition. Bring me word when you hold your first Relic.",
    active:()=>"Return to High Priestess Vara when you have recovered a Relic.",
    ready:"A Relic in your hands. The candle burns again. May its light guide you to the rest.",
    done:"Four candles. Four relics. The Wardens are reborn in your deeds." },
  scholar:{ name:"The Hidden Archive", giver:"Scholar Wen", reward:{gold:260,xp:200}, portrait:"npc",
    offer:"There is a sealed chamber beneath the castle — the old Warden's Archive. Bring me any lore tablet you find in your dungeon travels and I will translate it.",
    active:()=>"Find a lore tablet in the Ancient Root Temple and return it to Scholar Wen.",
    ready:"Remarkable — this confirms the Hollow King's first rising was in the Age of Three Moons. A crucial piece. Thank you.",
    done:"I've filled three volumes. You've made history, Warden." },
};
...
function questTree(name, state, g, toast, setQuestTrack, setZone){
  ...
  if(name==="Town Elder Maro"){
    if(s.stage===0) return {
      start:{speaker:"Town Elder Maro",portrait:"elder",text:"So — you're the one the Spirit spoke of. A Warden's blood, here in Red Oak. These are dark days, child.",next:"a"},
      a:{speaker:"Town Elder Maro",portrait:"elder",text:"The King has summoned every able hand to Verandah. Go to the capital, seek Captain Aldric at the castle, and tell him Maro of Red Oak sent you.",choices:[{label:"I'll go at once.",action:()=>{ s.stage=1; toast("Main Quest: The King's Summons"); SFX.quest(); }}]},
    };
    if(s.stage>=1&&s.stage<5) return {start:{speaker:"Town Elder Maro",portrait:"elder",text:"Verandah lies down the road — use the signpost and choose the capital. Captain Aldric is expecting a Warden."}};
    return {start:{speaker:"Town Elder Maro",portrait:"elder",text:"A true Warden walks among us again. Red Oak is honored, and a little safer for it."}};
  }
  if(name==="Windward Sage"){
    return {
      start:{speaker:"Windward Sage",portrait:"sage",text:"You feel it too, Warden — the sky's unrest? The last Relic sleeps atop the Stormspire, where the wind never stills.",next:"a"},
      a:{speaker:"Windward Sage",portrait:"sage",text:"The Stormcaller guards it, herald of the tempest, old as the first thunder. Ascend the spire to the north. Gather all Four Relics, and the Heartstone may yet wake.",choices:[{label:"I am ready."},{label:"Farewell."}]},
    };
  }
  // Captain Aldric
  if(s.stage<1) return {start:{speaker:"Captain Aldric",portrait:"captain",text:"I've no time for wanderers off the road. If Elder Maro of Red Oak vouches for you, come back with his word."}};
  if(s.stage===1) return {
    start:{speaker:"Captain Aldric",portrait:"captain",text:"So. A Warden, Maro says. The Heartstone cracks and the Hollow King stirs in his prison. Words are cheap — prove the blood in your veins.",next:"a"},
    a:{speaker:"Captain Aldric",portrait:"captain",text:"Recover the Earth Relic from the Ancient Root Temple, east of the grove. Bring it to me and I'll believe you — and Verandah will stand with you.",choices:[{label:"It will be done.",action:()=>{ s.stage=2; toast("Quest: Recover the Earth Relic"); SFX.quest(); }}]},
  };
  if(s.stage>=2&&s.stage<5){
    if(!state.quest.hasEarthRelic) return {start:{speaker:"Captain Aldric",portrait:"captain",text:"You return empty-handed. The Earth Relic lies in the Ancient Root Temple, guarded by the Thornheart Treant. Do not return without it."}};
    return {
      start:{speaker:"Captain Aldric",portrait:"captain",text:"By the Four… you truly hold it. The Earth Relic. Its light — I had forgotten such a thing could shine.",onEnter:()=>{ if(s.stage<5){ s.stage=5; state.gold+=300; state.inv.warden_robes=(state.inv.warden_robes||0)+1; gainXP(state,300,toast); toast("Main Quest Complete! +300◈ & Warden Robes"); SFX.levelup(); updateHUD(g,g.setUi); } },next:"a"},
      a:{speaker:"Captain Aldric",portrait:"captain",text:"I name you Warden of the Realm. The mountain roads are open to you now — the signpost will carry you to Ironpeak. Speak with me again when you are ready for the next relic.",choices:[{label:"What lies in the mountains?",action:()=>{}}]},
    };
  }
  // Frost Relic questline
  if(s.stage===5) return {
    start:{speaker:"Captain Aldric",portrait:"captain",text:"The second relic — Frost — sleeps beneath the Ironpeak Mountains, in the Frostfang Cavern. A Frost Titan was set to guard it long ago, and guards it still.",next:"a"},
    a:{speaker:"Captain Aldric",portrait:"captain",text:"Take the signpost to Ironpeak. Find the cavern mouth in the snow, and bring back the Frost Relic. Wrap up warm, Warden.",choices:[{label:"I'll bring it back.",action:()=>{ s.stage=6; toast("Main Quest: The Frost Relic"); SFX.quest(); }}]},
  };
  if(s.stage===8 && !q.hasFlameRelic) return {
    start:{speaker:"Captain Aldric",portrait:"captain",text:"Two Relics reclaimed, Warden. The Emberveil Wastes burn to the south — use the travel signpost.",next:"a"},
    a:{speaker:"Captain Aldric",portrait:"captain",text:"Deep in the Scorchdeep Cavern waits the Flame Colossus. Ancient as the mountain it lives in. The Flame Relic is yours to take.",choices:[{label:"I'll bring it back.",action:()=>{ s.stage=9; toast("Quest: The Flame Relic 🔥"); SFX.quest(); }}]},
  };
  if(s.stage===9 && !q.hasFlameRelic) return {start:{speaker:"Captain Aldric",portrait:"captain",text:"The Emberveil Wastes lie to the south. Enter the Scorchdeep Cavern and slay the Flame Colossus."}};
  if(s.stage>=9 && q.hasFlameRelic) return {
    start:{speaker:"Captain Aldric",portrait:"captain",text:"Two relics, now. You felled the Titan? The cold of that thing could halt a heart — and you carry its relic like a lantern.",onEnter:()=>{ if(s.stage<10){ s.stage=10; state.gold+=500; state.inv.wardens_seal=(state.inv.wardens_seal||0)+1; gainXP(state,500,toast); toast("Flame Relic delivered! +500◈ & Warden's Seal"); SFX.levelup(); updateHUD(g,g.setUi); } },next:"a"},
    a:{speaker:"Captain Aldric",portrait:"captain",text:"Earth, Frost, Flame — three Relics. One remains. The Storm Relic is lost to the high peaks. When the mountain roads clear, Verandah rides with you."},
  };
  if(q.hollowKingDefeated) return {start:{speaker:"Captain Aldric",portrait:"captain",text:"The Hollow King is dust. The Heartstone blazes. I have served this realm forty years and never thought I'd see this day. Verandah owes you everything, Warden."}};
  if(q.hasStormRelic) return {
    start:{speaker:"Captain Aldric",portrait:"captain",text:"Four Relics. All of them. And the Heartstone stirs beneath our feet… I can feel it from here.",next:"a"},
    a:{speaker:"Captain Aldric",portrait:"captain",text:"A void portal has opened in the castle courtyard — the Heartstone Chamber lies beneath. The Hollow King will be there. End him, Warden. End it now.",choices:[{label:"I'll face him.",action:()=>{ if(s.stage<20){s.stage=20;toast("Final Quest: Enter the Heartstone Chamber");SFX.quest();} }}]},
  };
    if(s.stage===6){
    if(!state.quest.hasFrostRelic) return {start:{speaker:"Captain Aldric",portrait:"captain",text:"Ironpeak is to the north — the signpost will take you. The Frostfang Cavern holds the Frost Relic, and the Titan that hoards it."}};
    return {
      ...
      a:{speaker:"Captain Aldric",portrait:"captain",text:"Two of the Four. Flame and Storm remain, in lands not yet safe to travel. Rest, gather your strength — the realm owes you more than it can pay. (More chapters to come.)"},
    };
  }
  return {start:{speaker:"Captain Aldric",portrait:"captain",text:"Stand ready, Warden. Two relics are ours; the Hollow King fears every one we reclaim."}};
}
  ...
  const Row=({k,onClick,price,col,showCmp})=>{ const it=ITEMS[k]; const c=showCmp?cmp(k):null; return (
    <div style={{padding:"5px 2px",borderBottom:"1px dashed rgba(91,67,38,.2)",fontSize:12}}>
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        <img src={drawItemIcon(it.icon,36)} width={36} height={36} style={{borderRadius:4}}/>
  ...
  return <>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      <div>
        ...
        {stock.filter(k=>ITEMS[k]).map(k=><Row key={k} k={k} onClick={buy} price={ITEMS[k].price} col={state.gold>=ITEMS[k].price?"#3f7d54":"#9a9a9a"} showCmp={true}/>)}
      </div>
      <div>
        ...
        {sellable.filter(k=>ITEMS[k]).map(k=><Row key={k} k={k} onClick={sell} price={Math.max(1,Math.floor(ITEMS[k].price*.45))} col="#8a5a2a"/>)}
      </div>
    </div>
  </>;
}
...
function buildIronpeak(g){
  ...
  // ironpeak enemies spawned via spawnZoneEnemies after spawnEnemyWave
...
function buildFrostCavern(g){
  ...
  // enemies + boss
  // frost enemies spawned via spawnZoneEnemies after spawnEnemyWave
...
function tHEmber(x,z){
  // Volcanic plain — flat ash surface so entities rest on the visible ground
  return 0;
}
...
function buildEmber(g){
  ...
  // Enemies
  // ember enemies spawned via spawnZoneEnemies after spawnEnemyWave
...
function buildScorchdeep(g){
  ...
  // Enemies + boss
  // scorch enemies spawned via spawnZoneEnemies after spawnEnemyWave
...
// All non-overworld enemies — spawned AFTER spawnEnemyWave so they're not wiped
function spawnZoneEnemies(g){
  // Ironpeak
  ...
  spawnEnemy(g,0,-22,"colossus",1,"scorch");
  // Storm + Stormspire (previously in spawnStormEnemies)
...
function buildHollow(g){
  const grp=g.hollowGroup;
  const lm=c=>new THREE.MeshLambertMaterial({color:c,flatShading:true});
  const mk=(geo,c)=>{ const m=new THREE.Mesh(geo,lm(c)); m.castShadow=true; grp.add(m); return m; };

  // Floor — ancient rune-carved stone, circular
  const floor=new THREE.Mesh(new THREE.CircleGeometry(26,40),lm(0x1a1620)); floor.rotation.x=-Math.PI/2; floor.position.y=.01; floor.receiveShadow=true; grp.add(floor);
  // Inner ritual circle
  const inner=new THREE.Mesh(new THREE.RingGeometry(7,9,36),lm(0x2a1840)); inner.rotation.x=-Math.PI/2; inner.position.y=.02; grp.add(inner);
  const runes=new THREE.Mesh(new THREE.RingGeometry(12,14,36),lm(0x3a1050)); runes.rotation.x=-Math.PI/2; runes.position.y=.02; grp.add(runes);
  // Rune glow lines (emanating from center)
  for(let i=0;i<8;i++){ const a=i/8*Math.PI*2; const rl=new THREE.Mesh(new THREE.BoxGeometry(.08,0.04,14),new THREE.MeshBasicMaterial({color:0x660088,transparent:true,opacity:.5})); rl.rotation.y=a; rl.position.set(0,.03,0); grp.add(rl); }

  // Outer walls (implied darkness — tall dark pillars, no ceiling)
  for(let i=0;i<12;i++){
    const a=i/12*Math.PI*2, wr=24;
    const pillar=mk(new THREE.BoxGeometry(1.6,12,1.6),0x12101a); pillar.position.set(Math.cos(a)*wr,6,Math.sin(a)*wr);
    const cap=mk(new THREE.ConeGeometry(1.1,.8,4),0x1a1626); cap.position.set(Math.cos(a)*wr,12.5,Math.sin(a)*wr);
    // glowing trim
    const trim=new THREE.Mesh(new THREE.BoxGeometry(1.8,.15,1.8),new THREE.MeshBasicMaterial({color:0x660088,transparent:true,opacity:.5})); trim.position.set(Math.cos(a)*wr,2,Math.sin(a)*wr); grp.add(trim);
  }
  // Ruined warden statues (four directions)
  for(let i=0;i<4;i++){
    const a=i/4*Math.PI*2+Math.PI/4, sr=18;
    const sb=mk(new THREE.BoxGeometry(1.4,1,1.4),0x1e1a28); sb.position.set(Math.cos(a)*sr,.5,Math.sin(a)*sr);
    const sf=mk(new THREE.CylinderGeometry(.32,.38,2.4,8),0x1a1826); sf.position.set(Math.cos(a)*sr,2.2,Math.sin(a)*sr);
    const sh=mk(new THREE.SphereGeometry(.32,7,7),0x1a1826); sh.position.set(Math.cos(a)*sr,3.6,Math.sin(a)*sr);
    // cracked seam
    const sc2=new THREE.Mesh(new THREE.BoxGeometry(.06,2.2,.04),new THREE.MeshBasicMaterial({color:0x9944ff,transparent:true,opacity:.6})); sc2.position.set(Math.cos(a)*sr,2.2,Math.sin(a)*sr); grp.add(sc2);
  }

  // Four Relic pedestals (cardinal directions, radius 12)
  const relicCols=[0xcc4422,0x4499cc,0xff6600,0x9944ff]; // earth/frost/flame/storm
  const relicNames=["Earth","Frost","Flame","Storm"];
  g.hollowPedestals=[];
  [[0,-12],[12,0],[0,12],[-12,0]].forEach(([x,z],i)=>{
    const pb=mk(new THREE.CylinderGeometry(.6,.8,1.2,8),0x1e1a28); pb.position.set(x,.6,z);
    const pc=mk(new THREE.CylinderGeometry(.35,.4,.4,8),0x2a2035); pc.position.set(x,1.4,z);
    // Relic orb (glows when you bring the relic)
    const orb=new THREE.Mesh(new THREE.SphereGeometry(.28,10,10),new THREE.MeshBasicMaterial({color:0x331133,transparent:true,opacity:.5})); orb.position.set(x,1.9,z); grp.add(orb);
    const pl=new THREE.PointLight(relicCols[i],0,4); pl.position.set(x,2.2,z); grp.add(pl);
    g.hollowPedestals.push({x,z,light:pl,orb,col:relicCols[i],name:relicNames[i],lit:false});
  });

  // Heartstone at the center — cracked dark crystal on a pillar
  const hpillar=mk(new THREE.CylinderGeometry(.5,.7,3,8),0x1a1626); hpillar.position.y=1.5;
  const hcrystal=new THREE.Mesh(new THREE.OctahedronGeometry(1.0,0),new THREE.MeshLambertMaterial({color:0x2a1040,emissive:0x110022,emissiveIntensity:.5,transparent:true,opacity:.9})); hcrystal.position.y=3.6; grp.add(hcrystal);
  // Crack lines on crystal
  for(let i=0;i<5;i++){ const cr=new THREE.Mesh(new THREE.BoxGeometry(.06,.8,.06),new THREE.MeshBasicMaterial({color:0x440066,transparent:true,opacity:.6})); cr.position.set((Math.random()-.5)*.8,3.4+Math.random()*.4,(Math.random()-.5)*.8); cr.rotation.set(Math.random(),Math.random(),Math.random()); grp.add(cr); }
  const hlight=new THREE.PointLight(0x9944ff,.8,12); hlight.position.y=4.2; grp.add(hlight);
  g.heartstoneCrystal=hcrystal; g.heartstoneLight=hlight;

  // Boss dais (north, z=-14)
  const daisBase=mk(new THREE.CylinderGeometry(4,4.5,.5,10),0x1a1626); daisBase.position.set(0,.25,-14);
  const daisTop=mk(new THREE.CylinderGeometry(3.5,4,.3,10),0x22183a); daisTop.position.set(0,.55,-14);
  // Throne chair
  const tseat=mk(new THREE.BoxGeometry(2.6,.5,2),0x110e1a); tseat.position.set(0,1,-17);
  const tback=mk(new THREE.BoxGeometry(2.6,4,.4),0x110e1a); tback.position.set(0,3,-18);
  [-1,1].forEach(sx=>{ const ta=mk(new THREE.BoxGeometry(.4,.4,2.2),0x110e1a); ta.position.set(sx*1.3,1.7,-17); const ts=mk(new THREE.ConeGeometry(.25,.7,4),0x0e0c18); ts.position.set(sx*1.3,2.2,-17); });
  // Throne spikes
  for(let i=0;i<5;i++){ const ts=mk(new THREE.ConeGeometry(.2,.8,4),0x0e0c18); ts.position.set((-2+i)*1.0*.62,5.2,-18); grp.add(ts); }

  // Entry portal (south, z=16)
  const portalRing=new THREE.Mesh(new THREE.TorusGeometry(2.4,.3,10,28),new THREE.MeshBasicMaterial({color:0x9944ff})); portalRing.position.set(0,2.4,16); grp.add(portalRing);
  const portalFill=new THREE.Mesh(new THREE.CircleGeometry(2.2,24),new THREE.MeshBasicMaterial({color:0x6600cc,transparent:true,opacity:.55,side:THREE.DoubleSide})); portalFill.rotation.x=Math.PI/2; portalFill.position.set(0,.05,16); grp.add(portalFill);
  const portalFillV=new THREE.Mesh(new THREE.CircleGeometry(2.2,24),new THREE.MeshBasicMaterial({color:0x6600cc,transparent:true,opacity:.55,side:THREE.DoubleSide})); portalFillV.position.set(0,2.4,16); grp.add(portalFillV);
  const portalLight=new THREE.PointLight(0x9944ff,2.5,8); portalLight.position.set(0,2.4,16); grp.add(portalLight);
  g.hollowExit=new THREE.Vector3(0,0,16);

  // Shockwave pool
  g.hollowShockwaves=[];
}

function enterHollow(g,state,toast,setZone){
  const q=state.quest;
  if(!q.hasEarthRelic||!q.hasFrostRelic||!q.hasFlameRelic||!q.hasStormRelic){
    toast("You must hold all Four Relics to enter."); return;
  }
  switchZone(g,"hollow",{x:0,z:12},toast,setZone);
  // Light the relic pedestals
  const flags=[q.hasEarthRelic,q.hasFrostRelic,q.hasFlameRelic,q.hasStormRelic];
  if(g.hollowPedestals) g.hollowPedestals.forEach((p,i)=>{ if(flags[i]&&!p.lit){ p.lit=true; p.light.intensity=2.0; p.orb.material.color.setHex(p.col); p.orb.material.opacity=.9; } });
  // Spawn the Hollow King if not already present
  if(!g.hollowKingSpawned&&!q.hollowKingDefeated){ g.hollowKingSpawned=true; spawnEnemy(g,0,-14,"hollowking",1,"hollow"); toast("◈ The Hollow King rises from his throne…"); SFX.levelup(); }
  else if(q.hollowKingDefeated){ toast("The chamber is at peace. The Heartstone shines."); }
}

function exitHollow(g,state,toast,setZone){
  switchZone(g,"verandah",{x:0,z:-16},toast,setZone);
}

// ── FROST TITAN — Glacier Charge + Blizzard Aura + Ice Shards ──
function updateBossFrost(g,dt,state,toast){
  const p=g.player.position;
  const boss=g.enemies.find(e=>e.type==="titan"&&!e.dead); if(!boss) { g.setBoss?.(null); return; }
  const dist=Math.hypot(p.x-boss.mesh.position.x,p.z-boss.mesh.position.z);
  if(!g.fBossActive&&dist<22){ g.fBossActive=true; toast("❄ The Frost Titan stirs!"); }
  if(!g.fBossActive) return;
  const hpFrac=boss.hp/boss.maxhp;
  g.fBossT=(g.fBossT||0)+dt; if(g.fBossT>.15){ g.fBossT=0; g.setBoss?.({name:"Frost Titan",hp:Math.max(0,boss.hp),max:boss.maxhp}); }
  // Blizzard aura — slows player when close
  if(dist<5){ g.frostSlow=0.45; } else { g.frostSlow=Math.max(0,(g.frostSlow||0)-dt*.5); }
  // Glacier Charge — telegraphs, then dashes
  boss.chargeCd=(boss.chargeCd||12)-dt;
  if(boss.chargeCd<=0&&!boss.charging){ boss.charging={t:1.8,tx:p.x,tz:p.z}; boss.chargeCd=hpFrac<.5?8:14; toast("⚠ Frost Titan CHARGES — dodge!"); }
  if(boss.charging){
    boss.charging.t-=dt;
    if(boss.charging.t<=0){ // dash
      const dx=boss.charging.tx-boss.mesh.position.x,dz=boss.charging.tz-boss.mesh.position.z,dd=Math.hypot(dx,dz);
      boss.mesh.position.x+=dx; boss.mesh.position.z+=dz;
      if(Math.hypot(p.x-boss.charging.tx,p.z-boss.charging.tz)<2.5&&g.invuln<=0){ damagePlayer(g,state,boss.dmg*1.8,toast); toast("Crushed by the Glacier Charge!"); }
      boss.charging=null;
    }
  }
  // Ice Shards (3 projectiles) at low HP
  if(hpFrac<.6){ boss.shardCd=(boss.shardCd||6)-dt; if(boss.shardCd<=0){ boss.shardCd=6; for(let i=-1;i<=1;i++) fireProjectile(g,state,"arrow",.8,i*.35); toast("Ice Shards!"); } }
  // Phase 2 at 40%
  if(hpFrac<.4&&!boss.phase2){ boss.phase2=true; boss.speed=2.2; toast("❄ TITAN BLIZZARD — the cavern freezes!"); }
}

// ── FLAME COLOSSUS — Magma Stamps + Fire Charge + Ember Shower ──
function updateBossScorch(g,dt,state,toast){
  const p=g.player.position;
  const boss=g.enemies.find(e=>e.type==="colossus"&&!e.dead); if(!boss){ g.setBoss?.(null); return; }
  const dist=Math.hypot(p.x-boss.mesh.position.x,p.z-boss.mesh.position.z);
  if(!g.sBossActive&&dist<24){ g.sBossActive=true; toast("🔥 The Flame Colossus erupts from the magma!"); }
  if(!g.sBossActive) return;
  const hpFrac=boss.hp/boss.maxhp;
  g.sBossT=(g.sBossT||0)+dt; if(g.sBossT>.15){ g.sBossT=0; g.setBoss?.({name:"Flame Colossus",hp:Math.max(0,boss.hp),max:boss.maxhp}); }
  if(!g.magmaZones) g.magmaZones=[];
  // Magma Stamp — leaves burning ground zone
  boss.stampCd=(boss.stampCd||8)-dt;
  if(boss.stampCd<=0){ boss.stampCd=hpFrac<.5?5:9;
    const mx=boss.mesh.position.x,mz=boss.mesh.position.z;
    const ring=new THREE.Mesh(new THREE.CircleGeometry(3,16),new THREE.MeshBasicMaterial({color:0xff4400,transparent:true,opacity:.55,side:THREE.DoubleSide}));
    ring.rotation.x=-Math.PI/2; ring.position.set(mx,.12,mz); g.scorchGroup.add(ring);
    g.magmaZones.push({x:mx,z:mz,r:3,life:12,ring}); toast("Magma Stamp!");
  }
  // Check magma zone damage + tick down life
  g.magmaZones=g.magmaZones.filter(mz=>{ mz.life-=dt; mz.ring.material.opacity=0.3*(mz.life/12); if(mz.life<=0){mz.ring.parent?.remove(mz.ring);return false;}
    if(Math.hypot(p.x-mz.x,p.z-mz.z)<mz.r&&g.invuln<=0&&Math.random()<.06){ damagePlayer(g,state,boss.dmg*.55,toast); toast("Burned by magma!"); } return true; });
  // Fire Charge at 50%
  if(hpFrac<.5&&!boss.phase2){ boss.phase2=true; boss.speed=2.0; toast("🔥 COLOSSUS ENRAGES!"); }
  boss.chargeCd=(boss.chargeCd||10)-dt;
  if(boss.chargeCd<=0&&hpFrac<.7&&!boss.charging){ boss.charging={t:1.2,tx:p.x,tz:p.z}; boss.chargeCd=10; toast("⚠ Fire Charge — move!"); }
  if(boss.charging){ boss.charging.t-=dt; if(boss.charging.t<=0){ const dx=boss.charging.tx-boss.mesh.position.x,dz=boss.charging.tz-boss.mesh.position.z; boss.mesh.position.x+=dx*.8; boss.mesh.position.z+=dz*.8; if(Math.hypot(p.x-boss.charging.tx,p.z-boss.charging.tz)<3&&g.invuln<=0) damagePlayer(g,state,boss.dmg*2,toast); boss.charging=null; } }
}

// ── STORMCALLER — Lightning Lance + Storm Step + Tempest Phase ──
function updateBossStorm(g,dt,state,toast){
  const p=g.player.position;
  const boss=g.enemies.find(e=>e.type==="stormcaller"&&!e.dead); if(!boss){ g.setBoss?.(null); return; }
  const dist=Math.hypot(p.x-boss.mesh.position.x,p.z-boss.mesh.position.z);
  if(!g.stBossActive&&dist<20){ g.stBossActive=true; toast("⚡ The Stormcaller summons the tempest!"); }
  if(!g.stBossActive) return;
  const hpFrac=boss.hp/boss.maxhp;
  g.scUiT=(g.scUiT||0)+dt; if(g.scUiT>.12){ g.scUiT=0; g.scBarOn=true; g.setBoss?.({name:"Stormcaller, Herald of the Tempest",hp:Math.max(0,boss.hp),max:boss.maxhp}); }
  // Lightning Lance — fires a bolt toward player (dodge sideways)
  boss.lanceCd=(boss.lanceCd||5)-dt;
  if(boss.lanceCd<=0){ boss.lanceCd=hpFrac<.4?3:6;
    // Telegraph
    boss.lanceTele={t:1.0,tx:p.x,tz:p.z}; toast("⚠ Lightning Lance — dodge!"); }
  if(boss.lanceTele){ boss.lanceTele.t-=dt; if(boss.lanceTele.t<=0){
    const d2=Math.hypot(p.x-boss.lanceTele.tx,p.z-boss.lanceTele.tz); if(d2<1.8&&g.invuln<=0) damagePlayer(g,state,boss.dmg*1.6,toast);
    spawnSparks(g,boss.lanceTele.tx,1,boss.lanceTele.tz,true); boss.lanceTele=null; } }
  // Storm Step — teleport to random position in arena
  boss.stepCd=(boss.stepCd||10)-dt;
  if(boss.stepCd<=0&&!boss.lanceTele){ boss.stepCd=hpFrac<.4?7:13;
    const a=Math.random()*Math.PI*2, r=8+Math.random()*6;
    boss.mesh.position.x=Math.cos(a)*r; boss.mesh.position.z=Math.sin(a)*r;
    toast("The Stormcaller vanishes in a flash!"); spawnSparks(g,boss.mesh.position.x,2,boss.mesh.position.z,true); }
  // Tempest phase at 40%
  if(hpFrac<.4&&!boss.phase2){ boss.phase2=true; boss.speed=2.4; toast("⚡ STORMCALLER UNLEASHES THE TEMPEST!"); }
}

function updateHollow(g,dt,state,toast,setQuestTrack){
  const q=state.quest; if(!q) return;
  const boss=g.enemies.find(e=>e.type==="hollowking"&&!e.dead);

  // Heartstone pulse — grows brighter as boss loses HP
  if(g.heartstoneLight&&!q.hollowKingDefeated){
    const pct=boss?boss.hp/boss.maxhp:1;
    const t=Date.now()*.002;
    g.heartstoneLight.intensity=(1-pct)*5+.6+Math.sin(t)*.2;
    g.heartstoneLight.color.setHex(boss?0xaa44ff:0xffd080);
    if(g.heartstoneCrystal&&!q.hollowKingDefeated){ g.heartstoneCrystal.material.emissiveIntensity=(1-pct)*.8+.3; }
  }

  if(boss){
    const pct=boss.hp/boss.maxhp;
    // Phase 2 trigger (<65%)
    if(pct<.65&&!boss.phase2){
      boss.phase2=true; boss.speed=3.0;
      toast("◈ The Hollow King tears void from the air — PHASE 2!");
      for(let i=0;i<3;i++){ const a=i/3*Math.PI*2; spawnEnemy(g,Math.cos(a)*16,Math.sin(a)*16,"wraith",2,"hollow"); }
      if(g.heartstoneLight){ g.heartstoneLight.color.setHex(0xcc66ff); }
    }
    // Phase 3 trigger (<30%)
    if(pct<.30&&!boss.phase3){
      boss.phase3=true; boss.speed=4.0;
      toast("◈ THE HOLLOW KING BERSERKS — PHASE 3!");
      for(let i=0;i<4;i++){ const a=i/4*Math.PI*2+.4; spawnEnemy(g,Math.cos(a)*14,Math.sin(a)*14,"wraith",2,"hollow"); }
      if(g.heartstoneLight){ g.heartstoneLight.color.setHex(0xff88ff); g.heartstoneLight.intensity=8; }
    }

    // Shockwave attack (ring expanding from boss position)
    boss.shockCd=(boss.shockCd||8)-dt;
    const interval=boss.phase3?2.2:boss.phase2?3.8:7;
    if(boss.shockCd<=0&&boss.state==="chase"){
      boss.shockCd=interval;
      const cx=boss.mesh.position.x, cz=boss.mesh.position.z;
      const ring=new THREE.Mesh(new THREE.TorusGeometry(1,.22,8,36),new THREE.MeshBasicMaterial({color:0xaa33ff,transparent:true,opacity:.8}));
      ring.rotation.x=Math.PI/2; ring.position.set(cx,.12,cz); g.hollowGroup.add(ring);
      g.hollowShockwaves.push({ring,cx,cz,r:1,speed:12,dmg:boss.dmg*.75,hit:false});
      if(!boss.phase3) toast("⚠ Void shockwave — MOVE!");
    }

    // Phase 1 wraith summons
    if(!boss.phase2){ boss.summonCd=(boss.summonCd||12)-dt; if(boss.summonCd<=0){ boss.summonCd=12+Math.random()*5; const a=Math.random()*Math.PI*2; spawnEnemy(g,Math.cos(a)*16,Math.sin(a)*16,"wraith",1,"hollow"); } }

    // Boss HP bar (throttled)
    boss.hkT=(boss.hkT||0)+dt; if(boss.hkT>.15){ boss.hkT=0; g.setBoss?.({name:"The Hollow King",hp:Math.max(0,boss.hp),max:boss.maxhp}); }
  } else if(!q.hollowKingDefeated&&g.hollowKingSpawned){
    // boss dead but quest flag not set yet — bar will clear via death handler
  } else { g.setBoss?.(null); }

  // Update expanding shockwave rings
  if(g.hollowShockwaves){
    const p=g.player.position;
    g.hollowShockwaves=g.hollowShockwaves.filter(sw=>{
      sw.r+=sw.speed*dt;
      sw.ring.scale.setScalar(sw.r);
      sw.ring.material.opacity=Math.max(0,.8-sw.r*.035);
      // Damage player when ring passes through them
      const dist=Math.hypot(p.x-sw.cx,p.z-sw.cz);
      if(!sw.hit&&Math.abs(dist-sw.r)<1.4){ sw.hit=true; if(g.invuln<=0){ damagePlayer(g,state,sw.dmg,toast); toast("◈ Hit by void shockwave!"); } }
      if(sw.r>28){ sw.ring.parent?.remove(sw.ring); return false; }
      return true;
    });
  }

  // Animate crystal rotation
  if(g.heartstoneCrystal) g.heartstoneCrystal.rotation.y+=dt*.4;
}
function buildOverworldExtras(g){
  ...
  const RZ=-43;
  // Main water — full world width, wider, richer blue
  const water=new THREE.Mesh(new THREE.PlaneGeometry(180,9,1,1),new THREE.MeshLambertMaterial({color:0x2a68a8,transparent:true,opacity:.88,flatShading:true}));
  water.rotation.x=-Math.PI/2; water.position.set(0,.38,RZ); grp.add(water);
  // Shallow edges (lighter blue)
  [-1,1].forEach(s=>{ const sh=new THREE.Mesh(new THREE.PlaneGeometry(180,2.2),new THREE.MeshLambertMaterial({color:0x3a88cc,transparent:true,opacity:.6})); sh.rotation.x=-Math.PI/2; sh.position.set(0,.39,RZ+s*4.2); grp.add(sh); });
  // Banks — raised earth, slightly uneven
  [[-5.4,0x4a6a3a,3.5],[5.4,0x4a6a3a,3.5]].forEach(([dz,c,w])=>{ const bank=new THREE.Mesh(new THREE.BoxGeometry(180,.7,w),LM(c)); bank.position.set(0,.3,RZ+dz); grp.add(bank); });
  // Stone cap on banks
  [[-4.8,0x7a8a6a],[4.8,0x7a8a6a]].forEach(([dz,c])=>{ const cap=new THREE.Mesh(new THREE.BoxGeometry(180,.15,1.8),LM(c)); cap.position.set(0,.65,RZ+dz); grp.add(cap); });
  // River rocks scattered
  [[-60,0],[-40,2.8],[18,-3.2],[-25,-2],[46,2.5],[55,-2.8],[70,1.5],[-72,-2],[32,3],[-50,1]].forEach(([rx,rz])=>{
    const r2=Math.random()*.4+.3; const rock=new THREE.Mesh(new THREE.DodecahedronGeometry(r2,0),LM(0x6a7a6a)); rock.position.set(rx,.35+r2*.3,RZ+rz); rock.rotation.set(Math.random(),Math.random(),Math.random()); grp.add(rock);
  });
  // Reeds along north bank
  for(let i=0;i<22;i++){ const rx=-80+i*7.5+Math.random()*4; const rz2=RZ-5.2+Math.random()*.8;
    const stem=new THREE.Mesh(new THREE.CylinderGeometry(.06,.08,1.6+Math.random()*.5,5),LM(0x4a6a30)); stem.position.set(rx,.85,rz2); grp.add(stem);
    const top=new THREE.Mesh(new THREE.CylinderGeometry(.1,.2,.28,6),LM(0x3a4a20)); top.position.set(rx,1.75+Math.random()*.3,rz2); grp.add(top);
  }
  g.riverY=performance.now();
  // --- Bridge at x=0 (improved) ---
  const bridge=new THREE.Group();
  const deck=new THREE.Mesh(new THREE.BoxGeometry(5,.4,11),LM(0x6b4a2b)); deck.position.set(0,.75,RZ); bridge.add(deck);
  // Planks
  for(let i=0;i<9;i++){ const plank=new THREE.Mesh(new THREE.BoxGeometry(4.8,.15,1.0),LM(0x7a5230)); plank.position.set(0,.96,RZ-4.5+i*1.12); bridge.add(plank); }
  // Railings
  [-1,1].forEach(s=>{
    for(let i=0;i<7;i++){ const post=new THREE.Mesh(new THREE.BoxGeometry(.18,.9,.18),LM(0x5a3a1a)); post.position.set(s*2.4,1.3,RZ-3.5+i*1.2); bridge.add(post); }
    const rail=new THREE.Mesh(new THREE.BoxGeometry(.14,.14,11),LM(0x5a3a1a)); rail.position.set(s*2.4,1.72,RZ); bridge.add(rail);
  });
  // Stone pillars under bridge
  [-1,1].forEach(s=>{ const pil=new THREE.Mesh(new THREE.BoxGeometry(1.4,2.5,1.4),LM(0x7a7870)); pil.position.set(s*2.1,.5,RZ); bridge.add(pil); });
